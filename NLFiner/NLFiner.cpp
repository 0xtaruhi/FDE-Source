...
namespace FDU { namespace NLF {
	...
	void NLFiner::rebuild_verbose_netlist()
	{
		...
		if ( Library* design_prim_lib = _design->find_library(PRIMITIVE_LIB) ) return;
		copy_template_cell_lib();
		map<string, size_t> name_repo; // size_t will be initialized to 0 when the key is first visited
		for (Instance* inst: _design->top_module()->instances())
...
// 					FDU_LOG(DEBUG) << "Instance: " + inst->name();
// 				}
// 			}
// 		}
	...
	{
		Library* design_prim_lib   = _design->create_library(PRIMITIVE_LIB, false); // do not set external, else it won't output
	...
	void NLFiner::rebuild_verbose_instance( Instance* inst, map<string, size_t>& name_repo )
	{
		...
		auto corresponding_mod = 
			boost::find_if(cell_work_lib->modules(), [inst_type](Module* mod) { return mod->type() == inst_type; } );
		ASSERT(corresponding_mod != cell_work_lib->modules().end(), 
			"cannot find corresponding module \"" + inst_type + "\" in cell lib");

		string mod_name("c" + inst_type + "__" + boost::lexical_cast<string>(name_repo[inst_type]++) + "__");
		Module* new_mod = (*corresponding_mod)->clone(mod_name, design_work_lib);
		FDU_LOG(DEBUG) << inst->name() << " change down_module to " << new_mod->name();
		inst->set_down_module(new_mod);

		// redefine primitive module
		for (Instance* prim: new_mod->instances())
		{
			Module* mod = _design->find_library(PRIMITIVE_LIB)->find_module(prim->down_module()->name());
			ASSERT(mod, "Cannot find corresponding downmodule in " + PRIMITIVE_LIB + " in the design");
	...
	void NLFiner::refine_instance( Instance* inst )
	{
		...
		/// <1> load all configs
		vec_str configs;
		for (PropRepository::config_iter it  = inst->configs().begin(); it != inst->configs().end(); ++it)
		{
		...
		Property<bool>& NO_USER_LOGIC = create_temp_property<bool>(MODULE, "_NO_USER_LOGIC", false);
		if (block == "slice" && inst_type.property_value(NO_USER_LOGIC))
		{
			...
			for (Instance *inst: inst_type.instances())
			{
				if (to_lower_copy(inst->down_module()->name()) == "vcc" ||
					to_lower_copy(inst->down_module()->name()) == "gnd")   continue;
		...
		do {
			...
			op_sweep(inst_type);
		} while (inst_type.num_instances() != num_inst || inst_type.num_nets() != num_net);

		if (block == "slice" && !inst_type.property_value(NO_USER_LOGIC)) // something related to the LUT
			handle_WSGEN(inst_type);
	}

	// restructure cell for a single op
	void NLFiner::restructure(Module &cell, const vec_str& lib_names, string bit, string op)
	{
		if (lib_names.size() == 0) return;
		...
		else if (to_lower_copy(operations[0])=="vcc" || to_lower_copy(operations[0])=="gnd")
		{
		...
		else if (to_lower_copy(operations[0])=="dummy")
		{
			...
			Property<bool>& no_user_logic = create_temp_property<bool>(MODULE, "_NO_USER_LOGIC");
	...
	void NLFiner::op_mux(Module &cell, const vec_str& lib_names, const string& bit)
	{
		for (const string& prim: lib_names)
		{
			Module::inst_iter it = boost::find_if(cell.instances(), match_cell(prim));
			...
			if (actual_bit == "0") actual_bit = "I0"; else
			if (actual_bit == "1") actual_bit = "I1";

			for (Pin *pin: prim_mux->pins())
			{	
				if ( to_lower_copy(pin->port()->name()) != to_lower_copy(actual_bit) && !pin->is_source() )
				{	// the input of mux (not named bit) will be removed
					pin->disconnect();
				}
			}
	...
	void NLFiner::op_vccgnd(Module &cell, const vec_str& lib_names, const string& bit)
	{
		...
		Module::inst_iter it = boost::find_if(cell.instances(), match_cell(prim_name));
	...
	void NLFiner::op_attr(Module &cell, const vec_str& lib_names, const string& bit, const string& prop_name)
	{
		for (const string& prim: lib_names)
		{
			Module::inst_iter it = boost::find_if(cell.instances(), match_cell(prim));
			Instance *prim_attr = (it == cell.instances().end()) ? 0 : *it; // such as LUT, init0
			if (prim_attr == NULL) continue;
	...
	void NLFiner::handle_WSGEN(Module& newCell)
	{
		...
		if (f) f_type = f->property_value(type);
		if (g) g_type = g->property_value(type);

		if ((f_type == "LUT" || f_type == "") && (g_type == "LUT" || g_type == ""))
		{
			Module::inst_iter wit = boost::find_if(newCell.instances(), match_cell("WSGEN"));
			Instance *wsgen = (wit == newCell.instances().end()) ? 0 : *wit;	
			if (wsgen) wsgen->release();
			if (f)
			{
	...
	void NLFiner::op_sweep(Module &cell)
	{
		...
		while(inst_it != cell.instances().end())
		{
			...
			{
				if (pin->is_sink() && pin->net() != NULL)
				{	// input pin of the instance
					num_conn_ipin++;
				}
				else if (pin->is_source() && pin->net() != NULL)
			...
			if (to_lower_copy(inst_it->down_module()->name()) == "vcc" && num_conn_outpin != 0)
			{
				++inst_it;
				continue;
			}
			if (to_lower_copy(inst_it->down_module()->name()) == "gnd" && num_conn_outpin != 0)
			{
				++inst_it;
				continue;
			}
			...
			if (num_conn_ipin == 0 || num_conn_outpin == 0)
			{	// either no connected input pins or output pins means the instance/prmitive is unused
				for (Pin* pin: inst_it->pins()) pin->disconnect();
		...
		while (net_it != cell.nets().end())
		{	// note: pin's unhook is done in step 1
			if (net_it->sink_pins().begin()==net_it->sink_pins().end () || net_it->source_pins().begin()==net_it->source_pins().end())
			{
				while (!net_it->pins().empty())
				{	// all pins removed
