...
namespace FDU { namespace NLF {
	...
	class NLFiner
	{
		struct match_cell {
			...
			bool operator() (const Instance* t) { return to_match == t->down_module()->name(); }
		private:
			string to_match;
		};
	...
	private:
		...
		void restructure(Module &cell, const vec_str& lib_names, string bit, string op);
		...
		void op_attr    (Module &cell, const vec_str& lib_names, const string& bit, const string& property_name);
