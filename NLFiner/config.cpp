...
namespace FDU { namespace NLF {
	...
	void Config::debug_info() const
	{
		...
		for (map<string, string>::const_iterator iter=_bit_map.begin(); iter!=_bit_map.end(); iter++)
		{
...
// 	}
	void ConfigLoader::load( const string& file_name )
	{
		...
		if (node->name() == string("library"))
			foreach_child(block_node, node, "block")
				load_block(block_node);
		else
			ASSERT(0, "Parse failed. Config lib format error, first node should be library.");
	}
	...
	void ConfigLoader::load_block( xml_node* node )
	{
		...
		foreach_child(prim_node, node, "primitive")
			load_prim(prim_node);
	}
	...
	void ConfigLoader::load_prim( xml_node* node )
	{
		...
		ASSERT(alib_names.size(), "parse config lib failed. lib name not found at primitive " + aname);
		string aposes(get_attribute(node, "pos"));
		ASSERT(aposes.size(), "parse config lib failed. pos not found at primitive " + aname);

		char_separator<char> sep(" \t\n|");
		tokenizer<char_separator<char> > lib_tokens(alib_names, sep);
		for (const string& token: lib_tokens) prim->add_lib_name(token);
		tokenizer<char_separator<char> > pos_tokens(aposes, sep);
		for (const string& token: pos_tokens) prim->add_bit_pos(lexical_cast<size_t>(token));

		foreach_child(bit_node, node, "config")
			load_bit(bit_node, prim);
	...
	Config::vec_str ConfigRepo::find_lib_names(const string& block, const string& prim) const
	{
		...
		{
			FDU_LOG(WARN) << "fail to lookup \"" << block_name << "\" in configuration rule data base";
		...
		if (prim_it == block_it->second.end())
		{
			if(prim_name != "_BEL_PROP")
				FDU_LOG(WARN) << "fail to lookup \"" << block_name << "::" << prim_name << "\" in configuration rule data base";
			return Config::vec_str();
		}
	...
	Config::vec_int ConfigRepo::find_bit_poses(const string& block, const string& prim) const
	{
		string block_name(to_lower_copy(block));
		string prim_name(to_upper_copy(prim));
		repo::const_iterator block_it = _config.find(block_name);
		if (block_it == _config.end())
		{
			FDU_LOG(WARN) << "fail to lookup \"" << block_name << "\" in configuration rule data base" << endl;
		...
		{
			if(prim_name != "_BEL_PROP")
				FDU_LOG(WARN) << "fail to lookup \"" << block_name << "::" << prim_name << "\" in configuration rule data base" << endl;
			return Config::vec_int();
		}
	...
	Config::bit_pair ConfigRepo::get_operation(const string& block,   ///< instance module name (SLICE or IOB)
											  ...
											  ) const
	{
		...
		vector<string> conf_sep_list;  // separated configuration vBit (vBit[0]=BXMUX, vBit[1]=OFF, etc.)
		...
		if (bit_poses.size() == 0) return Config::bit_pair("none", "none");
		...
		if (match != 1)
		{
			for (const string& op: conf_sep_list)
			{	// find one OFF
				if (op == "OFF")
				{
					match = 1;
