...
namespace FDU { namespace NLF {
	...
	public:
		...
		void     add_bit_pair(const string& bit, const string& op) { _bit_map.insert(make_pair(bit, op)); }
	...
	public:
		...
		void add_config(const string& block, const string& pm, Config* pmConfig) { _config[block][pm] = pmConfig; }
		...
		Config::bit_pair get_operation (const string& block, const string& config) const;
	...
	inline Config::bit_pair Config::find_bit_pair(const string& bit) const
	{
		...
		if (iter != _bit_map.end()) return *iter;
