...
namespace BitGen { namespace bitstream {
	...
	{	
		for (FDU::cil_lib::bstrCMD* bstrcmd : _refbstrCMDLib->bstrCMDs())
		{
			...
			{
				int major_shift = _refbstrCMDLib->getbstrParameter("major_shift").getParameterValue();
				int initialNum = _refbstrCMDLib->getbstrParameter("initialNum").getParameterValue();
				int bits_per_grp_reversed = _refbstrCMDLib->getbstrParameter("bits_per_grp_reversed").getParameterValue();
				writeNomalTiles(major_shift, bits_per_grp_reversed, initialNum);
			}
			else if (cmd == "writeMem")
			{
				int wrdsAmnt_shift = _refbstrCMDLib->getbstrParameter("wrdsAmnt_shift").getParameterValue();
				int fillblank      = _refbstrCMDLib->getbstrParameter("fillblank").getParameterValue();
				int mem_amount     = _refbstrCMDLib->getbstrParameter("mem_amount").getParameterValue();
