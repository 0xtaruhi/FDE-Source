...
namespace BitGen { namespace bitstream {
	...
	int CMDLoaderBase::getFRMWords(std::vector<unsigned int> &FRMWords, int tileCol, int FRM, int bits_per_grp_reversed, int initialNum){
	...
	int CMDLoaderBase::getMajorWords(std::vector<unsigned int>& majorWords, int tileCol, int FRMAmnt, int bits_per_grp_reversed, int initialNum){
		int majorWrdsAmnt = 0;
		std::vector<unsigned int> FRMWords;
		for(int FRMIdx = 0; FRMIdx < FRMAmnt; ++FRMIdx){
			majorWrdsAmnt += getFRMWords(FRMWords, tileCol, FRMIdx, bits_per_grp_reversed, initialNum);
	...
	void CMDLoaderBase::bsHeader() {
		_ofs << "0000_0000" << "\t// chip_type: " << args._device << '\n';
		_ofs << "0000_0000" << "\t// bit file name: " << args._bitstream << '\n';
		_ofs << "0000_0000" << "\t// current time: " << timestamp() << '\n';
		fillBlank(3);
	}
	...
	void CMDLoaderBase::mapOutFDRI(int wordLength){ 
		_ofs << "3000_4000" << "\t//" << std::dec << wordLength << " words" << "\n";
	...
	void CMDLoaderBase::writeNomalTiles(int major_shift, int  bits_per_grp_reversed,int initialNum){
...
//partial bitstream
			if (args._partialbitstream >= 0)
			{
				if(tileCol == args._partialbitstream){
					int majorWrdsAmnt = getMajorWords(majorWords, tileCol, FRMAmount, bits_per_grp_reversed, initialNum);
