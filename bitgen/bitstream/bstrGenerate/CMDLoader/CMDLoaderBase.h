...
namespace BitGen { namespace bitstream {
	...
	public:
/*		static const int _BITS_PER_FRM = 396; // not used, just for FDP1000K*/
	...
	protected:
		...
		int getFRMWords(std::vector<unsigned int> &FRMWords, int tileCol, int FRM, int bits_per_grp_reversed, int initialNum);
		int getMajorWords(std::vector<unsigned int>& majorWords, int tileCol, int FRMAmnt, int bits_per_grp_reversed, int initialNum);
		...
		void writeNomalTiles(int major_shift, int  bits_per_grp_reversed, int initialNum);
...
// inline functions
	...
	inline unsigned int CMDLoaderBase::toWord(const std::vector<int>& bits, int wordOffs){
		unsigned int word = 0;
	...
	inline void CMDLoaderBase::wrdToOstream(unsigned int base, unsigned int wordNum){
	...
	inline void CMDLoaderBase::wrdsToOstream(const std::vector<unsigned int>& words){
		for(unsigned int word: words)
			wrdToOstream(0, word);
	}

	inline void CMDLoaderBase::memToOstream(const BitGen::bitstream::dfotMemBstr::BitList& bitList){
		for(const std::string& str: bitList)
			_ofs << str << "\n";
	}

	inline void CMDLoaderBase::mountSMFA(int majorAddr, int major_shift)//80k,shift:19   1000k:17
	{
		_ofs << "3000_2001" << "\n";
		wrdToOstream(0, majorAddr << major_shift);
	}

	inline void CMDLoaderBase::reverse(std::vector<int>& FRMBits, int bits_per_grp_reversed)
	{
