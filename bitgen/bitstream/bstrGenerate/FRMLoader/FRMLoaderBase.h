...
namespace BitGen { namespace bitstream {
	...
	protected:
		...
		void bitsToOstream(const std::vector<int>& colBits, const sizeSpan& bitSize, int majorAddr);
	...
	inline void FRMLoaderBase::tellAddr(int addr, const std::string& type, int bits){ // 6 bits for each addr
		_ofs << ((addr >> (--bits)) & 0x01) << type << addr << "\n";
		for (int i = (--bits); i >= 0; --i)
			_ofs << ((addr >> i) & 0x01) << "\n";
	}

	inline void FRMLoaderBase::bitsToOstream(const std::vector<int> &colBits, const sizeSpan &bitSize, int majorAddr){
		int major_addr_bits = _refbstrCMDLib->getbstrParameter("major_addr_bits").getParameterValue();
		int frame_addr_bits = _refbstrCMDLib->getbstrParameter("frame_addr_bits").getParameterValue();
		int bitRowAmnt = bitSize._rowSpan, bitColAmnt = bitSize._columnSpan, bitIdx = 0;
