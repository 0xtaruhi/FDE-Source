...
namespace BitGen { namespace bitstream {
	...
	inline void FRMLoader1000K::tellAddrFrm(int addr, const std::string& type){	//6 bits for each addr
		tellAddrMajor(addr,type);
	}

	inline void FRMLoader80K::tellAddrFrm(int addr, const std::string& type) {	//6 bits for each addr
