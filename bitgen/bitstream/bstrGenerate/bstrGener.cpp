...
namespace BitGen { namespace bitstream {
	...
	void bstrGener::generate(const std::string& bstrPath, 
	...
	void bstrGener::showLog(const std::string& workDir){
		...
		//string cktPath  = workDir + "\\";
 		...
 		{
			xmlWriter nmlWriter(xmlWriter::NORMAL, *dfotTile), ovlWriter(xmlWriter::OVERLAPS, *dfotTile);
		...
		if (cktTile->isUsed())
		{
			xmlWriter nmlWriter(xmlWriter::NORMAL, *cktTile), ovlWriter(xmlWriter::OVERLAPS, *cktTile);
