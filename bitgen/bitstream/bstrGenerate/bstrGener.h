	...
	public:
		bstrGener(Circuit* refCkt, cilLibrary* refCilLib) 
			: _dfotTiles(refCilLib->getTileLib()), 
			  ...
			  _cktMems	(&_dfotMems),
			  _CMDLoader(&_cktMems, &_cktTiles, refCilLib->getMajorLib(), refCilLib->getbstrCMDLib()),
			  _FRMLoader(&_cktMems, &_cktTiles, refCilLib->getMajorLib(), refCilLib->getbstrCMDLib()) {}

		void generate(const std::string& bstrPath, 
