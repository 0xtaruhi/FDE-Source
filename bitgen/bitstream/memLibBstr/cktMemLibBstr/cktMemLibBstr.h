...
namespace BitGen { namespace bitstream {
	...
	inline cktMemBstr& cktMemLibBstr::getCktMem(const std::string& memName){
		cktMemIter it = find_if(cktMems(), [&memName](const cktMemBstr* cktMem) { return cktMem->getName() == memName; });
		ASSERTD(it != _cktMems.end(), "cktMemLib: not exist such BRAM named ... " + memName);
		return **it;
	}

	inline void cktMemLibBstr::getMemContents(dfotMemBstr::BitList& bitList, int memIdx){
