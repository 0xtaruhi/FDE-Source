...
namespace BitGen { namespace bitstream {
	...
	public:
		...
		explicit cktTileBstr(dfotTileBstr* refDfotTile = 0) : _refDfotTile(refDfotTile) 
		{ construct(); }
	...
	inline int cktTileBstr::getFRMBits(std::vector<int>& FRMBits, int FRM){
		if(_used) return dfotTileBstr::getFRMBits(FRMBits, FRM);
		else	  return _refDfotTile->getFRMBits(FRMBits, FRM);
	}
