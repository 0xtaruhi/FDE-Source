...
//		checkOverlaps();
	...
	void cktTileLibBstr::initialize(const std::string& device){
		...
		for(const Instance* inst: scaleLib->find_module(device)->instances()){
			...
			int bitPos = archInst->bit_pos().x*_fpgaScale._columnSpan + archInst->bit_pos().y;
			_cktTiles.at(bitPos) = new cktTileBstr(&_refDfotTileLib->getDfotTile(archInst->down_module()->name()));
	...
	void cktTileLibBstr::recordCktCfgs1000K(){
		...
		for(cfgElem cfg: cfgs){
			...
			if(archlib->is_io_bound(siteName)){ 
				cfg._siteName = "IOB" + to_string((archlib->find_pad_by_name(siteName)).z);
	...
	void cktTileLibBstr::recordCktCfgsFDP80K(){
		...
		{
// 			padFORsopc = "N15 N16 N14 P16 T14 T13 P13 R13 N12 P12 R10 T10 N9 P9 R9 T9 T8 R8 N8 R7 P5 T5 R4 P4 T4 T3 P1 N1 N3 N2 N4 P8 K3 K2";
			//for xdl format netlist
			padFORsopc = "K15 K14 K13 L14 L13 M16 M15 M14 M13 N15 N14 N16 P16 T14 T13 P13 R13 N12 P12 R10 T10 N9 P9 R9 T9 T8 R8 P8 N8 R7 P5 N5 R4 P4 T4 T3 P1 N1 N3 N2 M4 K16 J14 J15 J13";
		...
		for(cfgElem& cfg: cfgs){
				...
				if(cfg._cfgElemName == "INIT_A" || cfg._cfgElemName == "INIT_B")
					...
					BitGen::bitstream::cktTileBstr::tileBstr* ckttileBstr = &pcktTile->getTileBstr();
					...
					if(cfgType == "INITP"){
						BitGen::bitstream::cktTileBstr::tileBstr& ckttileBstr_i = pcktTile->getTileBstr();
						...
						int word_bl[256];//存放电路文件xdl的相关信息
						...
						for (int ix_tileBstr = 0; ix_tileBstr < 8; ++ix_tileBstr) {
							for (int idx = 0; idx < 32; ++idx) {
								ckttileBstr_i[frm * 8 + ix_tileBstr + 64 * bram_p[idx]] = word_bl[idx + ix_tileBstr * 32];
							}
						}
					}
						...
						BitGen::bitstream::cktTileBstr::tileBstr& ckttileBstr_i = pcktTile->getTileBstr();
						if (ckttileBstr_i.empty()) {//预处理校验位不需要初始化的情况
							for (int idx_sram = 0; idx_sram < 320 * 8 * 8; ++idx_sram) {
								ckttileBstr->push_back(0);
							}
						}
	...
	void cktTileLibBstr::recordCktCfgs(){
		if (args._device == CHIPTYPE::FDP1000K || args._device == CHIPTYPE::FDP3000K)
			recordCktCfgs1000K();
		else if(args._device == CHIPTYPE::FDP80K || args._device == CHIPTYPE::FDP500K || args._device == CHIPTYPE::FDP500KIP)
			recordCktCfgsFDP80K();
	}
	...
	void cktTileLibBstr::recordCktPips(){
		...
		for(pipInfo& pip: pips){
			...
			route._srcNet    = pip._srcNet; route._snkNet = pip._snkNet;
		}
	}
	...
	sizeSpan cktTileLibBstr::getColBits(std::vector<int>& colBits, int tileCol){
		...
		ASSERTD(tileCol >= 0 && tileCol < cols, "cktTileLib: col of tile out of range");
		...
		for(int row = 0; row < rows; ++row){
			...
			ASSERTD(
				bitColAmnt == bitArrySize._columnSpan, 
				"cktTileLib: bit-array not align at tile ... " + boost::lexical_cast<string>(sizeSpan(row, tileCol))
			);
	...
	void cktTileLibBstr::checkOverlaps(){
		for (cktTileBstr* cktTile : cktTiles())
			if (cktTile->checkOverlaps())
				FDU_LOG(WARN) << Warning("bitgen: overlap bits exist at circuit tile - " + cktTile->getName());
	}
