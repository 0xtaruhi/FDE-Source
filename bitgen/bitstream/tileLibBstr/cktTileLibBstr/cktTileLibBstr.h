...
namespace BitGen { namespace bitstream {
	...
	public:
		cktTileLibBstr(FDU::cil_lib::cilLibrary*     refCil = 0,
					   ...
					   BitGen::bitstream::cktMemLibBstr*   refCktMemLib = 0
					      ...
					      _refCkt(refCkt),
						  _refDfotTileLib(refDfotTileLib),
	...
	inline cktTileBstr& cktTileLibBstr::getTile(const std::string& tileName){
		cktTileIter it = find_if(cktTiles(), [&tileName](const cktTileBstr* tile) { return tile->getName() == tileName; });
		ASSERTD(it != _cktTiles.end(), "cktTileLib: no such tile named ... " + tileName);
		return **it;
	}
	...
	inline cktTileBstr& cktTileLibBstr::getTile(const sizeSpan &tilePos){
		...
		ASSERTD(
			(tilePos._rowSpan    >= 0 && tilePos._rowSpan    < rows)
		  &&(tilePos._columnSpan >= 0 && tilePos._columnSpan < cols),
		  "cktTileLib: no such tile located ... " + boost::lexical_cast<string>(tilePos)
	    );
		return *_cktTiles.at(tilePos._rowSpan*cols + tilePos._columnSpan);
	}

	inline sizeSpan cktTileLibBstr::getFRMBits(std::vector<int>& FRMBits, int tileCol, int FRM){
		int rows = _fpgaScale._rowSpan, cols = _fpgaScale._columnSpan, FRMBitsAmnt = 0;
		ASSERTD(tileCol >= 0 && tileCol < cols, "cktTileLib: col of tile out of range");
		for(int i = 0; i < rows; ++i) FRMBitsAmnt += getTile(sizeSpan(i, tileCol)).getFRMBits(FRMBits, FRM);
