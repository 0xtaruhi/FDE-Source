...
namespace BitGen { namespace bitstream {
	...
	struct contBitsDfotTileBstr{
		...
		bitTile& addBit(const bitTile& bit) { _bits.push_back(bit); return _bits.back(); }
	...
	inline bool contBitsDfotTileBstr::existLodgerBits(){
		Bits::iterator it = std::find_if(_bits.begin(), _bits.end(), [](const bitTile& bit) { return bit.isLodger(); });
		return it != _bits.end();
	}

	inline bool contBitsDfotTileBstr::hasOffsetBits(){
		Bits::iterator it = std::find_if(_bits.begin(), _bits.end(), [](const bitTile& bit) { return bit.hasOffset(); });
		return it != _bits.end();
	}

	inline bool contBitsDfotTileBstr::isEmpty(){
		return _bits.empty();
	}
