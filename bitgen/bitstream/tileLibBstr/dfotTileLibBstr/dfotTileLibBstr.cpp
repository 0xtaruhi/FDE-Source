...
namespace BitGen { namespace bitstream {
	...
	void dfotTileLibBstr::regulate(){
		...
		for(dfotTileBstr* tile: dfotTiles()){
			...
			for(bitTile& bit: lodgerBits){
				dfotTileIter it = find_if(dfotTiles(), [&bit](const dfotTileBstr* dfotTile) { return dfotTile->getName() == bit._ownerTile; });
				ASSERTD(it != _dfotTiles.end(), "lodger bit: does not exist my owner Tile ... " + bit._ownerTile);
	...
	void dfotTileLibBstr::checkOverlaps(){
		...
		if (dfotTile->checkOverlaps())
			FDU_LOG(WARN) << Warning("bitgen: overlap bits exist at default tile - " + dfotTile->getName());
	}
