...
namespace FDU {
	namespace cil_lib {
		...
		public:
			...
			using const_bstrParametersType = cilContainer<bstrParameter>::const_range_type;
		...
		public:
			...
			const_bstrParametersType bstrParameters() const { return _bstrParameters.range(); }
			bstrParameter* addbstrParameter(bstrParameter* bstrparameter) { return _bstrParameters.add(bstrparameter); }
			bstrParameter& getbstrParameter(std::string name){ return *bstrParameters().find(name); }
