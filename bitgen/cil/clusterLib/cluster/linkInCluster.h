...
namespace FDU { namespace cil_lib {
	...
	public:
		linkCluster(const std::string& refSiteName, siteLib* refSiteLib, sizeSpan containerSize) 
			: CilBase("link"), _refSiteName(refSiteName), _refSiteLib(refSiteLib), _containerSize(containerSize)
		{ construct(); }
		
		void construct();
		pinInLinkClus& setSrcOrSnk(pinType type, const std::string& pin, int row, int col);
	...
	public:
		explicit contLinksCluster(siteLib* refSiteLib = 0) : _refSiteLib(refSiteLib) {}
