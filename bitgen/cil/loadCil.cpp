...
namespace FDU { namespace cil_lib {
	...
	cilLibrary* CilLibHandler::loadCil(xml_node* node)
	{
		...
		foreach_child(eleLibNode, node, "element_library")
			loadEleLib(eleLibNode);
		foreach_child(siteLibNode, node, "site_library")
			loadSiteLib(siteLibNode);
		foreach_child(clstLibNode, node, "cluster_library")
			loadClstLib(clstLibNode);
		foreach_child(transLibNode, node, "transmission_library")
			loadTrnsLib(transLibNode);
		foreach_child(tileLibNode, node, "tile_library")
			loadTileLib(tileLibNode);
		foreach_child(majorLibNode, node, "major_library")
			loadMajorLib(majorLibNode);
		foreach_child(bramLibNode, node, "bram_library")
			loadBramLib(bramLibNode);
	...
	void CilLibHandler::loadElem(xml_node* node, elemLib* lib)
	{
		...
		lib->addElement(ele);
		foreach_child(srams, node, "sram_info")
			loadSrams(srams, ele);
		foreach_child(paths, node, "path_info")
			loadPaths(paths, ele);
	}

	void CilLibHandler::loadSrams(xml_node* node, Element* ele)
	{
		foreach_child(sram, node, "sram")
			loadSram(sram, &ele->getSrams());
	}

	void CilLibHandler::loadPaths(xml_node* node, Element* ele)
	{
		foreach_child(path, node, "path")
			loadPath(path, &ele->getPaths());
	}
	...
	void CilLibHandler::loadPath(xml_node* node, contPathsElem* paths)
	{
		...
		if(seg[0] != 'n')
			segBool = true;
		pathElem* path = new pathElem("", get_attribute(node, "in"), get_attribute(node, "out"), segBool);
		paths->addPath(path);

		foreach_child(cfgs, node, "configuration_info")
		{
			foreach_child(cfg, cfgs, "sram")
				loadSram(cfg, &path->getCfgInfo());
		}
	}
	...
	{
		Site& site = *lib->addSite(new Site(get_attribute(node, "name"), _cilLib->getElemLib()));
	...
	void CilLibHandler::loadSramName(xml_node* node, sramNameSpSite& sramNameSp)
	{
		sramNameInNameSpSite& sramName =
			*sramNameSp.addSramName(new sramNameInNameSpSite(get_attribute(node, "name"), _cilLib->getElemLib()));
		foreach_child(bCellNode, node, "basic_cell")
		{
			sramName.addBasicCell(
				new bCellInSramNameSite(
					get_attribute(bCellNode, "name"),
					get_attribute(bCellNode, "sram"),
					_cilLib->getElemLib()));
		}
	}

	void CilLibHandler::loadCfgElem(xml_node* node, cfgElemsSite& cfgElems, sramNameSpSite& sramNameSp)
	{
		cfgElemSite& cfgElem = *cfgElems.addCfgElem(
			new cfgElemSite(get_attribute(node, "name"), _cilLib->getElemLib(), &sramNameSp));
		foreach_child(funcInCfgNode, node, "function")
			loadFuncInCfg(funcInCfgNode, cfgElem, sramNameSp);
	}

	void CilLibHandler::loadFuncInCfg(xml_node* node, cfgElemSite& cfgElem, sramNameSpSite& sramNameSp)
	{
		string dft(get_attribute(node, "default"));
		...
		funcInCfgElemSite& funcInCfg = *cfgElem.addFunc(
			new funcInCfgElemSite(get_attribute(node, "name"), quomodo, manner, boolDft, _cilLib->getElemLib(), &sramNameSp));
		foreach_child(sramInFuncNode, node, "sram")
			loadSramInFunc(sramInFuncNode, funcInCfg, sramNameSp);
	}

	void CilLibHandler::loadSramInFunc(xml_node* node, funcInCfgElemSite& funcInCfgElem, sramNameSpSite& sramNameSp)
	{
		...
		funcInCfgElem.addSram(
			new sramInCfgElemSite(
				get_attribute(node, "basic_cell"), get_attribute(node, "name"),
				content, optAddr, _cilLib->getElemLib(), &sramNameSp));
	}
	...
	{
		Cluster& clst = *lib->addCluster(
			new Cluster(get_attribute(node, "name"),
						get_attribute(node, "type"),
						_cilLib->getSiteLib()));
		foreach_child(dimensionNode, node, "dimension")
			loadDimension(dimensionNode, clst);
		foreach_child(linkNode, node, "link_info")
			loadLinks(linkNode, clst);
	}
	...
	void CilLibHandler::loadHrzORVtc(xml_node* node, dimenCluster& dim, dimenCluster::direction dir)
	{
		dim.setSizeByDirect(dir,
			stoi(get_attribute(node, "cluster_size")),
	...
	void CilLibHandler::loadLinks(xml_node* node, Cluster& clst)
	{
		...
		{
			linkCluster& link = *links.addLink(
				new linkCluster(clst.getRefSiteName(),
				_cilLib->getSiteLib(),
	...
	void CilLibHandler::loadSrcORSnk(xml_node* node, linkCluster& link, linkCluster::pinType type)
	{
		link.setSrcOrSnk(type,
			get_attribute(node, "pin_name"),
	...
	void CilLibHandler::loadTrnsLib(xml_node* node)
	{
		...
		{
			_cilLib->getTrnsLib()->addTrans(new Trans(
				get_attribute(transNode, "name"),
	...
	{
		Tile& tile = *lib->addTile(new Tile(
		...
		foreach_child(clstNode, clstsNode, "cluster")
			loadClstInTile(clstNode, tile);
	...
	void CilLibHandler::loadClstInTile(xml_node* node, Tile& tile)
	{
		...
		sizeSpan lct = location == "" ? sizeSpan(-1, -1) : lexical_cast<sizeSpan>(location);
	...
	void CilLibHandler::loadSiteInClst(xml_node* node, clstTile& clst, Tile& tile)
	{
		...
		sizeSpan pst = pos == "" ? sizeSpan(-1, -1) : lexical_cast<sizeSpan>(pos);//VCC does not have a position info.
		siteInTile& site = *clst.addSite(new siteInTile(
			get_attribute(node, "name"),
			clst.getRefSiteName(),
			clst.getRefTileName(),
			pst,
	...
	void CilLibHandler::loadSiteInTrans(xml_node* node, trnsTile& trans, Tile& tile)
	{
		string pos(get_attribute(node, "position"));
		sizeSpan pst = pos == "" ? sizeSpan(-1, -1) : lexical_cast<sizeSpan>(pos);
		siteInTile& site = *trans.addSite(new siteInTile(
			get_attribute(node, "name"),
			trans.getRefSiteName(),
			trans.getRefTileName(),
			pst,
			_cilLib->getSiteLib()));
		foreach_child(siteSramNode, node, "site_sram")
			loadSiteSram(siteSramNode, site, tile);
...
// #define CHECK_SRAM_REDEFINE
	void CilLibHandler::loadSiteSram(xml_node* node, siteInTile& site, Tile& tile)
	{
		foreach_child(sramNode, node, "sram")
		{
			...
			sizeSpan off = offset == "" ? sizeSpan(0, 0) : lexical_cast<sizeSpan>(offset);
			sizeSpan localPlace = lexical_cast<sizeSpan>(get_attribute(sramNode, "local_place"));
			sramInSiteSramTile* newSram = new sramInSiteSramTile(
				get_attribute(sramNode, "sram_name"),
				get_attribute(sramNode, "basic_cell"),
				localPlace,
				off,
			...
			if(site.hasSram(*newSram)){
				FDU_LOG(WARN) << newSram->getBasicCell() + " : " + newSram->getName() << " sram redefined in tile " << tile.getName();
				delete newSram;
			} else {
#endif
	...
	void CilLibHandler::loadMajorLib(xml_node* node)
	{
		...
		{
			_cilLib->getMajorLib()->addMajor(new Major(
				stoi(get_attribute(majorNode, "address")),
	...
	void CilLibHandler::loadBramLib(xml_node* node)
	{
		foreach_child(bramNode, node, "bram")
		{
			_cilLib->getBramLib()->addBram(new Bram(
				...
				stoi(get_attribute(bramNode, "bl")),
				get_attribute(bramNode, "type")));
		}
	}
	...
	void CilLibHandler::loadbstrCMD(xml_node* node)
	{
		foreach_child(cmdNode, node, "parameter"){
			_cilLib->getbstrCMDLib()->addbstrParameter(new bstrParameter(
				get_attribute(cmdNode, "name"),
				std::stoi(get_attribute(cmdNode, "value"))));
		}
		foreach_child(cmdNode, node, "command")
		{
			_cilLib->getbstrCMDLib()->addbstrCMD(new bstrCMD(
			get_attribute(cmdNode, "cmd"),
			get_attribute(cmdNode, "parameter")));
		}
	}
