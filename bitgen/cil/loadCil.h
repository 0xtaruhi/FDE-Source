...
namespace FDU {namespace cil_lib {
	...
	private:
		...
		void loadCfgElem(xml_node* node, cfgElemsSite& cfgElems, sramNameSpSite& sramNameSp);
		void loadFuncInCfg(xml_node* node, cfgElemSite& cfgElem, sramNameSpSite& sramNameSp);
		void loadSramInFunc(xml_node* node, funcInCfgElemSite& funcInCfgElem, sramNameSpSite& sramNameSp);
		...
		void loadHrzORVtc(xml_node* node, dimenCluster& dim, dimenCluster::direction dir);
		void loadSrcORSnk(xml_node* node, linkCluster& link, linkCluster::pinType type);
