...
/// sram_namespace in config_info
	...
	struct bCellInSramNameSite {
		...
		bCellInSramNameSite(const std::string& name, const std::string& sram, elemLib* refElemLib = 0) 
	...
	public:
		...
		bCellInSramNameSite* addBasicCell(bCellInSramNameSite* basicCell) {return _basicCells.add(basicCell); }
	...
	public:
		...
		using const_sramNamesType = cilContainer<sramNameInNameSpSite>::const_range_type;
	...
	public:
		...
		sramNameInNameSpSite* addSramName(sramNameInNameSpSite* sramName) { return _sramNames.add(sramName); }
		sramNameInNameSpSite& getSramName(const std::string &sramName) { return *sramNames().find(sramName); }
	};
...
/// cfg_element in config_info
	...
	struct sramInCfgElemSite {
		...
		sramInCfgElemSite(const std::string& bCell, const std::string& name, int content,int optAddr = -1,
	...
	public:
		funcBase(const std::string& name, const std::string& quomodo = "", const std::string& manner = "") 
			: CilBase(name), _quomodo(quomodo), _manner(manner) {}

		std::string getQuomodo() const { return _quomodo; }

		void transform(cfgFunc& func){ func._name = _name; func._quomodo = _quomodo; func._manner = _manner; }
	};
	...
	public:
		funcInCfgElemSite(const std::string& name, const std::string& quodomo = "", const std::string& manner = "", 
			              bool defaultOrNot = false, elemLib* refElemLib = 0, sramNameSpSite* refSramNameSp = 0) 
			: funcBase(name, quodomo, manner), _default(defaultOrNot), _refElemLib(refElemLib), _refSramNameSp(refSramNameSp) 
		{}
	...
	public:
		cfgElemSite(const std::string& name, elemLib* refElemLib = 0, sramNameSpSite* refSramNameSp = 0) 
		...
		funcInCfgElemSite* addFunc(funcInCfgElemSite* func) { return _functions.add(func); }
		funcInCfgElemSite* getFunc(const std::string& name) { return functions().find(name); }
	...
	public:
		...
		cfgElemSite* addCfgElem(cfgElemSite* cfgElem)    { return _cfgElements.add(cfgElem); }
		cfgElemSite& getCfgElem(const std::string& name) { return *cfgElems().find(name); }
