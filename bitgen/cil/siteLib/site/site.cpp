...
namespace FDU { namespace cil_lib {
	...
	pathInfo Site::getPath(const routeInfo& route){
		...
		if(!src->is_mpin())
		{
				...
				if(!snk->is_mpin())
			{
				if(src->instance()->name() == snk->instance()->name()){
					Element* elem = _refElemLib->getElement(src->instance()->down_module()->name());
		...
		if(pathAmount == 0)
			// used for dummy path such like "F5COUPLING3 -> S0_X", so change exception to warning
			FDU_LOG(WARN) << Warning("GRM: no required path here ... [Net] " + srcNet->name() + "->" + snkNet->name());
//			throw CilException("GRM: no required path here ... [Net] " + srcNet->name() + "->" + snkNet->name());

		//in 80k, E2BEG4->TS0 like net does have more than one path, so change exception to warning
 		else
		{
			if (pathAmount > 1)
				FDU_LOG(WARN) << Warning("GRM: more than 1 path here ... [Net] " + srcNet->name() + "->" + snkNet->name());
			//throw CilException("GRM: more than 1 path here ... [Net] " + srcNet->name() + "->" + snkNet->name());
		}
 		return path;
	}
	...
	void Site::getPathPips(vecBits& bits, const routeInfo& route){
		...
		if(!path.isEmpty()){ // used for dummy path such like "F5COUPLING3 -> S0_X", so add "if" branch
			string elemName = (_refSiteArch->instances().find(path._bCellName))->down_module()->name();
