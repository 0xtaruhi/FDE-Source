...
namespace FDU { namespace cil_lib {
	...
	public:
		Site(const std::string& name, elemLib* refElemLib = 0) : CilBase(name), _refElemLib(refElemLib)
		{construct();}
	...
	inline void Site::listDfotCfgs(vecCfgs& cfgs){
		_cfgInfo.listDfotCfgs(cfgs);
	}
