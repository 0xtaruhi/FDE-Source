...
namespace FDU { namespace cil_lib {
	...
	void siteSramTile::setRefs(const std::string& refSiteName, const std::string& refTileName, siteLib* refSiteLib){
	...
	void siteInTile::listDfotCfgs(vecCfgs& cfgs){
		//ASSERTD(_siteSram.getSize() > 0, BstrException("siteSram in cluster of tile: no sram(Config Sram) in this site"));
		vecCfgs tCfgs;
		_refSiteLib->getSite(_refSiteName).listDfotCfgs(tCfgs);
		for(cfgElem& cfg: tCfgs){ cfg._siteName = _name; cfgs.push_back(cfg); }
	}
	...
	void siteInTile::listDfotPips(vecBits& bits){
		//ASSERTD(_siteSram.getSize() > 0, BstrException("siteSram in cluster of tile: no sram(PIP Sram) in this site"));
		vecBits tBits;
		_refSiteLib->getSite(_refSiteName).listDfotPips(tBits);
		for(bitTile& bit: tBits){ bit._siteName = _name; bits.push_back(bit); }
	}

	Site* siteInTile::getRefSite() { return &_refSiteLib->getSite(_refSiteName); }

	Site* clstTile::getRefSite(const std::string& siteName){
		siteClstTileIter it = find_if(sites(), [&siteName](const siteInTile* site) { return site->getName() == siteName; });
