...
namespace FDU { namespace cil_lib {
	...
	public:
		sramInSiteSramTile(const string& name, const string& basicCell, sizeSpan localPlace, sizeSpan tileOffset,
			const string& ownerTile, const string& landlordTile, const string& refSiteName, siteLib* refSiteLib = 0)
			: CilBase(name), _basicCell(basicCell), _localPlace(localPlace), _tileOffset(tileOffset),
			_ownerTile(ownerTile), _landlordTile(landlordTile), _refSiteName(refSiteName), _refSiteLib(refSiteLib) {}
	...
	class siteSramTile{
	//public:
	//	using sramsTileType		  = cilContainer<sramInSiteSramTile>::range_type;
	//	using const_sramsTileType = cilContainer<sramInSiteSramTile>::const_range_type;
	//	using sramTileIter		  = cilContainer<sramInSiteSramTile>::iterator;
	//	using const_sramTileIter  = cilContainer<sramInSiteSramTile>::const_iterator;
...
//		tileLib*	_refTileLib;
	...
	public:
		...
		//sramInSiteSramTile* getSram(const std::string& sramName) { return _srams[sramName]; }
		...
		void setRefs(const std::string& refSiteName, const std::string& refTileName, siteLib* refSiteLib);
	};
...
//		tileLib*	  _refTileLib;
	...
	public:
		siteInTile(const std::string& name, const std::string& refSiteName, const std::string& refTileName, 
			sizeSpan position, siteLib* refSiteLib = 0) : CilBase(name), 
			_refSiteName(refSiteName), _refTileName(refTileName), _position(position), _refSiteLib(refSiteLib) { construct(); }
		...
		bool hasSram(const sramInSiteSramTile& sram) {return _siteSram.hasSram(sram);}
		void construct();
	};
...
//		tileLib*	_refTileLib;
	...
	public:
		clstTile(const std::string& name, sizeSpan location, const std::string& refTileName, clstLib* refClstLib = 0)
			: CilBase(name), _location(location), _refTileName(refTileName), _refClstLib(refClstLib) { construct(); }
		...
		siteInTile& getSite(const std::string& siteName) { return *sites().find(siteName); }
...
//		tileLib*	_refTileLib;
	...
	public:
		...
		void setRefTileName(const std::string& refTileName)	{_refTileName = refTileName;}
...
//		void setRefTileLib(tileLib* ref) { _refTileLib = ref; }
		...
		clstTile& getClst(const std::string& clstName) { return *clusters().find(clstName); }
