...
//		tileLib* _refTileLib;
	...
	public:
		Tile(const std::string& name, sizeSpan sramSize, clstLib* refClstLib = 0, trnsLib* refTrnsLib = 0) 
			: CilBase(name), _sramSize(sramSize), _refClstLib(refClstLib), _refTrnsLib(refTrnsLib) 
		{construct();}
	...
	inline std::string Tile::getGRMName(){
		return _trnss.getGRMName();
	}
	...
	inline void Tile::listDfotCfgs(vecCfgs &cfgs){
		_clsts.listDfotCfgs(cfgs);
	}

	inline void Tile::listDfotPips(vecBits& bits){
		_trnss.listDfotPips(bits);
	}

	inline Site* Tile::getRefSite(const std::string& siteName){
		Site* site = _clsts.getRefSite(siteName);
		if (!site) site = _trnss.getRefSite(siteName);
	...
	inline sramInSiteSramTile* Tile::getRefSram(const bitTile& bit){
		...
		if(!sram) sram = _trnss.getRefSram(bit);
		ASSERT(sram, "tile: no such sram named ... " + bit._siteName + " " + bit._basicCell + " " + bit._sramName);
		return sram;
	}
