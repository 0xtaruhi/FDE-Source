...
namespace FDU { namespace cil_lib {
	...
	std::string trnsTile::getGRMName(){
		ASSERTD(_sites.size() <= 1, "transmission in tile: there are more than 1 grm site here");
	...
	Site* trnsTile::getRefSite(const std::string& siteName){
		siteTrnsTileIter it = find_if(sites(), [&siteName](const siteInTile* site) { return site->getName() == siteName; });
