...
//		tileLib*	_refTileLib;
	...
	public:
		trnsTile(const std::string& name, sizeSpan location, const std::string& refTileName, trnsLib* refTrnsLib = 0) 
			: CilBase(name), _location(location), _refTileName(refTileName), _refTrnsLib(refTrnsLib) { construct(); }
...
//		void setRefTileLib(tileLib* ref) { _refTileLib = ref; }
		...
		trnsTile& getTrns(const std::string& trnsName) { return *trnss().find(trnsName); }
		std::string getGRMName();
