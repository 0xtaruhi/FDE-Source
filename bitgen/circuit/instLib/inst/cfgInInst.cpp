...
namespace BitGen { namespace circuit {
	...
	void cfgInst::constructFromXML(){
		...
		_used = (_name == "_INST_PROP" || 
				 _function._funcName == "#OFF" || 
				 _function._funcName.empty()) ? false : true;
	}

	void contCfgsInst::addCfgXML(const std::string &name, const std::string &funcName){
		cfgInst* cfg = new cfgInst(name, funcName);
		if(cfg->isUsed()) _cfgs.add(cfg);
		else			  delete cfg;
	}
