...
namespace BitGen {
	namespace circuit {
		struct funcInCfgInst{
			...
			funcInCfgInst(const std::string& funcName = "", const std::string& workMode = "")
		...
		inline void funcInCfgInst::toFunction(cfgFunc& func){
			func._name = _funcName; func._quomodo = _workMode; func._manner = "";
		}
		...
		public:
			...
			cfgInst& getCfg(const std::string& cfgName) { return *configs().find(cfgName); }
		};
	}
}
