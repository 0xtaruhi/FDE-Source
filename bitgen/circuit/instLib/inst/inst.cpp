...
namespace BitGen { namespace circuit {
	...
	void Inst::adaptCfgTo(vecCfgs& cfgs, const cfgInst& cfg){
		cfgElem tCfg;
		tCfg._tileName = _position._tile; tCfg._siteName = _position._site;
		tCfg._cfgElemName = cfg.getName(); cfg.getFunction().toFunction(tCfg._cfgElemFunc);
		cfgs.push_back(tCfg);
	}
...
// for XML
	...
	void Inst::constructFromXML(){
		...
		for(const ArchInstance* archInst: tile->down_module()->instances()){
			if(archInst->down_module()->type() == _instType && archInst->z_pos() == pos.z){
				site = archInst->name();
				break;
			}
		}
		ASSERT(!site.empty(), "Inst: can't find reference instance in tile ... " + _name);
		...
		for (const PropertyBase* prop: inst->properties()){
			if(prop->name() != "h_set" && 
			   ...
			   prop->name() != "rloc" && 
			   prop->name() != "set_type")
			   _cfgs.addCfgXML(prop->name(), 
					prop->string_value(inst));
		}
