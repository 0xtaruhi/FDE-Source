...
// for XDL
	...
	bool parsePinPip::operator()(const cktBase::xdlMatch& what) const{
		...
		else if (ppName == "cfg"){} // ignored
		else if (ppName == "" &&  note == "#"){} // ignored
		else
...
// 			pinNet* t = new pinNet(pin.name(), pin.owner().name(), _refInstLib);
// 			pin.is_source() ? addOPin(t) : addIPin(t);
// 		}
		...
		//       refer to STA's annotation(in HandleInterconnect function used "*зЂвт*")
		//       on my laptop
//		_pips.adjustBiDirPips();
//		_pips.adjustBRAMPips();
	}
