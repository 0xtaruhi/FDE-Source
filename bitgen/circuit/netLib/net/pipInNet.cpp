...
namespace BitGen { namespace circuit {
	...
	void pipNet::constructFromXDL(const std::string& info){
		...
		boost::smatch match;
		ASSERT(boost::regex_match(info, match, pipRegex), "pip: invalid pip info in xdl ... " + info);
		_tile	= match[1].str();
		_wireL	= match[2].str();
		_dir	= match[3].str();
		_wireR  = match[4].str();
		...
		if (_wireL.substr(0, 4) == "TBUF" && _wireR.substr(0, 4) == "TBUF" && _dir == "=-")
			_dir = "->";
	}
	...
	void pipNet::adjustBRAMTile(int offset){
		if(offset){
			...
			// used for fdp3000k-cb228
				...
				_tile = _tile.substr(0, 1) + "EMPTY" + _tile.substr(5) + "C0"; // "C0" for dummy
				int row, col;
	...
	void pipNet::adjustBRAMPip(){
		if(isBRAMPip()){
			...
			int wireOff = 0;
			if(wireLOff == 0 && wireROff != 0)		 wireOff = wireROff;
			else if (wireLOff != 0 && wireROff == 0) wireOff = wireLOff;
			else if (wireLOff != 0 && wireROff != 0){
				ASSERTD(
					wireLOff == wireROff, 
					"circuit: invalid BRAM pip ... " + _tile + ": " + _wireL + "->" + _wireR
				);
				wireOff = wireLOff;
			}
			else wireOff = 0;
	...
	return 
		find_if(pips(), [](const pipNet* pip) { return pip->isBiDirPip(); }) != _pips.end(); 
	}

	bool contPipsNet::hasBRAMPip() const {
	return 
		find_if(pips(), [](const pipNet* pip) { return pip->isBRAMPip(); }) != _pips.end(); 
	}

	void contPipsNet::adjustBRAMPips(){
		if(hasBRAMPip())
			for (pipNet* pip : pips()) pip->adjustBRAMPip();
	}

	void pipNet::saveTo(vecPips& vPips){
		pipInfo tPip;
		tPip._tileName = _tile; tPip._siteName = ""; tPip._srcNet = _wireL; tPip._snkNet = _wireR;
		vPips.push_back(tPip);
	}

	void contPipsNet::listPips(vecPips& vPips){
		for (pipNet* pip : pips()) pip->saveTo(vPips);
	}
	...
	static auto matchRoute(const string& cktTileName, const string& wireName) {
		return [&](const routeInfo& rt) { return rt._siteName == cktTileName && rt._srcNet == wireName; };
	}

	// need to test
	string contPipsNet::getDriveAbove(pipNet& curPip, 
									  vector<routeInfo>& drivers)
	{
		string beDrivedNet = "";
		// judge driver upward
		pipNetIter pipIt = boost::find(pips(), &curPip);
		for (pipIt = pipIt == pips().begin() ? pipIt : pipIt - 1; pipIt >= pips().begin(); --pipIt) {
			auto driverIt = std::find_if(drivers.begin(), drivers.end(), matchRoute(pipIt->getTile(), pipIt->getWireR()));
		...
		if(beDrivedNet.empty()){
			...
			FDU_LOG(WARN) << Warning("circuit: pip \"" + curPip.getWireL() + curPip.getDir() + curPip.getWireR() + "\" can not be judged upward, try to judge downward acccrding to wireR");
			for (pipNetIter pipIt = boost::find(pips(), &curPip) + 1; pipIt < pips().end(); ++pipIt) {
				auto driverIt = std::find_if(drivers.begin(), drivers.end(), matchRoute(pipIt->getTile(), pipIt->getWireR()));
				if(driverIt != drivers.end()) {
					beDrivedNet = driverIt->_snkNet;
					break;
		...
		if (beDrivedNet.empty()) {
			...
			FDU_LOG(WARN) << Warning("circuit: pip \"" + curPip.getWireL() + curPip.getDir() + curPip.getWireR() + "\" can not be judged upward, try to judge downward acccrding to wireL");
			for (pipNetIter pipIt = boost::find(pips(), &curPip) + 1; pipIt < pips().end(); ++pipIt) {
				if (pipIt->getDir() == "=-") {
					auto driverIt = std::find_if(drivers.begin(), drivers.end(), matchRoute(pipIt->getTile(), pipIt->getWireL()));
					if(driverIt != drivers.end()) {
						beDrivedNet = driverIt->_snkNet;
						break;
					}
		...
		if(pip.isBiDirPip()){
			...
			if(!beDrivedNet.empty()){
				if(beDrivedNet == wireL) {}
					...
					throw CktException("can't judge path " + cktTileName + ":"	+ wireL + " =- " + wireR + " 's direction");
			}
			else 
				throw CktException("can't judge path " + cktTileName + ":"	+ wireL + " =- " + wireR + " 's direction");
		}
	...
	{
		if(hasBiDirPip())
		for (pipNet* pip : pips()) adjustEachBiDirPip(*pip);
	}
