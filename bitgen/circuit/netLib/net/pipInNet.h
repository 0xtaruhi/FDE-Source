...
namespace BitGen { namespace circuit {
	...
	public:
		...
		pipNet(const std::string& tile, 
			   const std::string& wireL, 
			   const std::string& wireR,
...
//		bool operator == (const pipNet&) const;
	...
	public:
		...
		void listPotentialDrivers(std::string cktTileName,
								  std::string wireName, 
								  std::vector<routeInfo>& drivers);
		string getDriveAbove(pipNet& curPip, 
	...
	inline bool pipNet::isBiDirPip() const
	{ return _dir == "=-"; }
	...
	inline bool
	operator == (const pipNet& lhs, const pipNet& rhs){
		return lhs.getTile()  == rhs.getTile()  &&
			   lhs.getWireL() == rhs.getWireL() &&
			   lhs.getWireR() == rhs.getWireR() &&
