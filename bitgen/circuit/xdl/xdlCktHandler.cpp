...
// function object for parsing
	...
	bool parseObject::operator ()(const cktBase::xdlMatch& what) const {
		...
		static const char* dsignExp = 
			// possibly leading whitespace:
			"\\s*"
			// design information: [1] = cktName, [2] = devName, [3] = pkgName, [5] = speed, [6] = ncdVersion
//			"\"(.*?)\"\\s*(.*?)([pq]+\\d+)(.*?)\\s*([vV]\\d*\\.\\d*)\\s*.*";
		...
		static const char* instExp = 
			// possibly leading whitespace:
			"\\s*"
			// inst information: [1] = instName, [2] = instType, [3] = spot, [4] = instPos, [5] = siteOfInst
			"\"(.*?)\"\\s*\"(.*?)\"\\s*,\\s*([a-zA-Z]+)\\s*([a-zA-Z0-9]+)\\s*(.*?)\\s*,\\s*\\n*"
...
//			"\"(.*?)\"\\s*.*?\\s*,\\s*.*?\\s*\\n*"
		...
		if(objName == "design"){
			ASSERT(boost::regex_match(objInfo, *match, dsignRegex), "design: invalid design info in xdl");
			_curCkt->getDesign()->constructFromXDL(match);
		}
		else if (objName == "inst"){
			ASSERT(boost::regex_match(objInfo, *match, instRegex), "inst: invalid inst info in xdl");
			_curCkt->getInsts()->addInst(new Inst(match));
		}
		else if (objName == "net"){
			ASSERT(boost::regex_match(objInfo, *match, netRegex), "net: invalid net info in xdl");
