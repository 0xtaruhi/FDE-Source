...
//			ASSERT(it != end(), FDU::Exception("container: no such member ..." + name));
	...
	template<typename PT> T& add(PT t) { T& rt = *t; push_back(t); return rt; }		// PT is T* or std::auto_ptr<T>
...
*/
template<typename T> struct cilContainer : COS::PtrContainer< T, std::vector<T*>, true> {};
template<typename T> struct cktContainer : COS::PtrContainer< T, std::vector<T*>, true> {};
template<typename T> struct bstrContainer : COS::PtrContainer< T, std::vector<T*>, true> {};
#endif