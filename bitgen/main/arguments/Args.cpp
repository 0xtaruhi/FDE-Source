...
void Args::tryParse(int argc, char* argv[]){
	try{
		...
		visible.add_options()
			("arch,a",		po::value<string>(), "arch    file path (default at 1st)")
			("cil,c",       po::value<string>(), "cil     file path (default at 2nd)")
			("netlist,n",   po::value<string>(), "netlist file path (default at 3rd)")
			("bitstream,b", po::value<string>(), "output  file path (default at 4th)")
			("partialbitstream,p", po::value<int>(), "partial bitstream tilecol")
			("frm,f",							 "export  frm format bitstream")
			("sopc,s",							 "use for sopc test : only valid for FDP80K")
			("help,h",							 "show help messages");

		po::options_description hidden("Hidden options");
		hidden.add_options()
			("lib,l",       po::value<string>(), "library directory")
			("work,w",      po::value<string>(), "working directory")
			("log,o",                            "output log file or not")
...
void Args::dispHelp() const {
	FDU_LOG(INFO) << "\nUsage: ";
	FDU_LOG(INFO) << "  " << "bitgen [--arch|-a] <archFile.xml|XML> [--cil|-c] <cilFile.xml|XML> [--netlist|-n] <netlist.xml|XML> [--bitstream|-b] <bitFile> [--frm|-f] [--sopc|-s]";
	exit(EXIT_FAILURE);
}

void Args::check(){
//	ASSERT(!_device.empty(),	exception("[CMD] Args: invalid chip name"));
...
// 		cout << Warning("Args: bitstream file name has postfix") << endl;

	if(!_lib_dir.empty() && !_work_dir.empty()){
		if(_logSwitch){
			_log_dir   = _work_dir + _LOGFILE_PATH + _netlist.substr(0, _netlist.find('.'));
