...
int	getWireType(const string& wireName)
{
	...
	if(wireType.compare("BRAM_RADDR") == 0)return 0;

	wireType = wireName.substr(0,9);
	if(wireType.compare("BRAM_RDIN") == 0)return 1;

	wireType = wireName.substr(0,10);
	if(wireType.compare("BRAM_RDOUT") == 0)return 2;

	wireType = wireName.substr(0,2);
	if(wireType.compare("H6") == 0)return 3;

	wireType = wireName.substr(0,2);
	if(wireType.compare("V6") == 0)return 4;

	wireType = wireName.substr(0,6);
	if(wireType.compare("TOP_H6") == 0) return 5;

	wireType = wireName.substr(0,6);
	if(wireType.compare("TOP_V6") == 0) return 6;

	wireType = wireName.substr(0,6);
	if(wireType.compare("BOT_H6") == 0) return 7;

	wireType = wireName.substr(0,6);
	if(wireType.compare("BOT_V6") == 0) return 8;

	wireType = wireName.substr(0,7);
	if(wireType.compare("LEFT_H6") == 0) return 9;

	wireType = wireName.substr(0,7);
	if(wireType.compare("LEFT_V6") == 0) return 10;

	wireType = wireName.substr(0,8);
	if(wireType.compare("RIGHT_H6") == 0) return 11;

	wireType = wireName.substr(0,8);
	if(wireType.compare("RIGHT_V6") == 0) return 12;

	wireType = wireName.substr(0,5);
	if(wireType.compare("UL_H6") == 0) return 13;

	wireType = wireName.substr(0,5);
	if(wireType.compare("UL_V6") == 0) return 14;

	wireType = wireName.substr(0,5);
	if(wireType.compare("UR_H6") == 0) return 15;

	wireType = wireName.substr(0,5);
	if(wireType.compare("UR_V6") == 0) return 16;

	wireType = wireName.substr(0,5);
	if(wireType.compare("LL_H6") == 0) return 17;

	wireType = wireName.substr(0,5);
	if(wireType.compare("LL_V6") == 0) return 18;

	wireType = wireName.substr(0,5);
	if(wireType.compare("LR_H6") == 0) return 19;

	wireType = wireName.substr(0,5);
	if(wireType.compare("LR_V6") == 0) return 20;

	wireType = wireName.substr(0,4);
	if(wireType.compare("TBUF") == 0) return 21;

	wireType = wireName.substr(0,9);
	if(wireType.compare("LEFT_TBUF") == 0) return 22;

	wireType = wireName.substr(0,10);
	if(wireType.compare("RIGHT_TBUF") == 0) return 23;
...
{
	if(wireDir.compare("W") == 0) return std::string("E");
	if(wireDir.compare("E") == 0) return std::string("W");
	if(wireDir.compare("S") == 0) return std::string("N");
	if(wireDir.compare("N") == 0) return std::string("S");
	if(wireDir.compare("D") == 0) return std::string("A");
	if(wireDir.compare("A") == 0) return std::string("D");
	if(wireDir.compare("C") == 0) return std::string("B");
	if(wireDir.compare("B") == 0) return std::string("C");
	if(wireDir.compare("M") == 0) return std::string("M");
	throw CktException("Net: unknown direction: " + wireDir);
}

int getBRAMRow(int row)
{
	if(row % 4 == 0) return row;
	else return row + 4 - (row % 4);
}

string getBRAMSeg(int row)
{
	if(row % 4 == 0) return std::string("D");
	else if(row % 4 == 1) return std::string("A");
	else if(row % 4 == 2) return std::string("B");
...
void addDrivers(const string& cktTileName, 
				const string& wireName, 
				int wireType, 
				vector<routeInfo>& drivers)
{
	...
	case 0:
		...
		if(OppoDir.compare("S") == 0)
			...
			throw CktException("Net: unknown direction of BRAM_RADDR " + tileName + ":" + wireName);
	...
	case 1:
		...
		if(OppoDir.compare("S") == 0)
			...
			throw CktException("unknown direction of BRAM_RDIN " + tileName + ":" + wireName);
		osstrBrkNym << "BRAMR" << row << "C" << column;
		BrkNym = osstrBrkNym.str();
		break;
	case 2:
		...
		if(OppoDir.compare("S") == 0)
			...
			throw CktException("unknown direction of BRAM_RDOUT " + tileName + ":" + wireName);
		osstrBrkNym << "BRAMR" << row << "C" << column;
		BrkNym = osstrBrkNym.str();
		break;
	case 3:
		...
		if(OppoDir.compare("E") == 0)
		{
			...
			if(column > 0) osstrBrkNym<<"R"<<row<<"C"<<column;
			else if(column == 0){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_"+DriveName;}
			else if(column== -1){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_H6A"+WireNum;}
			else if(column== -2)
			{
				...
				//
				osstrBrkNym.str("");
				osstrBrkNym<<"LR"<<row;DriveName="LEFT_H6B"+WireNum;
			}
			else if(column== -3)
			{
				osstrBrkNym<<"BRAMR"<<getBRAMRow(row)<<"C0";
				...
				//
				osstrBrkNym.str("");
				osstrBrkNym<<"LR"<<row;DriveName="LEFT_H6M"+WireNum;
			}
			else if(column== -4){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_H6C"+WireNum;}
			else if(column== -5){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_H6D"+WireNum;}
			else{ osstrBrkNym.clear();DriveName.clear();}
		}
...
//			else if(column ==31)
				...
				{ osstrBrkNym<<"RR"<<row; DriveName="RIGHT_"+DriveName; }
//			else if(column ==32)
			else if(column == INNER_SCALE._columnSpan + 2)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_H6D"+WireNum;}
//			else if(column ==33)
			else if(column == INNER_SCALE._columnSpan + 3)
			{
				...
				//
				osstrBrkNym.str("");
				osstrBrkNym<<"RR"<<row;DriveName="RIGHT_H6C"+WireNum;
			}
//			else if(column ==34)
			else if(column == INNER_SCALE._columnSpan + 4)
			{
				...
				//
				osstrBrkNym.str("");
				osstrBrkNym<<"RR"<<row;DriveName="RIGHT_H6M"+WireNum;
			}
//			else if(column ==35)
			else if(column == INNER_SCALE._columnSpan + 5)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_H6B"+WireNum;}
//			else if(column ==36)
			else if(column == INNER_SCALE._columnSpan + 6)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_H6A"+WireNum;}
			else
				{ osstrBrkNym.clear();DriveName.clear();}
		}
		else
			throw CktException("unknown direction of H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 4:
		WireDir = wireName.substr(2,1);
		...
		if(OppoDir.compare("S") == 0)
		{
			...
			if(row > 0) osstrBrkNym<<"R"<<row<<"C"<<column;
			else if(row == 0){ osstrBrkNym<<"TC"<<column;DriveName="TOP_"+DriveName;}
			else if(row== -1){ osstrBrkNym<<"TC"<<column;DriveName="TOP_V6D"+WireNum;}
			else if(row== -2){ osstrBrkNym<<"TC"<<column;DriveName="TOP_V6C"+WireNum;}
			else if(row== -3){ osstrBrkNym<<"TC"<<column;DriveName="TOP_V6M"+WireNum;}
			else if(row== -4){ osstrBrkNym<<"TC"<<column;DriveName="TOP_V6B"+WireNum;}
			else if(row== -5){ osstrBrkNym<<"TC"<<column;DriveName="TOP_V6A"+WireNum;}
			else{ osstrBrkNym.clear();DriveName.clear();}
		}
		else if(OppoDir.compare("N") == 0)
		{
...
//			else if(row ==21)
				...
				{ osstrBrkNym<<"BC"<<column;DriveName="BOT_"+DriveName;}
//			else if(row ==22)
			else if(row == INNER_SCALE._rowSpan + 2)
				{ osstrBrkNym<<"BC"<<column;DriveName="BOT_V6A"+WireNum;}
//			else if(row ==23)
			else if(row == INNER_SCALE._rowSpan + 3)
				{ osstrBrkNym<<"BC"<<column;DriveName="BOT_V6B"+WireNum;}
//			else if(row ==24)
			else if(row == INNER_SCALE._rowSpan + 4)
				{ osstrBrkNym<<"BC"<<column;DriveName="BOT_V6M"+WireNum;}
//			else if(row ==25)
			else if(row == INNER_SCALE._rowSpan + 5)
				{ osstrBrkNym<<"BC"<<column;DriveName="BOT_V6C"+WireNum;}
//			else if(row ==26)
			else if(row == INNER_SCALE._rowSpan + 6)
				{ osstrBrkNym<<"BC"<<column;DriveName="BOT_V6D"+WireNum;}
			else
				{ osstrBrkNym.clear();DriveName.clear();}
		}
		else
			throw CktException("unknown direction of V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 5:
		...
		{
			column = BrkRC._columnSpan - 6;
//			if(column > 0 && column!=10 && column!=15){ osstrBrkNym<<"TC"<<column;DriveName = "TOP_"+DriveName;}
			if((args._device == CHIPTYPE::FDP1000K && column > 0 && column != 10 && column != 15) 
			|| (args._device == CHIPTYPE::FDP3000K && column > 0 && column != 19 && column != 24))
			{
				osstrBrkNym<<"TC"<<column;
				DriveName = "TOP_"+DriveName;
			}
			else if(column == 0){ osstrBrkNym<<"TL";DriveName="UL_"+DriveName;}
			else if(column== -1){ osstrBrkNym<<"TL";DriveName="UL_H6A"+WireNum;}
			else if(column== -2){ osstrBrkNym<<"TL";DriveName="UL_H6B"+WireNum;}
			else if(column== -3){ osstrBrkNym<<"TL";DriveName="UL_H6M"+WireNum;}
			else if(column== -4){ osstrBrkNym<<"TL";DriveName="UL_H6C"+WireNum;}
			else if(column== -5){ osstrBrkNym<<"TL";DriveName="UL_H6D"+WireNum;}
//			else if(column== 10)
				 ...
				 || (args._device == CHIPTYPE::FDP3000K && column == 19))
			{
				...
				osstrBrkNym.str("");
				osstrBrkNym<<"TC"<<column;DriveName = "TOP_H6E"+WireNum;
			}
//			else if(column== 15)
...
//			if(column < INNER_SCALE._columnSpan + 1 && column!=16 && column!=21)
			if((args._device == CHIPTYPE::FDP1000K && column < INNER_SCALE._columnSpan + 1 && column != 16 && column != 21)\
			|| (args._device == CHIPTYPE::FDP3000K && column < INNER_SCALE._columnSpan + 1 && column != 25 && column != 30))
			{ 
				osstrBrkNym<<"TC"<<column;
				DriveName = "TOP_"+DriveName;
			}
//			else if(column ==31)
				...
				{ osstrBrkNym<<"TR";DriveName="UR_"+DriveName;}
//			else if(column ==32)
			else if(column == INNER_SCALE._columnSpan + 2)
				{ osstrBrkNym<<"TR";DriveName="UR_H6D"+WireNum;}
//			else if(column ==33)
			else if(column == INNER_SCALE._columnSpan + 3)
				{ osstrBrkNym<<"TR";DriveName="UR_H6C"+WireNum;}
//			else if(column ==34)
			else if(column == INNER_SCALE._columnSpan + 4)
				{ osstrBrkNym<<"TR";DriveName="UR_H6M"+WireNum;}
//			else if(column ==35)
			else if(column == INNER_SCALE._columnSpan + 5)
				{ osstrBrkNym<<"TR";DriveName="UR_H6B"+WireNum;}
//			else if(column ==36)
			else if(column == INNER_SCALE._columnSpan + 6)
				{ osstrBrkNym<<"TR";DriveName="UR_H6A"+WireNum;}
//			else if(column ==16)
				 ...
				 || (args._device == CHIPTYPE::FDP3000K && column == 25))
			{
				...
				osstrBrkNym.str("");
				osstrBrkNym<<"TC"<<column;DriveName = "TOP_H6W"+WireNum;
			}
//			else if(column ==21)
			...
			throw CktException("unknown direction of TOP_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 6:
		WireDir = wireName.substr(6,1);
...
/*		if(OppoDir.compare("N") == 0)
		...
		if(WireDir.compare("A") == 0) osstrBrkNym<<"R1C"<<column;
		else if(WireDir.compare("B") == 0) osstrBrkNym<<"R2C"<<column;
		else if(WireDir.compare("M") == 0) osstrBrkNym<<"R3C"<<column;
		else if(WireDir.compare("C") == 0) osstrBrkNym<<"R4C"<<column;
		else if(WireDir.compare("D") == 0) osstrBrkNym<<"R5C"<<column;
		else if(WireDir.compare("S") == 0) osstrBrkNym<<"R6C"<<column;
		else
			throw CktException("unknown direction of TOP_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 7:
		WireDir = wireName.substr(6,1);
...
/*		BrkNym = "R21"+tileName.substr(1);*/
		BrkNym = "R" + lexical_cast<string>(INNER_SCALE._rowSpan + 1) + tileName.substr(1);
		BrkRC = lexical_cast<sizeSpan>(BrkNym);
		if(OppoDir.compare("E") == 0)
		{
			column = BrkRC._columnSpan - 6;
//			if(column > 0 && column!=10 && column!=15)
			...
			else if(column == 0){ osstrBrkNym<<"BL";DriveName="LL_"+DriveName;}
			else if(column== -1){ osstrBrkNym<<"BL";DriveName="LL_H6A"+WireNum;}
			else if(column== -2){ osstrBrkNym<<"BL";DriveName="LL_H6B"+WireNum;}
			else if(column== -3){ osstrBrkNym<<"BL";DriveName="LL_H6M"+WireNum;}
			else if(column== -4){ osstrBrkNym<<"BL";DriveName="LL_H6C"+WireNum;}
			else if(column== -5){ osstrBrkNym<<"BL";DriveName="LL_H6D"+WireNum;}
//			else if(column== 10)
			else if((args._device == CHIPTYPE::FDP1000K && column == 10)\
				 || (args._device == CHIPTYPE::FDP3000K && column == 19))
			{
				...
				osstrBrkNym.str("");
				osstrBrkNym<<"BC"<<column;DriveName = "BOT_H6E"+WireNum;
			}
//			else if(column== 15)
			else if((args._device == CHIPTYPE::FDP1000K && column == 15)\
				 || (args._device == CHIPTYPE::FDP3000K && column == 24))
...
//			else if(column ==31)
				...
				{ osstrBrkNym<<"BR";DriveName="LR_"+DriveName;}
//			else if(column ==32)
			else if(column == INNER_SCALE._columnSpan + 2)
				{ osstrBrkNym<<"BR";DriveName="LR_H6D"+WireNum;}
//			else if(column ==33)
			else if(column == INNER_SCALE._columnSpan + 3)
				{ osstrBrkNym<<"BR";DriveName="LR_H6C"+WireNum;}
//			else if(column ==34)
			else if(column == INNER_SCALE._columnSpan + 4)
				{ osstrBrkNym<<"BR";DriveName="LR_H6M"+WireNum;}
//			else if(column ==35)
			else if(column == INNER_SCALE._columnSpan + 5)
				{ osstrBrkNym<<"BR";DriveName="LR_H6B"+WireNum;}
//			else if(column ==36)
			else if(column == INNER_SCALE._columnSpan + 6)
				{ osstrBrkNym<<"BR";DriveName="LR_H6A"+WireNum;}
//			else if(column ==16)
			else if((args._device == CHIPTYPE::FDP1000K && column == 16)\
				 || (args._device == CHIPTYPE::FDP3000K && column == 25))
			{
				...
				osstrBrkNym.str("");
				osstrBrkNym<<"BC"<<column;DriveName = "BOT_H6W"+WireNum;
			}
//			else if(column ==21)
			else if((args._device == CHIPTYPE::FDP1000K && column == 21)\
				 || (args._device == CHIPTYPE::FDP3000K && column == 30))
			...
			throw CktException("unknown direction of BOT_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 8:
		WireDir = wireName.substr(6,1);
...
//			osstrBrkNym<<"R20C"<<column;
			...
			throw CktException("unknown direction of BOT_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 9:
...
//			osstrBrkNym<<"R"<<row<<"C6";
		...
		else if(WireDir.compare("B") == 0)
		{
			...
			//
			osstrBrkNym.str("");
			osstrBrkNym<<"R"<<row<<"C4";DriveName = "H6W"+WireNum;
		}
		else if(WireDir.compare("M") == 0)
		{
			osstrBrkNym<<"BRAMR"<<getBRAMRow(row)<<"C0";
			...
			//
			osstrBrkNym.str("");
			osstrBrkNym<<"R"<<row<<"C3";DriveName = "H6W"+WireNum;
		}
		else if(WireDir.compare("C") == 0) 
			...
			throw CktException("unknown direction of LEFT_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 10:
		WireDir = wireName.substr(7,1);
		...
		{
			row = BrkRC._rowSpan - 6;
			if(row > 0){ osstrBrkNym<<"LR"<<row;DriveName = "LEFT_"+DriveName;}
			else if(row == 0){ osstrBrkNym<<"TL";DriveName="UL_"+DriveName;}
			else if(row == -1){ osstrBrkNym<<"TL";DriveName="UL_V6D"+WireNum;}
			else if(row == -2){ osstrBrkNym<<"TL";DriveName="UL_V6C"+WireNum;}
			else if(row == -3){ osstrBrkNym<<"TL";DriveName="UL_V6M"+WireNum;}
			else if(row == -4){ osstrBrkNym<<"TL";DriveName="UL_V6B"+WireNum;}
			else if(row == -5){ osstrBrkNym<<"TL";DriveName="UL_V6A"+WireNum;}
			else{ osstrBrkNym.clear();DriveName.clear();}
		}
		else if(OppoDir.compare("N") == 0)
		{
...
//			if(row< 21)
			if(row < INNER_SCALE._rowSpan + 1)
				{ osstrBrkNym<<"LR"<<row;DriveName = "LEFT_"+DriveName;}
//			else if(row ==21)
			else if(row == INNER_SCALE._rowSpan + 1)
				{ osstrBrkNym<<"BL";DriveName="LL_"+DriveName;}
//			else if(row ==22)
			else if(row == INNER_SCALE._rowSpan + 2)
				{ osstrBrkNym<<"BL";DriveName="LL_V6A"+WireNum;}
//			else if(row ==23)
			else if(row == INNER_SCALE._rowSpan + 3)
				{ osstrBrkNym<<"BL";DriveName="LL_V6B"+WireNum;}
//			else if(row ==24)
			else if(row == INNER_SCALE._rowSpan + 4)
				{ osstrBrkNym<<"BL";DriveName="LL_V6M"+WireNum;}
//			else if(row ==25)
			else if(row == INNER_SCALE._rowSpan + 5)
				{ osstrBrkNym<<"BL";DriveName="LL_V6C"+WireNum;}
//			else if(row ==26)
			else if(row == INNER_SCALE._rowSpan + 6)
				{ osstrBrkNym<<"BL";DriveName="LL_V6D"+WireNum;}
			else{ osstrBrkNym.clear();DriveName.clear();}
		}
		else
			throw CktException("unknown direction of LEFT_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 11:
...
//		BrkNym = tileName.substr(1)+"C31";
		BrkNym = tileName.substr(1) + "C" + lexical_cast<string>(INNER_SCALE._columnSpan + 1);
		BrkRC = lexical_cast<sizeSpan>(BrkNym);
		row = BrkRC._rowSpan;
//		if(OppoDir.compare("E") == 0)
...
//			osstrBrkNym<<"R"<<row<<"C25";
			...
			throw CktException("unknown direction of RIGHT_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 12:
		WireDir = wireName.substr(8,1);
...
//		BrkNym = tileName.substr(1)+"C31";
		...
		{
			row = BrkRC._rowSpan - 6;
			if(row > 0){ osstrBrkNym<<"RR"<<row;DriveName = "RIGHT_"+DriveName;}
			else if(row == 0){ osstrBrkNym<<"TR";DriveName="UR_"+DriveName;}
			else if(row == -1){ osstrBrkNym<<"TR";DriveName="UR_V6D"+WireNum;}
			else if(row == -2){ osstrBrkNym<<"TR";DriveName="UR_V6C"+WireNum;}
			else if(row == -3){ osstrBrkNym<<"TR";DriveName="UR_V6M"+WireNum;}
			else if(row == -4){ osstrBrkNym<<"TR";DriveName="UR_V6B"+WireNum;}
			else if(row == -5){ osstrBrkNym<<"TR";DriveName="UR_V6A"+WireNum;}
			else{ osstrBrkNym.clear();DriveName.clear();}
		}
		else if(OppoDir.compare("N") == 0)
		{
...
//			else if(row ==22)
				...
				{ osstrBrkNym<<"BR";DriveName="LR_V6A"+WireNum;}
//			else if(row ==23)
			else if(row == INNER_SCALE._rowSpan + 3)
				{ osstrBrkNym<<"BR";DriveName="LR_V6B"+WireNum;}
//			else if(row ==24)
			else if(row == INNER_SCALE._rowSpan + 4)
				{ osstrBrkNym<<"BR";DriveName="LR_V6M"+WireNum;}
//			else if(row ==25)
			else if(row == INNER_SCALE._rowSpan + 5)
				{ osstrBrkNym<<"BR";DriveName="LR_V6C"+WireNum;}
//			else if(row ==26)
			else if(row == INNER_SCALE._rowSpan + 6)
				{ osstrBrkNym<<"BR";DriveName="LR_V6D"+WireNum;}
			else
				{ osstrBrkNym.clear();DriveName.clear();}
		}
		else
			throw CktException("unknown direction of RIGHT_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 13:
		...
		if(WireDir.compare("E") == 0) osstrBrkNym<<"TC6";
		else if(WireDir.compare("A") == 0) osstrBrkNym<<"TC5";
		else if(WireDir.compare("B") == 0) osstrBrkNym<<"TC4";
		else if(WireDir.compare("M") == 0) osstrBrkNym<<"TC3";
		else if(WireDir.compare("C") == 0) osstrBrkNym<<"TC2";
		else if(WireDir.compare("D") == 0) osstrBrkNym<<"TC1";
		else throw CktException("unknown direction of UL_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 14:
		WireDir = wireName.substr(5,1);
		...
		if(WireDir.compare("A") == 0) osstrBrkNym<<"LR1";
		else if(WireDir.compare("B") == 0) osstrBrkNym<<"LR2";
		else if(WireDir.compare("M") == 0) osstrBrkNym<<"LR3";
		else if(WireDir.compare("C") == 0) osstrBrkNym<<"LR4";
		else if(WireDir.compare("D") == 0) osstrBrkNym<<"LR5";
		else if(WireDir.compare("S") == 0) osstrBrkNym<<"LR6";
		else throw CktException("unknown direction of UL_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 15:
		WireDir = wireName.substr(5,1);
...
//			osstrBrkNym<<"TC25";
			...
			throw CktException("unknown direction of UR_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 16:
		WireDir = wireName.substr(5,1);
		...
		if(WireDir.compare("A") == 0) osstrBrkNym<<"RR1";
		else if(WireDir.compare("B") == 0) osstrBrkNym<<"RR2";
		else if(WireDir.compare("M") == 0) osstrBrkNym<<"RR3";
		else if(WireDir.compare("C") == 0) osstrBrkNym<<"RR4";
		else if(WireDir.compare("D") == 0) osstrBrkNym<<"RR5";
		else if(WireDir.compare("S") == 0) osstrBrkNym<<"RR6";
		else throw CktException("unknown direction of UR_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 17:
		WireDir = wireName.substr(5,1);
		...
		if(WireDir.compare("E") == 0) osstrBrkNym<<"BC6";
		else if(WireDir.compare("A") == 0) osstrBrkNym<<"BC5";
		else if(WireDir.compare("B") == 0) osstrBrkNym<<"BC4";
		else if(WireDir.compare("M") == 0) osstrBrkNym<<"BC3";
		else if(WireDir.compare("C") == 0) osstrBrkNym<<"BC2";
		else if(WireDir.compare("D") == 0) osstrBrkNym<<"BC1";
		else throw CktException("unknown direction of LL_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 18:
		WireDir = wireName.substr(5,1);
...
//			osstrBrkNym<<"LR20";
			...
			throw CktException("unknown direction of LL_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 19:
		WireDir = wireName.substr(5,1);
...
//			osstrBrkNym<<"BC25";
			...
			throw CktException("unknown direction of LR_H6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 20:
		WireDir = wireName.substr(5,1);
...
//			osstrBrkNym<<"RR20";
			...
			throw CktException("unknown direction of LR_V6 " + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 21:
		...
		if(WireDir.compare("3") == 0)
		{
			...
			if(column > 0){ osstrBrkNym<<"R"<<row<<"C"<<column;DriveName="TBUF_STUB3";}
			else if(column == 0){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_TBUF1_STUB";}
			else if(column== -1){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_TBUFO2";}
			else if(column== -2){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_TBUFO3";}
			else if(column== -3){ osstrBrkNym<<"LR"<<row;DriveName="LEFT_TBUFO0";}
			else{ osstrBrkNym.clear();DriveName.clear();}
		}
...
//			if(column < 31)
			if(column < INNER_SCALE._columnSpan + 1)
				{ osstrBrkNym<<"R"<<row<<"C"<<column;DriveName="TBUF3";}
//			else if(column== 31)
			else if(column == INNER_SCALE._columnSpan + 1)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_TBUFO1";}
//			else if(column== 32)
			else if(column == INNER_SCALE._columnSpan + 2)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_TBUFO0";}
//			else if(column== 33)
			else if(column == INNER_SCALE._columnSpan + 3)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_TBUFO3";}
//			else if(column== 34)
			else if(column == INNER_SCALE._columnSpan + 4)
				{ osstrBrkNym<<"RR"<<row;DriveName="RIGHT_TBUFO2";}
			else
				{ osstrBrkNym.clear();DriveName.clear();}
		}
		else throw CktException("unknown direction of TBUF" + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 22:
		WireDir = wireName.substr(10,1);
		...
		BrkRC = lexical_cast<sizeSpan>(BrkNym);
		row = BrkRC._rowSpan;
		if(WireDir.compare("0") == 0){ osstrBrkNym<<"R"<<row<<"C1";DriveName="TBUF3";}
		else if(WireDir.compare("3") == 0){ osstrBrkNym<<"R"<<row<<"C2";DriveName="TBUF3";}
		else if(WireDir.compare("2") == 0){ osstrBrkNym<<"R"<<row<<"C3";DriveName="TBUF3";}
		else if(WireDir.compare("_") == 0){ osstrBrkNym<<"R"<<row<<"C4";DriveName="TBUF3";}
		else throw CktException("unknown direction of LEFT_TBUF" + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	case 23:
...
//			{ osstrBrkNym<<"R"<<row<<"C27";DriveName="TBUF_STUB3";}
			{ osstrBrkNym<<"R"<<row<<"C"<<INNER_SCALE._columnSpan-3;DriveName="TBUF_STUB3";}
		else if(WireDir.compare("0") == 0)
//			{ osstrBrkNym<<"R"<<row<<"C28";DriveName="TBUF_STUB3";}
			{ osstrBrkNym<<"R"<<row<<"C"<<INNER_SCALE._columnSpan-2;DriveName="TBUF_STUB3";}
		else if(WireDir.compare("3") == 0)
//			{ osstrBrkNym<<"R"<<row<<"C29";DriveName="TBUF_STUB3";}
			{ osstrBrkNym<<"R"<<row<<"C"<<INNER_SCALE._columnSpan-1;DriveName="TBUF_STUB3";}
		else if(WireDir.compare("2") == 0)
//			{ osstrBrkNym<<"R"<<row<<"C30";DriveName="TBUF_STUB3";}
			{ osstrBrkNym<<"R"<<row<<"C"<<INNER_SCALE._columnSpan-0;DriveName="TBUF_STUB3";}
		else 
			throw CktException("unknown direction of RIGHT_TBUF" + tileName + ":" + wireName);
		BrkNym = osstrBrkNym.str();
		break;
	default:
