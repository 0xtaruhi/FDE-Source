...
namespace ARCH {
	...
	public:
		ArchPort(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from);
	...
	public:
		...
		typedef PtrList<Instance>::typed<ArchInstance>::const_range    const_instances_type;
		typedef PtrList<Instance>::typed<ArchInstance>::iterator       inst_iter;
		typedef PtrList<Instance>::typed<ArchInstance>::const_iterator const_inst_iter;
	...
	public:
		...
		void add_pad(const string& n, const Point& p) { _pads.push_back(PadInfo(n, p)); }
...
//		void	set_carry_num(int carry_num)		  {_carry_num = carry_num;}
		...
		static FPGADesign* loadArchLib(const string& file, CosFactory* factory = new ArchFactory());
...
//		int				   _carry_num;
	...
	inline ArchCell* ArchInstance::down_module() const { return static_cast<ArchCell*>(Instance::down_module()); }
	inline ArchCell* ArchInstance::module()      const { return static_cast<ArchCell*>(Instance::module()); }
