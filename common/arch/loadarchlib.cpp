...
namespace COS { namespace XML {
	...
	Design* ArchLibHandler::load_design(xml_node* node, Design* design) {
		FPGADesign* arch = dynamic_cast<FPGADesign*>(NetlistHandler::load_design(node, design));
	...
	/* new devide_info format
	   <device_info scale="35,55" wire_timming="43.8816e-3,34.8561e-3" slice_per_tile="2" carry_chain="0;1" bram_col="0,54"/>
	*/
	void ArchLibHandler::load_info(xml_node* node, FPGADesign* arch) {
		...
		iss >> R >> d >> C;
		if (iss) arch->set_wire_timing(R, C);
	...
	Module* ArchLibHandler::load_module(xml_node* node, Library* lib) {
		ArchCell* cell = dynamic_cast<ArchCell*>(NetlistHandler::load_module(node, lib));
	...
	Port* ArchLibHandler::load_port(xml_node* node, Module* mod) {
		ArchPort* port = dynamic_cast<ArchPort*>(NetlistHandler::load_port(node, mod));
	...
	Instance* ArchLibHandler::load_instance(xml_node* node, Module* mod) {
		ArchInstance* inst = dynamic_cast<ArchInstance*>(NetlistHandler::load_instance(node, mod));
...
namespace ARCH {
	...
	NetlistHandler* ArchFactory::make_xml_read_handler() { return new ArchLibHandler; }

	/// load arch library from a XML file
	FPGADesign* FPGADesign::loadArchLib(const string& file_name, CosFactory* factory) {
		CosFactory::pointer old_factory = CosFactory::set_factory(factory);
		std::unique_ptr<NetlistHandler> alh(CosFactory::instance().make_xml_read_handler());
