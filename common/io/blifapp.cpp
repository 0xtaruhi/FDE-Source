...
#define USAGE "Usage: blif2xml -i blif_file -l library -x xml_file  [-v verilog_file] [-r report_file] [-h]"
...
int BLIFApp::process_args(int argc, char* argv[]){
	try
	{
		...
		desc.add_options()
			...
			("input,i", po::value<string>(), "input file to be parsed")
			("xml,x", po::value<string>(), "export xml")
			("library,l", po::value<string>(), "cell library")
			("report,r", po::value<string>(), "report file")
			("verilog,v", po::value<string>(), "verilog file")
			("map,m", "parse for mapper")
			("pack,p", "parse for packer")
			;
		po::variables_map vm;
		po::store(po::parse_command_line(argc, argv, desc), vm);
		po::notify(vm);

		if (vm.count("help")) print_usage(USAGE);
		if (vm.count("input")) {
