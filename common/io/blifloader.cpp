...
namespace {
	...
	public:
		blif_writer_impl(const Design& design, ostream& os) : _design(design), _os(os) {}
		void write_blif();
	};
	...
	void blif_writer_impl::write_blif() {
		...
		//cell instances
		for_each(top_cell.instances(), bind(&blif_writer_impl::write_instance, this, _1));
		_os << ".end" << endl;
	}
	...
	void blif_writer_impl::write_instance(const Instance& inst) {
		...
		if (cell_type=="LUT") {
			...
			foreach (const Pin& pin, inst.pins())
				...
				_os << innet << ' ' << outnet << ' ' << inst.property_value<string>("type","") << ' ' << clk_net << ' ';
	...
	class blif_loader_impl {
		...
		string inst_name(const string& prefix) { return prefix + '_' + lexical_cast<string>(_inst_count++); }
	...
	blif_loader_impl::blif_loader_impl(Design& design, istream& is) :
		_design(design), _is(is), _inst_count(0), _net_count(0), _put_back(false), _vcc(0), _gnd(0)
	{ prepare_lib_cells(); }
	...
	void blif_loader_impl::load_blif(const string& lib) {
		while (_is) {
			...
			if (keyword == ".model")		create_model(lib);
			else if (keyword == ".inputs")	create_io(INPUT);
			else if (keyword == ".outputs")	create_io(OUTPUT);
			else if (keyword == ".clock")	;
			else if (keyword == ".names")	create_logic();
			else if (keyword == ".latch")	create_latch();
			else if (keyword == ".end")		break;
			else throw netlist_error("blifloader: unknown token: " + keyword);
		} 
	}

	void blif_loader_impl::get_tokens() {
		if (_put_back) { _put_back = false; return; }
		...
		while (_is) {
			...
			if (pos != string::npos) line.erase(pos);		// trim comment
			trim(line);
	...
	void blif_loader_impl::create_latch() {
		...
		if (rst) rst_n = true;
		else rst = _top_cell->nets().find("rst");
		if (rst) rst->set_type(RESET);
		...
		foreach (Pin& pin, instance.pins()) {
			switch(pin.type()) {
			case NORMAL:
				if (pin.is_sink()) pin.hookup(n_in);
				else pin.hookup(n_out);
				break;
			case CLOCK:
				if (n_clk.name() == "NIL")
					;
	...
	void blif_loader_impl::create_logic() {
		...
		//Assert(size >= 0 && size <= max_lut, netlist_error("blifloader: logic size error\n"));
		
		vector<string> net_names = tokens;

		//consider that .name gnd. there is no following line		
		//Assert(get_truth_tokens(), netlist_error("blifloader: truthtable error 1\n"));
		get_truth_tokens();

		if (size == 0) {
			Assert(tokens.size() == 1, netlist_error("blifloader: truthtable error 2\n"));
			switch (tokens[0][0]) {
				case '0': {
					Net* gnd1 = _top_cell->nets().find(net_names[1]);
					Net* gnd2 = &get_gnd_net();
					if (!gnd1) gnd2->rename(net_names[1]);
					else if (gnd1 != gnd2) gnd1->merge(*gnd2);
					return;
				  }
				case '1': {
					Net* vcc1 = _top_cell->nets().find(net_names[1]);
					Net* vcc2 = &get_vcc_net();
					if (!vcc1) vcc2->rename(net_names[1]);
					else if (vcc1 != vcc2) vcc1->merge(*vcc2);
					return;
				}
				default : {
					if (net_names[1] == "gnd") {
		...
		if (size == 1) {
			Assert(tokens.size() == 2 || tokens[1] == "1", netlist_error("blifloader: truthtable error 3"));
			switch (tokens[0][0]) {
				case '0': cell = INV; break;
				case '1': cell = BUF; break;
				default : throw netlist_error("blifloader: truthtable error 4");
			}
		}
		...
		int i = 0;
		foreach (Pin& pin, instance.pins()) {
			if (pin.is_source()) pin.hookup(*nets[size]);
			else pin.hookup(*nets[i++]);
		}
		...
		{
			if (size > max_lut)
				instance.set_property("output_cover", calc_init(ParseFor::PACKER, size), 1);
			else
	...
	Net& blif_loader_impl::get_net(const string& name) {
		if (Net* net = _top_cell->nets().find(name)) return *net;
	...
	string blif_loader_impl::calc_init(ParseFor parse_for, size_t lut_size) {
		...
		if (parse_for == ParseFor::MAPPER)
		{
			...
			{
				Assert(tokens.size() == 2 || tokens[1] == "1", netlist_error("blifloader: truthtable error 5"));
		...
		else if (parse_for == ParseFor::PACKER)
		{
			...
			while (it != covers.end()) {
				...
				size_t p = s.find('-');
				if ( p != string::npos ) {
					s[p] = '0'; covers.push_back(s);
					s[p] = '1'; covers.push_back(s);
					it = covers.erase(it);
				}
				else {
