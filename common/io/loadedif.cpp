...
#define ASSERTLN(a, s) ASSERT(a, (format("Parsing Error, line %d: %s") % _line % (s)))
...
namespace {		// rename map
	...
	class RnKey {
		...
		string _name;
	public:
		RnKey(const Object* owner, const string& type, const string& name) : _owner(owner), _name(type + name) {}
		bool operator < (const RnKey& rhs) const {
			if (_owner < rhs._owner) return true;
	...
	Rename* get_rename(const Object* owner, const string& type, const string& name) {
...
namespace COS { namespace EDIF {

Rename::Rename(const string& nname, const string& oname) : new_name(nname), is_bus(false), msb(0), lsb(0) {
...
Rename::Rename(const string& name, int size) : new_name(name), old_name(name), is_bus(true), msb(size - 1), lsb(0) {}
...
Parser::Parser(Design* design, istream& is) :
	_source(is), _line(1), cur_design(design), cur_lib(0), cur_module(0), cur_net(0),
...
Token Parser::get_token() {
	...
	default:
		...
		if(buf[0] == '-' || isdigit(buf[0])) {
			int64_t val;
			try { val = boost::lexical_cast<int64_t>(buf); }
			catch (...) { ASSERTLN(0, "integer error"); }
...
Token Parser::parse_ignore() {
	...
	do {
		tok = get_token();
		if(tok.type() == T_LPAR) count++;
		else if(tok.type() == T_RPAR) count--;
...
Token Parser::parse_continue() {
	...
	for (;;) {
		Token last_tok = tok;
		tok = parse();
		if (tok.type() == T_EOF) return tok;
		if (tok.type() == T_RPAR) return last_tok;
	} 
}
...
string Parser::get_name(const Token& tok, const Object* owner, const string& type) const {
	string name;
	if (tok.type() == T_NAME)
		name = tok.value<string>();
	else if (tok.type() == T_RENAME) {
		RnPtr rnp = tok.value<RnPtr>();
				ASSERTLN(!rnp->is_bus || rnp->bus_size()==1, type + ": bus name not allowed");
		name = rnp->name();
...
string Parser::get_refname(const Token& tok, const Object* owner, const string& type) const {
	string name;
	ASSERTLN(tok.type() == T_NAME, type + " name required");
	name = tok.value<string>();
	Rename* rn = get_rename(owner, type, name);
	if (!rn) return name;
	if (!rn->is_bus) return rn->name();
...
Token Parser::parse_library() {
	...
	if (name == _cell_lib_old) name = _cell_lib_new;
...
Token Parser::parse_array() {
	Token tname = parse();
	ASSERTLN(tname.type() == T_NAME || tname.type() == T_RENAME, "array: name required");
	Token tsize = parse();
	ASSERTLN(tsize.type() == T_INT, "array: size required");
	int size = tsize.value<int64_t>();
	RnPtr rnp = tname.type() == T_RENAME ? tname.value<RnPtr>() :
...
template<typename T> inline void set_property(Object* obj, const string& name, const Token& tok) {
	if constexpr (is_same_v<T, string>)
		obj->set_property(create_property<T>(obj->class_id(), name), tok.value<T>());
	else if (obj->class_id() == COS::INSTANCE && name.compare(0, 4, "INIT") == 0)
		// convert the type of INIT* property to string
		obj->set_property(create_property<string>(obj->class_id(), name), to_string(tok.value<T>()));
	else
		obj->set_property(create_property<T>(obj->class_id(), name), tok.value<T>());
}
...
Token Parser::parse_property() {
	...
	tok = parse();
	switch (tok.type()) {
	case T_INT: 	set_property<int64_t>(cur_obj(), name, tok);	break;
	case T_DOUBLE:	set_property<double>(cur_obj(), name, tok);	break;
	case T_QSTR:	set_property<string>(cur_obj(), name, tok);	break;
	default: break;
	}
	return parse_ignore();
}
