...
namespace COS { namespace EDIF {
	...
	enum token_type { T_EOF, T_LPAR, T_RPAR, T_QSTR, T_INT, T_DOUBLE, T_NAME, T_RENAME,
		T_ERROR, T_NULL, T_CREF, T_LREF, T_PREF, T_IREF, T_DIR, T_ARRAY, T_MEMBER
	};
	...
	private:
		...
		void ungetc(char ch);
		Token get_token();
		std::string get_name(const Token& tok, const COS::Object* owner, const std::string& type) const;
		std::string get_refname(const Token& tok, const COS::Object* owner, const std::string& type) const;
