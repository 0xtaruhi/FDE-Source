...
namespace {
	...
	void consistency_check(const Net* net, ostream& logger) {
		...
		if (num_sources > 1) logger << *net << " : multiple sources." << endl;
		if (!num_sources) logger << *net << " : no sources." << endl;
		if (net->num_pins() <= num_sources) logger << *net << " : no sinks." << endl;
	}

	void consistency_check(const Port* port, ostream& logger) {
		if (!port->mpin()->is_mpin())  logger << port << " : mpin error." << endl;
		if (port->mpin()->port() != port) logger << port << " : mpin port error." << endl;
//		if (port->mpin()->name() != port->name()) logger << port << " : mpin name error." << endl;
	}

	void consistency_check(const Pin* pin, ostream& logger) {
		if (pin->name() != pin->port()->name()) logger << pin << " : name error." << endl;
		const Net* net = pin->net();
		if (net && find(net->pins(), pin) == net->pins().end()) logger << pin << " : net error." << endl;
	}
...
#define member_check_all(owner, members, logger) \
	for (const auto* m : owner->members) member_check<decltype(m)>(owner, logger)(m)
