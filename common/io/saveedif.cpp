...
namespace {
	...
	void write_net(const Net& net, ostream& os)
	{
		...
		{	
			if (!pin.is_mpin() && !pin.port().group())
				os << format("								(portRef %s (instanceRef %s ))\n") %rename(pin.name()) %rename(pin.owner().name());
			else if (!pin.is_mpin() && pin.port().group())
				os << format("								(portRef ( member %s %s )(instanceRef %s ))\n") %pin.port().group()->name() %pin.port().grp_idx() %rename(pin.owner().name());
 			else if (pin.is_mpin() && pin.port().group())
				os << format("								(portRef ( member %s %s ))\n") %pin.port().group()->name() %pin.port().grp_idx();
	...
	{	
		if (!cell.property_value(IO::used, false)) return;
		...
		foreach(const Port & port,cell.ports())
		{
			...
			else if (port.grp_idx() == 0)
			{
				...
				os << port.group()->name();
				os << " \""<<port.group()->name() << '[' << port.group()->msb() << ':' << port.group()->lsb() << "]\") ";
	...
	void write_edif_header(const string design_name, const string author, const string program, const string version, ostream& os)
	{
		...
		os << "		(status" << endl;	
		os << "				(written" << endl;	
		os << format("					(timeStamp  %d %d %d %d %d %d)\n") 
		 			%(local->tm_year+1900) %(local->tm_mon+1) %local->tm_mday %local->tm_hour %local->tm_min %local->tm_sec;
		os << "					(author \"" << author << "\")" << endl;
		os <<"					(program \"" << program << "\"" << "(version \"" << version << "\"))" << endl;
		os <<"				)" << endl;	
		os << "		)" << endl;	
	}
	...
	{
		os << format("		(design %s (cellRef %s (libraryRef %s)))\n") %design.name() % design.top_module().name() % design.top_module().library().name();
		os << ")";
	}
