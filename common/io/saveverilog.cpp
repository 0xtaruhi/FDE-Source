...
namespace {
	...
	void write_init(const string& inst_name, const string& pname, const string& pvalue, ostream& os) {
		os << format("\n  defparam %s.%s = %d'h%s;") % rename(inst_name) % pname % (pvalue.length()*4) % pvalue;
	}
	...
	void write_property(const Instance* inst, ostream& os) {
		for (PropertyBase* prop : inst->properties()) {
			...
			if (inst->down_module()->type() == "FFLATCH") continue; // for yosys
			string pname = prop->name();
			if (pname.find("INIT") != 0) continue;
	...
	void write_instance(const Instance* inst, ostream& os) {
		...
		os << "\n  " << rename(inst->down_module()->name()) <<  ' ' << rename(inst->name());
	...
	void write_module(const Module* mod, ostream& os) {
			...
			if (!(nports == 1 && vname == find_if(net->pins(), is_mpin)->name())) {
				...
				if (nports)
					for (const Pin* pin : net->pins())
						if (pin->is_mpin())
							if (pin->is_source())
								str_assign += str(format("  assign %s = %s;\n") % vname % rename_pin(pin));
							else
		...
		// module instances
		for (const Instance* inst : mod->instances()) write_instance(inst, os);
		os << "endmodule\n";
	}

	void write_library(const Library* lib, ostream& os) {
		for(const Module* module : IO::Writer::used_module(lib)) write_module(module, os);
	}

	void write_design(const Design* design, ostream& os) {
		for (const Library* lib : IO::Writer::used_lib(design))
			if (!lib->is_external()) write_library(lib, os);
	}
