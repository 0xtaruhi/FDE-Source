...
namespace COS {
	XML::NetlistHandler* COSRTFactory::make_xml_read_handler() { return new XML::RTNetlistHandler; }
...
namespace XML {
	...
	Net* RTNetlistHandler::load_net(xml_node* node, Module* mod) {
		...
		foreach_child(pip_node, node, "pip")
			load_pip(pip_node, net);
		return net;
	}
	...
	void RTNetlistHandler::load_pip(xml_node* node, COSRTNet* net) {
		...
		PIP::pip_dir dir = adir.empty() ? PIP::unidir : lexical_cast<PIP::pip_dir>(adir);
