...
namespace COS {
	XML::NetlistHandler* TimingFactory::make_xml_read_handler() { return new XML::TNetlistHandler; }
...
namespace XML {
	...
	Port* TNetlistHandler::load_port(xml_node* node, Module* mod) {
		...
		foreach_child(timing_node, node, "timing")
			load_timing(timing_node, port);
		return port;
	}
	...
	void TNetlistHandler::load_timing(xml_node* node, TPort* port) {
		...
		foreach_child(value_node, node, 0) {	// old netlist
			...
			if (name == "timing_type") type = value;
			else if (name == "related_pin") pin = value;
			else if (name == "timing_sense") sense = value;
			else if (name == "intrinsic_rise") rise = lexical_cast<double>(value);
			else if (name == "intrinsic_fall") fall = lexical_cast<double>(value);
		}
