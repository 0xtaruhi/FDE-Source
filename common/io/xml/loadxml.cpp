...
namespace COS {
	XML::NetlistHandler* CosFactory::make_xml_read_handler() { return new XML::NetlistHandler; }

namespace XML {
	using namespace boost;

	Design* NetlistHandler::load_libs(xml_node* node, Design* design) {
		foreach_child(lib_node, node, "external")
			load_library(lib_node, design);
	...
	Design* NetlistHandler::load_design(xml_node* node, Design* design) {
		...
		if (!top_node) top_node = node->first_node("topCell");		// old netlist
	...
	Library* NetlistHandler::load_library(xml_node* node, Design* design, bool external) {
		Library* lib = design->find_or_create_library(get_attribute(node, "name"), external);
		ASSERT(lib, "parse failed. library not created");
		foreach_child(mod_node, node, "module")
	...
	Module* NetlistHandler::load_module(xml_node* node, Library* lib) {
		...
		if (!contents_node) return mod;

		foreach_child(inst_node, contents_node, "instance") {
			Instance* inst = load_instance(inst_node, mod);
			if (!inst && atype == "RULE_CELL") return mod;
			ASSERT(inst, "parse failed. module not found");
		}
		foreach_child(net_node, contents_node, "net")
			load_net(net_node, mod);

		foreach_child(bus_node, contents_node, "bus")
			load_bus(bus_node, mod);
	...
	Instance* NetlistHandler::load_instance(xml_node* node, Module* mod) {
		...
		if (amref.empty()) amref = get_attribute(node, "cellRef");
...
//		ASSERTM(dmod, "parse failed. moduleRef: module not found");
		if (!dmod) return 0;			// for pack rule
	...
	Net* NetlistHandler::load_net(xml_node* node, Module* mod) {
		...
		foreach_child(pin_node, node, "portRef")
			load_pin(pin_node, net);
	...
	Bus* NetlistHandler::load_bus(xml_node* node, Module* mod) {
		...
		foreach_child(port_node, node, "portRef")
			load_bus_port(port_node, bus);
...
//  Constraint
	...
	void load_constraints(xml_node* node, Design* design) {
		foreach_child(port_node, node, "port") {
			...
			if (!spos.empty()) inst->set_property(pad, spos);
			string stype = get_attribute(port_node, "clk_type");
			if (!stype.empty()) inst->set_property(clk_type, stype);
		}
	}
...
namespace COS { namespace IO {
	...
	void XmlLoader::load(std::istream& istrm) {
		...
		std::unique_ptr<NetlistHandler> nlh(CosFactory::instance().make_xml_read_handler());
		if      (node->name() == string("design"))  nlh->load_design (node, design());
		else if (node->name() == string("library")) nlh->load_library(node, design());
		else                                        nlh->load_libs   (node, design());
	}
