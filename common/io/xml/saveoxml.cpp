...
namespace COS { namespace XML {
	...
	protected:
		...
		xml_node* write_property(xml_node* node, PropertyBase* prop, const Object* obj);
	...
	xml_node* ONetlistWriter::write_property(xml_node* node, PropertyBase* prop, const Object* obj) {
	...
	xml_node* ONetlistWriter::write_config(xml_node* node, Config* cfg, const Object* obj) {
	...
	xml_node* ONetlistWriter::write_instance(xml_node* mod_node, const Instance* inst) {
	...
	xml_node* ONetlistWriter::write_timing(xml_node* node, const TimingInfo* tinfo) {
		xml_node* timing_node = create_element(node, "timing");
		if (!tinfo->related_pin.empty())
			set_attribute(create_element(timing_node, "related_pin"), "value", tinfo->related_pin);
		if (!tinfo->timing_sense.empty())
			set_attribute(create_element(timing_node, "timing_sense"), "value", tinfo->timing_sense);
		if (!tinfo->timing_type.empty())
			set_attribute(create_element(timing_node, "timing_type"), "value", tinfo->timing_type);
		if (tinfo->intrinsic_rise != 0.0)
			set_attribute(create_element(timing_node, "intrinsic_rise"), "value", tinfo->intrinsic_rise);
		if (tinfo->intrinsic_fall != 0.0)
			set_attribute(create_element(timing_node, "intrinsic_fall"), "value", tinfo->intrinsic_fall);
		return timing_node;
	}
