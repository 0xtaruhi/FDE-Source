...
namespace COS {
	XML::NetlistWriter* CosFactory::make_xml_write_handler() { return new XML::NetlistWriter; }

	namespace XML {
		using namespace COS;

		xml_node* NetlistWriter::create_element(xml_node* node, const char* name, const Object* obj, int copy_string) {
		...
		xml_node* NetlistWriter::write_property(xml_node* node, PropertyBase* prop, const Object* obj) {
			...
			if (type != "string") set_attribute(prop_node, "type", type, COPY_VALUE);
			set_attribute(prop_node, "value", prop->string_value(obj), COPY_VALUE);
			return prop_node;
		}

		xml_node* NetlistWriter::write_config(xml_node* node, Config* cfg, const Object* obj) {
		...
		xml_node* NetlistWriter::write_instance(xml_node* mod_node, const Instance* inst) {
...
namespace COS { namespace IO {
	...
	void XmlWriter::write(std::ostream& ostrm) const {
		mark_used();
		std::unique_ptr<NetlistWriter> nlw(CosFactory::instance().make_xml_write_handler());
