...
namespace COS {
	...
	class Design; class Library; class Module; class Instance; class Port; class Net; class Bus; class Pin;
	namespace XML { class NetlistHandler; class NetlistWriter; }
...
//  CosFactory
	...
	public:
		...
		static pointer set_factory(pointer& f)    { pointer p = std::move(_instance); _instance = std::move(f); return p; }
		static pointer set_factory(CosFactory* f) { pointer p{f}; return set_factory(p); }

		virtual ~CosFactory() {}
//		virtual Design*   make_design(const string& name);
		virtual Library*  make_library(const string& name, Design* owner, bool extrn);
		virtual Module*   make_module(const string& name, const string& type, Library* owner);
		virtual Instance* make_instance(const string& name, Module* down_module, Module* owner);
		virtual Net*      make_net(const string& name, NetType type, Module* owner, Bus* bus);
		virtual Bus*      make_bus(const string& name, Module* owner, int msb, int lsb);
		virtual Port*     make_port(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from);
		virtual Pin*      make_pin(const string& name, Port* port, Module* owner, int index, int mindex);
...
//  Design
	...
	public:
		...
		const Library* find_library(const string& name) const { return libs().find(name); }
		Library* find_or_create_library(const string& name, bool extrn = false) {
			if (Library* lib = find_library(name)) return lib;
			else return create_library(name, extrn);
		}
...
//  Library
	...
	public:
		...
		Module* create_module(const string& cell_name, const string& cell_type = "")
			{ return _modules.add(CosFactory::instance().make_module(cell_name, cell_type, this)); }
		...
		const Module* find_module(const string& name) const { return modules().find(name); }
...
//  Module
	...
	public:
		...
		Port* create_port(const string& name, int msb, int lsb, DirType dir, PortType type = NORMAL);
		Port* create_port(const string& name, DirType dir, PortType type = NORMAL);
		port_iter remove_port(port_iter pit);
		void      remove_port(Port* port)   { remove_port(boost::range::find(ports(), port)); }
		...
		std::pair<Instance*, inst_iter> create_instance(const string& inst_name, Module* down_module, inst_iter iit);
		...
		const Instance* find_instance(const string& name) const { return instances().find(name); }
		...
		Instance* find_up_instance(const string& name) { return up_instances().find(name); }
		const Instance* find_up_instance(const string& name) const { return up_instances().find(name); }
		...
		string path_name() const { return library()->path_name() + "/module:" + name(); }
...
//  Instance
	...
	public:
		...
		Pin* create_pin(Pin* mpin) { return _pins.add(CosFactory::instance().make_pin(mpin, this)); }
		Pin* find_pin(const string& name) { return pins().find(name); }
		const Pin* find_pin(const string& name) const { return pins().find(name); }
		...
		Instance* clone(Module* new_owmer) const { return static_cast<Instance*>(Object::clone(new_owmer)); }
		...
		PropRepository::configs_type configs() const { return get_configs(module_type()); }
	...
	protected:
		...
		{ return static_cast<Module*>(new_owmer)->create_instance(name(), down_module()); }
...
//  Port
	...
	public:
		Port(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from);
		...
		Module::const_pins_type mpins() const { return static_cast<const Module*>(module())->pins(this); }
		...
		Instance::const_pins_type up_pins(const Instance* inst) const { return inst->pins(this); }
		Pin*       up_pin(Instance* inst, int idx = 0)             { return inst->pin(_pin_from + idx); }
		const Pin* up_pin(const Instance* inst, int idx = 0) const { return inst->pin(_pin_from + idx); }

		Port* clone(Module* new_owmer) const { return static_cast<Port*>(Object::clone(new_owmer)); }
	...
	protected:
		Object* do_clone(Object* new_owmer) const {
			return is_vector() ?
					  static_cast<Module*>(new_owmer)->create_port(name(), msb(), lsb(), dir(), type())
					: static_cast<Module*>(new_owmer)->create_port(name(), dir(), type());
		}
...
//  Pin
	...
	public:
		...
		Module*     module()        const { return is_mpin() ? static_cast<Module*>(owner()) : 0;   }
		Instance*   instance()      const { return is_mpin() ? 0 : static_cast<Instance*>(owner()); }
		Module*     down_module()   const { return port()->module(); }
		Pin*        down_pin()      const { return _mpin;            }
		Pin* up_pin(Instance* inst) const { return is_mpin() ? inst->pin(_mindex) : 0; }
		...
		void disconnect();
		void reconnect(Net* net) { disconnect(); connect(net); }
...
//  Net
	...
	public:
		...
		Net* clone(Module* new_owmer) const { return static_cast<Net*>(Object::clone(new_owmer)); }
...
//  Bus
	...
	public:
		...
		void connect(Port* port, ConnectDir dir = CONN_LEFT) ;					// connect to module pins
		void connect(Instance* inst, Port* port, ConnectDir dir = CONN_LEFT) ;	// connect to instance pins

	private:
		friend Bus* Module::create_bus(const string& name, int msb, int lsb, NetType type);
		int _msb, _lsb;
...
//  inline functions
	...
	inline Instance* Module::create_instance(const string& inst_name, Module* down_module) {
		Instance* inst = _instances.add(CosFactory::instance().make_instance(inst_name, down_module, this));
		down_module->_up_instances.add(inst);
		return inst;
	}
	inline std::pair<Instance*, Module::inst_iter> Module::create_instance(const string& inst_name, Module* down_module, inst_iter iit)	{
		std::pair<Instance*, Module::inst_iter> p = _instances.insert(iit, CosFactory::instance().make_instance(inst_name, down_module, this));
...
//		if (port->module() != this) return -1;		// for Release
	...
	inline int Instance::pin_from(const Port* port) const {
		ASSERTD(port->module() == down_module(), "port belongs to a different module.");
//		if (port->module() != down_module()) return -1;		// for Release
		return port->_pin_from;
	}
	...
	inline void Pin::disconnect() { if (_net) _net->remove_pin(this), _net = 0; }
