...
namespace COS {
	...
	enum ObjectClass { OBJECT, DESIGN, LIBRARY, MODULE, INSTANCE, PORT, NET, PIN, BUS };
	...
	public:
		...
		{ ASSERT(0, "this function shound not be called."); return ""; }
	...
	public:
		...
		template<typename T> Property<T>& create_property(ObjectClass obj_class, const string& name, const T& def_val, StorageType st);
		template<typename T> Property<T>& create_temp_property(ObjectClass obj_class, const string& name, const T& def_val, StorageType st);
		...
		Config& create_config(const string& inst_type, const string& name, const string& def_val);
	...
	public:
		explicit Property(const T& def_val = T()) : PropertyBase("", TEMPORARY), _def_val(def_val) {}	// temporary property
		...
		T& get_ref(const Object* obj) {
			map_iter p = _prop_map.find(obj);
			if (p == _prop_map.end()) set_value(obj, _def_val);
...
//		{ _prop_map.try_emplace(obj, std::foward<A>(vals)...); }
	...
	protected:
		Property(const string& name, const T& def_val, StorageType st) : PropertyBase(name, st), _def_val(def_val) {}
//		friend Property<T>& PropRepository::create_temp_property(ObjectClass obj_class, const string& name, const T& def_val, StorageType st);
	...
	template<typename T> class RProperty : public Property<T> {	// Repository Property, T must support stream input/output
	public:
		void set_string_value(const Object* obj, const string& value) { Property<T>::set_value(obj, boost::lexical_cast<T>(value)); }
		string string_value(const Object* obj) const { return boost::lexical_cast<string>(Property<T>::get_value(obj)); }
		string value_type() const { return prop_type_traits<T>::type_string(); }
	protected:
		RProperty(const string& name, const T& def_val, StorageType st) : Property<T>(name, def_val, st) {}
//		friend Property<T>& PropRepository::create_property(ObjectClass obj_class, const string& name, const T& def_val, StorageType st);
		friend class PropRepository;
	};

	// specialization for Property<string>
	template<> inline void   RProperty<string>::set_string_value(const Object* obj, const string& value) { set_value(obj,value); }
	template<> inline string RProperty<string>::string_value(const Object* obj) const { return get_value(obj); }

	class Config : public RProperty<string> {
	private:		// cannot create temporary config
		Config(const string& name, const string& def_val) : RProperty<string>(name, def_val, COPY_SAVE) {}
		friend Config& PropRepository::create_config(const string& inst_type, const string& name, const string& def_val);
	};
	...
	template<typename T> Property<T>& PropRepository::create_property(ObjectClass obj_class, const string& name, const T& def_val, StorageType st) {
		auto pb = find_property(obj_class, name);
		if (auto p = dynamic_cast<RProperty<T>*>(pb)) return *p;
		ASSERT(!pb, name << " has been created as temporary property");
		return static_cast<Property<T>&>(*_propRepository[obj_class].add(new RProperty<T>(name, def_val, st)));
	}

	template<typename T> Property<T>& PropRepository::create_temp_property(ObjectClass obj_class, const string& name, const T& def_val, StorageType st) {
		if (auto p = dynamic_cast<Property<T>*>(find_property(obj_class, name))) return *p;
		return static_cast<Property<T>&>(*_propRepository[obj_class].add(new Property<T>(name, def_val, st)));
	}

	template<typename T> inline Property<T>& create_property(ObjectClass obj_class, const string& name, const T& def_val = T(), StorageType st = COPY_SAVE) {
		return PropRepository::instance().create_property(obj_class, name, def_val, st);
	}

	template<typename T> inline Property<T>& create_temp_property(ObjectClass obj_class, const string& name, const T& def_val = T(), StorageType st = TEMPORARY) {
		ASSERT(!(st & SAVE), "temporary property cannot save to file");
		return PropRepository::instance().create_temp_property(obj_class, name, def_val, st);
	}
	...
	inline Config& PropRepository::create_config(const string& inst_type, const string& name, const string& def_val) {
		if (Config* c = find_config(inst_type, name)) return *c;
		return *_configRepository[inst_type].add(new Config(name, def_val));
	}

	inline Config& create_config(const string& inst_type, const string& name, const string& def_val = "#OFF") {
