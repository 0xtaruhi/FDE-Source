...
//  pointer container and range type
	...
	struct PtrContainer : public C {
		...
		struct iter_pair : Rng {
			...
			template<typename R> iter_pair(const R& rng) : Rng(rng.begin(), rng.end()) {}

			typename std::iterator_traits<IT>::pointer find(const string& name) const {
				IT it = find_if(begin(), end(), [&name](const T* t) { return name == t->name(); } );
		...
		T* add(T* t) { C::insert(end(), t); return t; }
		...
		template<typename DT> struct typed {
			template<typename IT> struct iter_t : IT {
				...
				iter_t() : IT() {}
				iter_t(IT it) : IT(it) {}
				pointer operator*()  const { return static_cast<pointer>( IT::operator*() ); }
		...
		template<typename DT> typename typed<DT>::range       range()       { return typed<DT>::range(begin(), end());       }
		template<typename DT> typename typed<DT>::const_range range() const { return typed<DT>::const_range(begin(), end()); }
	...
	template<typename T, bool OWNER = true> using PtrVector = PtrContainer<T, std::vector<T*>, OWNER>;
	template<typename T, bool OWNER = true> using PtrList   = PtrContainer<T, std::list<T*>,   OWNER>;

	template<typename T, typename C = std::less<T*>, bool OWNER = true> struct PtrSet : PtrContainer<T, std::set<T*, C>, OWNER> {
...
//		typedef  std::set<T*, C, std::allocator<void*> >	SET;
        ...
        using const_iterator = typename PtrContainer<T, std::set<T*, C>, OWNER>::const_iterator;
        using range_type       = typename PtrContainer<T, std::set<T*, C>, OWNER>::range_type;
		using const_range_type = typename PtrContainer<T, std::set<T*, C>, OWNER>::const_range_type;
