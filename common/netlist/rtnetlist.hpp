...
//  RoutingConnection
	...
	inline std::istream& operator >> (std::istream& s, PIP::pip_dir& dir) { dir = pipmap.readEnum(s); return s; }
	inline std::ostream& operator << (std::ostream& s, PIP::pip_dir  dir) { return pipmap.writeEnum(s, dir); }
...
//  RTNet
	class COSRTNet : public Net {
		PtrVector<PIP> _pips;
	public:
		COSRTNet(const string& name, NetType type, Module* owner, Bus* bus) : Net(name, type, owner, bus) {}
		...
		PIP* create_pip(const string& from, const string& to, const Point& pos, PIP::pip_dir dir = PIP::unidir)
...
//  RTModule
	...
	public:
		RTModule(const string& name, const string& type,  Library* owner) : Module(name, type, owner) {}
		//  Nets
		...
		const COSRTNet* find_net(const string& name) const { return nets().find(name); }
	};
...
//  RTFactory
	...
	public:
		virtual Net* make_net(const string& name, NetType type, Module* owner, Bus* bus) { return new COSRTNet(name, type, owner, bus); }
		virtual Module* make_module(const string& name, const string& type, Library* owner)       { return new RTModule(name, type, owner); }
