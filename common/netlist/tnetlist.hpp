...
//  TPort
	class TPort : public Port {
		...
		double _C;
	public:
		TPort(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from) :
		...
		void create_timing(const string& type, const string& pin, const string& sense, double rise, double fall)
		{ return _timings.push_back(TimingInfo(type, pin, sense, rise, fall)); }
		void create_timing(const TimingInfo& tinfo) { return _timings.push_back(tinfo); }
