...
Element* Element::create_item(const string& id, const string& label, const string& value) {
...
void Report::set_item(const string& id, const string& value) {
	...
	if (target) target->set_value(value);
	else FDU_LOG(WARN) << "[Report::set_item] Cannot locate target with id: " << id;
}

void Report::set_item(const string& id, int part, int all) {
	if(part > all) std::swap(part, all);
	set_item(id, str(format("%d out of %d \t%.2f%%") % part % all % (part * 100. / all)));
}
...
void Row::set_item(const string& id, const string& value){
	...
	Element* target = find_element(id);
	if (target) target->set_value(value);
	else FDU_LOG(WARN) << "[Row::set_item] Failed to locate column with id: " << id;
}
...
std::istream& operator >> (std::istream& s, TimingType& t) { t = typemap.readEnum(s); return s; }
std::ostream& operator << (std::ostream& s, TimingType  t) { return typemap.writeEnum(s, t);    }

static const char* modes[] = { "R2R", "R2O", "I2R" };
static EnumStringMap<TimingMode> modemap(modes);
std::istream& operator >> (std::istream& s, TimingMode& m) { m = modemap.readEnum(s); return s; }
std::ostream& operator << (std::ostream& s, TimingMode  m) { return modemap.writeEnum(s, m);    }
...
STASection::STASection(Element* parent, TimingType type) : Element(parent, "section"), path_count_(0)
{ set_attr("type", to_string(type)); }

STAPath* STASection::create_path(const string& src, const string& src_mod , const string& dst, const string& dst_mod) {
...
STANode* STAPath::create_node(const string& dir, const string& name, double tarr) {
...
Element* read_item(xml_node* node, Element* parent) {
	...
	foreach_child (child, node,) {
		...
		if (node_name == "item") read_item(child, item);
		else FDU_LOG(WARN) << "read_section: invalid tag name: " << node_name;
	}
	return item;
}
...
Table* read_table(xml_node* node, Section* parent) {
	...
	foreach_child (child, node,) {
		string node_name = child->name();
		if(node_name == "column") read_attrs(table->create_column(), child);
		else FDU_LOG(WARN) << "read_table: invalid tag name: " << node_name;
	}
	return table;
}
...
Section* read_section(xml_node* node, Report* parent) {
	...
	foreach_child (child, node,) {
		string node_name = child->name();
		if (node_name == "item")      read_item(child, sec);
		else if(node_name == "table") read_table(child, sec);
		else FDU_LOG(WARN) << "read_section: invalid tag name: " << node_name;
	}
	return sec;
}
...
void ReportWriter::write_report(const Report* rpt) {
	...
	create_pi(&document(), "xml-stylesheet", str(format("type='text/xsl' href='%1%'") % rpt->xslt()), COPY_VALUE);
...
void Report::read_template(const string& temp_name) {
	...
	foreach_child(node, root,) {
		...
		if(node_name == "item")          read_item(node, this);
		else if (node_name == "section") read_section(node, this);
		else FDU_LOG(WARN) << "read_template: invalid tag name: " << node_name;
	}
}
