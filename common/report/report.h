...
public:
	...
	Element(Element* parent, const string& name = "item") : parent_(parent), name_(name) {}
	...
	void set_attr(const string& name, const string& value) { attrs_[name] = value; }
	...
	void set_attrs(const string& id, const string& label, const string& value = "");

	Element* create_item(const string& id = "", const string& label = "", const string& value = "");
...
inline void Element::set_attrs(const string& id, const string& label, const string& value) {
	if (!id.empty())    set_id(id);
	if (!label.empty()) set_label(label);
	if (!value.empty()) set_value(value);
}

class Section; class Table; class Column; class Row;
...
public:
	...
	template<typename T>
	void set_item(const string& id, const T& value) { set_item(id, FDU::to_string(value)); }
...
class STAPart; class STADomain; class STASection; class STAPath; class STANode;
...
public:
	...
	STAPath* create_path(const string& src, const string& src_mod , const string& dst, const string& dst_mod);
private:
	int path_count_;
};
