...
///// MemPool
	...
	template<typename T>
	MemPool<T>::MemPool(int init_size /* = 20000 */)
	{
		init(init_size);
	}
	...
	template<typename T>
	typename MemPool<T>::TPtr MemPool<T>::next()
	{
	...
	template<typename T>
	void MemPool<T>::init(int init_size)
	{
		_next_pos = 0;
	...
	template<typename T>
	void MemPool<T>::resize(int size)
	{
...
///// RRGArchFactory
	...
	Instance* RRGArchFactory::make_instance(const string& name, Module* down_module, Module* owner)
	{ return new RRGArchInstance(name, down_module, owner); }
	Net* RRGArchFactory::make_net(const string& name, NetType type, Module* owner, Bus* bus)
	{ return new RRGArchNet(name, type, owner,bus); }
	Module* RRGArchFactory::make_module(const string& name, const string& type, Library* owner)
...
///// RRGFactory
	...
	RRGSwitch* RRGFactory::make_rrgswitch(ArchPath* owner, RRGNode* from_node, RRGNode* to_node, const Point& pos)
...
///// RRGNode
	...
	{
		return str(
			format("%1%_from_%2%_to_%3%") % owner_name()
										  % lexical_cast<string>(_from_pos) 
										  % lexical_cast<string>(_to_pos)
		);
	}

	RRGSwitch* RRGNode::find_switch(RRGNode* t)
	{
		switch_iter sw_it = find_if(switches(), [t](const RRGSwitch* sw) { return sw->to_node() == t; });
	...
	RRGNode::RRG_NODE_DIR  RRGNode::direction() const
	{
		return _from_pos   == _to_pos   ? IGN_DIR : 
			   _from_pos.y != _to_pos.y ? HORIZONTAL : VERTICAL;
	}
	...
	int	RRGNode::length() const
	{
		return  direction() == HORIZONTAL ? abs(_to_pos.y - _from_pos.y) : 
				direction() == VERTICAL   ? abs(_to_pos.x - _from_pos.x) : 0;
	}
	...
	RRGNode::RRG_NODE_TYPE RRGNode::type() const
	{
		...
		if (owner_name().find("GCLK") != string::npos)
			...
			(owner_name().find("LH") != string::npos
			...
			type = LONG;
		else if (owner_name().find("H6") != string::npos || owner_name().find("V6") != string::npos)
			type = HEX;
...
///// RRGSwitch
	...
	istream& operator >> (istream& s, RRGSwitch::RRG_SW_TYPE& sw) { sw = sw_type_map.readEnum(s); return s; }
	ostream& operator << (ostream& s, RRGSwitch::RRG_SW_TYPE  sw) { return sw_type_map.writeEnum(s, sw); }

	//const Point& RRGSwitch::pos() const {return _from_node->to_pos(); }

	RRGSwitch* RRGNode::create_switch(ArchPath* o, RRGNode* f, RRGNode* t, const Point& pos)
	{
...
///// RRGraph
	...
	RRGNode* RRGraph::find_logic_pin_node(const Instance* inst, const Pin& pin, const Point& pos) const
	{
...
// 			int a = 0;
		for(int i = 0; i < loop; ++i){
			...
			ArchCell* tile_cell = FPGADesign::instance()->get_inst_by_pos(t_pos)->down_module();
			for (const ArchInstance* ainst : tile_cell->instances()) {
				if (ainst->module_type() != inst->module_type() || ainst->z_pos() != pos.z)
					continue;
				const Pin* t_pin = ainst->find_pin(pin.name());
				if(t_pin == nullptr || t_pin->dir() != pin.dir()) return nullptr;
				if(!t_pin->net()) continue;
				for (const Pin* pin : t_pin->net()->pins()) {
					if (pin->is_mpin()) continue;
					if (pin->instance()->module_type() == CELL::GRM || pin->instance()->module_type() == CELL::GSB){
	...
	RRGNode* RRGraph::find_grm_net_node(const string& net_name, const Point& pos) const
	{
		int loop = is_ram(pos) ? RRGPara::RAM_SPAN_RNG : 1;
		for(int i = 0; i < loop; ++i) {
			Point t_pos(pos.x - i, pos.y, pos.z);
			...
			ArchCell::inst_iter grm_it = find_if(tile_cell->instances(), [this](const Instance* inst) { return is_grm(inst); });
			if(grm_it == tile_cell->instances().end()) return nullptr;
	...
	{ return inst->module_type() == CELL::GRM ? true :
				inst->module_type() == CELL::GSB ? true : false; }
	...
	void RRGraph::init_arch_info()
	{
		...
		for (Module* prim_cell : prim_lib->modules())
		{
			if (prim_cell->type() != CELL::GRM && prim_cell->type() != CELL::GSB)
				...
				for (RRGArchInstance* ainst : static_cast<RRGArchCell*>(prim_cell)->instances()) {
					for (ArchPath* path : ainst->down_module()->paths()) {
						...
						ainst->add_path(static_cast<RRGArchNet*>(f_pin->net()), 
									   static_cast<RRGArchNet*>(t_pin->net()), 
									   path); 
					}
				}
		}
...
///// Step 1
	...
	void RRGraph::init_grm_info(int cur_col)
	{
		for(int row = 0; row < _info._device_scale.x; ++row) {
			...
			ArchCell* tile = FPGADesign::instance()->get_inst_by_pos(curr_pos)->down_module();
			cur_col_tile(row) = tile; // used to improve speed			

			ArchCell::inst_iter grm_it = boost::find_if(tile->instances(), [this](const Instance* inst) { return is_grm(inst); });
			if(grm_it != tile->instances().end()) {
				reset_grm_nodes(*((*grm_it)->down_module()));
				_nodes_lut.at(curr_pos).resize((*grm_it)->down_module()->num_nets(), nullptr);
			}
		}
	}
...
///// Step 2
	...
	{
		RRGArchNet* grm_cpin_net = static_cast<RRGArchNet*>(grm_pin.down_pin()->net());
#ifdef ALLOW_DANGLING
		if(!grm_cpin_net){
		...
		if(grm_cpin_net->ptr_to_rrg_node_ptr(row)) {
			ASSERTD(pptr == grm_cpin_net->ptr_to_rrg_node_ptr(row),
					"multi sources: multi tile ports to one grm port " + grm_pin.name());
		} else {
			grm_cpin_net->set_ptr_to_rrg_node_ptr(row, pptr);
		}
	}

	void RRGraph::spread_ptr_to_tile_port_and_grm(int row, ArchPort& cur_tile_port, SideNodePPtrs& cur_side_node_pptrs)
	{
		int pin_num = cur_tile_port.width();
		for (int index = 0; index < pin_num; index++)
		{
			PtrToRRGNodePtr node_pptr = 
				cur_side_node_pptrs[cur_tile_port.side()][cur_tile_port.mpin(index)->index()];
			ASSERTD(node_pptr, cur_tile_port.name() + ": pptr of tile port is nullptr.");

			Pin* port_cpin = cur_tile_port.mpin(index);
			Net* cpin_net  = port_cpin->net();
// 			ASSERTD(cpin_net, rrg_error(string("dangling: tile port ").append(cur_tile_port.name()).append(" unconnected.")));
//			80k arch has some discontinous port serial number.

			if(cpin_net){
				for (Pin* pin : cpin_net->pins()) {	
					if(pin == port_cpin) continue;

					// connect to other tile ports
					if(pin->is_mpin()) {
						ArchPort* another_tile_port = static_cast<ArchPort*>(pin->port());
						cur_side_node_pptrs[another_tile_port->side()][pin->index()] = node_pptr;
					}
					// connect into GRM
					else if(pin->instance()->module_type() == CELL::GRM || pin->instance()->module_type() == CELL::GSB)
	...
	void RRGraph::distinguish_nets_in_a_column(int cur_col, NodePtrs& cur_node_ptrs, ColNodePPtrs& cur_col_node_pptrs)
	{
		...
		for(int row = 0; row < _info._device_scale.x; ++row) {
			...
			for(int side = 0, count = 0; count < RRGPara::NUM_TILE_SIDES - 1; ++side) {
				if(side != TOP) {
					cur_col_node_pptrs[row][side].assign(cur_col_node_pptrs[row][side].size(), nullptr);
					++count;
				}
			}
			...
			for (ArchPort* tile_port : tile.ports()) {
				for (Pin* pin : tile_port->mpins()) {
					if(!cur_side_node_pptrs[tile_port->side()][pin->index()]) {
						cur_side_node_pptrs[tile_port->side()][pin->index()] = cur_node_ptrs.next();
					}
				}
...
///// Step 3 
...
#ifdef DEVELOPING
	// slower
	void RRGraph::spread_nodes_from_last_column(int cur_col, ColNodePPtrs& last_col_node_pptrs, ColNodePPtrs& cur_col_node_pptrs)
	{
		for(int row = 0; row < _info._device_scale.x; ++row) {
#if defined(SPREAD_FROM_LEFT_TO_RIGHT)
			Point last_col_pos(row, cur_col-1);
			ArchCell* last_col_tile = FPGADesign::instance()->get_inst_by_pos(last_col_pos)->down_module();
			for (ArchPort* tile_port: last_col_tile->ports() {
				if (tile_port->side() != RIGHT) continue;
				for(Pin* pin: tile_port->mpins()) {
					int idx = pin->index();
					// assert last column
					ASSERTD(last_col_node_pptrs[row][RIGHT][idx],	// RRGNode**
							tile_port->name() + ": pptr of tile port at last column is nullptr.");
					/* 
					 * actually exist tile port cell pin that connect a net which never has another pin connected, 
					 * even in fqvr300's arch, so release this ASSERT here
					 */
	//				ASSERTD(*last_col_node_pptrs[row][RIGHT][idx],	// RRGNode*
	//					    rrg_error(tile_port.name() + ": ptr to RRGNode at last column is nullptr."));
					// assert current column
					ASSERTD(cur_col_node_pptrs[row][LEFT][idx],		// RRGNode**
							tile_port->name() + ": pptr of tile port at current column is nullptr.");
					if(*cur_col_node_pptrs[row][LEFT][idx]) {		// RRGNode*
						ASSERTD(*cur_col_node_pptrs[row][LEFT][idx] == *last_col_node_pptrs[row][RIGHT][idx],
								string("multi sources: ") + "No." + lexical_cast<string>(idx) + \
								"port of tile at " + lexical_cast<string>(Point(row, cur_col)));
					} else {
						*cur_col_node_pptrs[row][LEFT][idx] = *last_col_node_pptrs[row][RIGHT][idx];
					}
				}
			}
#elif defined(SPREAD_FROM_RIGHT_TO_LEFT)
			...
			ArchCell* last_col_tile = FPGADesign::instance()->get_inst_by_pos(last_col_pos)->down_module();
			for (ArchPort* tile_port : last_col_tile->ports()) {
				if (tile_port->side() != LEFT) continue;
				for (Pin* pin : tile_port->mpins()) {
					int idx = pin->index();
					// assert last column
					...
					*/
	//				ASSERTD(*last_col_node_pptrs[row][LEFT][idx],		// RRGNode*
	//					    tile_port.name() + ": ptr to RRGNode at last column is nullptr.");
					// assert current column
					ASSERTD(cur_col_node_pptrs[row][RIGHT][idx],		// RRGNode**
							tile_port->name() + ": pptr of tile port at current column is nullptr.");
					if(*cur_col_node_pptrs[row][RIGHT][idx]) {			// RRGNode*
						ASSERTD(*cur_col_node_pptrs[row][RIGHT][idx] == *last_col_node_pptrs[row][LEFT][idx],
							boost::format("multi sources: No. %1% port of tile at %2%") % idx % Point(row, cur_col));
					} else {
						*cur_col_node_pptrs[row][RIGHT][idx] = *last_col_node_pptrs[row][LEFT][idx];
					}
				}
			}
#endif
	...
	{
		for(int row = 0; row < _info._device_scale.x; ++row) {
			for(int idx = 0, range = last_col_node_pptrs[row][RIGHT].size(); idx < range; ++idx) {
				if(cur_col_node_pptrs[row][LEFT][idx] && last_col_node_pptrs[row][RIGHT][idx])
					*cur_col_node_pptrs[row][LEFT][idx] = *last_col_node_pptrs[row][RIGHT][idx];
			}
		}
	}
...
///// Step 4
	...
	static void check_net_is_inside_tile(ArchInstance& grm, RRGArchNet& net_in_grm)
	{
		for (Pin* grm_cell_pin : net_in_grm.pins()) {
			if (!grm_cell_pin->is_mpin()) continue;
			RRGArchNet* net_in_tile = static_cast<RRGArchNet*>(grm.find_pin(grm_cell_pin->name())->net());
			ASSERT(net_in_tile, "can not find corresponding tile level net of " + net_in_grm.name() + " in " + net_in_grm.owner()->name());
			
			RRGArchNet::pin_iter tile_cell_pin_it = find_if(net_in_tile->pins(), [](const Pin* pin) { return pin->is_mpin(); });
			ASSERT(tile_cell_pin_it == net_in_tile->pins().end(), 
					net_in_grm.name() + ": pptr of this net in grm is nullptr.");
		}
	}

	void RRGraph::build_grm(const Point& pos, ArchInstance& grm, NodePtrs& cur_node_ptrs)
	{
		...
		// for each net in GRM
		for (RRGArchNet* net : static_cast<RRGArchCell*>(grm.down_module())->nets())  {
		...
		for (RRGArchInstance* elem : static_cast<RRGArchCell*>(grm.down_module())->instances()) {
			// need to confirm
			bool pt_flag = elem->module_type().compare(sw_types[RRGSwitch::PT]) == 0 ? true : false;
			...
			for (RRGArchInstance::Path* path : elem->paths()) {
				...
				else if(sw->type() == RRGSwitch::PT && pt_flag){ // only consider once for PT
					f_node->plus_Cext(sw->Cin());
					t_node->plus_Cext(sw->Cout());
					pt_flag = false;
				}
				else {} // dummy switch, do nothing
			}
		}
 	}
