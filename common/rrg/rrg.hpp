...
// Class Declaration
	...
	class RRGArchInstance; class RRGArchNet; class RRGArchCell;
	class RRGNode; class RRGSwitch;
...
///// RRGArchInstance
	...
	public:
		...
		const_paths_type paths() const { return const_paths_type(_paths.begin(), _paths.end()); }
...
///// RRGArchCell
	...
	public:
		...
		typedef PtrList<Instance>::typed<RRGArchInstance>::const_iterator	const_inst_iter;
		typedef PtrList<Instance>::typed<RRGArchInstance>::range				instances_type;
		typedef PtrList<Instance>::typed<RRGArchInstance>::const_range		const_instances_type;
...
///// RRGFactory
	...
	public:
		...
		static Pointer  set_factory(RRGFactory* f) { Pointer p{f}; return set_factory(p); }

		virtual ~RRGFactory() {}
		virtual RRGNode*   make_rrgnode  (RRGArchNet* owner);
		virtual RRGSwitch* make_rrgswitch(ArchPath* owner, RRGNode* from_node, RRGNode* to_node, const Point& pos);
...
///// RRGSwitch
	...
	public:
		...
		RRGSwitch(ArchPath* o, RRGNode* f, RRGNode* t, const Point& pos)
			: _owner(o), _from_node(f), _to_node(t), _from_net(nullptr), _to_net(nullptr), _pos(pos) {}
		virtual ~RRGSwitch() {}
...
///// RRGNode
	...
	public:
		...
		enum RRG_NODE_TYPE { IGN_TYPE = -1, SOURCE, CLK, SINK, SINGLE, HEX, LONG, NUM_OF_LEN };
		...
		//void set_length		();
		//void set_type		();
		void set_timing		(double R, double C) { _timing._R = R; _timing._C = C; }
		void plus_Cext		(double c_ext)		 { _timing._Cext += c_ext; }	  

		RRGSwitch* create_switch(ArchPath* o, RRGNode* f, RRGNode* t, const Point& pos);
...
///// RRGraph
	...
	public:
		...
		void	 build_rrg();
		RRGNode* find_logic_pin_node(const Instance* inst, const Pin& pin, const Point& pos) const;
	...
	protected:
		...
		//		   and allocate a PtrToRRGNodePtr(RRGNode**) to each connected net separately
		void spread_ptr_into_grm			(int			 row, 
											 Pin&			 grm_pin,	 
											 PtrToRRGNodePtr pptr);
		void spread_ptr_to_tile_port_and_grm(int			 row,
											 ArchPort&		 cur_tile_port, 
											 SideNodePPtrs&  cur_side_node_pptrs);
		void distinguish_nets_in_a_column	(int			 col, 
		...
		// Step 4: build RRGNodes for all the nets in the grm which "*(PtrToRRGNodePtr) == nullptr"
