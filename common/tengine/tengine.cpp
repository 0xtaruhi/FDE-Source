...
#define filtered_foreach(type, var, rng, cond)			\
	for (type var : rng | boost::adaptors::filtered([](type p) { return p->cond; }))
#define filtered_foreach2(type, var, rng, cond1, cond2) \
	for (type var : rng | boost::adaptors::filtered([](type p) { return p->cond1 && p->cond2; }))
...
//  TFactory
	...
	TEdge* TFactory::make_tedge(TNode* f_tnode, TNode* t_tnode, double r_delay, double f_delay) 
...
//  TNode
	...
	TEdge* TNode::create_out_edge(TNode* t_tnode, double r_delay /* = TPara::ZERO */, double f_delay /* = TPara::ZERO */)
	{ return t_tnode->_in_tedges.add(_out_tedges.add(TFactory::instance().make_tedge(this, t_tnode, r_delay, f_delay))); }
	...
	{  
		for (const IntTDataPair& delay : max_delays)
			_t_req_vec[delay.first].set_value(delay.second._rising, delay.second._falling);
	}
	...
	void TNode::update_normal_tarrs(const TEdge* f_tedge)
	{
		...
		for (const TData& tdata : f_tedge->from_node()->t_arrs()){
			tarr_it->_rising  = max(tarr_it->_rising,  tdata._rising  + f_tedge->r_delay());
			tarr_it->_falling = max(tarr_it->_falling, tdata._falling + f_tedge->f_delay());
			++tarr_it;
		}
	}
	...
	void TNode::update_normal_treqs(const TEdge* t_tedge)
	{
		...
		for (const TData& tdata : t_tedge->to_node()->t_reqs()){
			treq_it->_rising  = min(treq_it->_rising,  tdata._rising  - t_tedge->r_delay());
			treq_it->_falling = min(treq_it->_falling, tdata._falling - t_tedge->f_delay());
			++treq_it;
		}
	}
...
//  TEngine
	...
	{ _tnodes.clear(); create_all_nodes(); create_all_conns(); topo_sort(); _is_built = true; }

	void TEngine::compute_tgraph()
	{ initial_tdata(); compute_tarr(); compute_treq(); compute_slack();}

	TNode* TEngine::create_node(TNode::TNodeType type, Pin* owner, bool set_pin_prop /* = true */)
	{
		TNode* tnode = nullptr;
		if(!(tnode = owner->property_value(TNODE))) {
			tnode = _tnodes.add(TFactory::instance().make_tnode(type, owner, num_tnodes()));
			if(set_pin_prop) owner->set_property(TNODE, tnode);
		}
		return tnode;
	}
	...
	{
//		ASSERTD(port_pin.is_connected(), tengine_error("dangling: top module port."));
		if(!mpin->is_connected()) return;

		Pin* pad_pin = mpin->is_sink() ? *mpin->net()->source_pins().begin() : 
											*mpin->net()->sink_pins().begin();

		TNode* port_node = create_node(mpin->is_sink() ? TNode::OUT_PORT  : TNode::IN_PORT,  mpin);
		TNode* pad_node	 = create_node(mpin->is_sink() ? TNode::CELL_OPIN : TNode::CELL_IPIN, pad_pin);

		if(mpin->is_source()) port_node->create_out_edge(pad_node);
	}

	void TEngine::create_composite_inst_pin_node(Pin* pin)
	{
		if(!pin->is_connected()) return;

		TNode* node = create_node(pin->is_sink() ? TNode::CELL_IPIN : TNode::CELL_OPIN, pin);
	...
	void TEngine::create_primitive_inst_pin_node(Pin* pin)
	{
		...
		if(pin->is_sink()){
			...
			if(!pin->instance()->has_input()){
				TNode* const_gen_src = _tnodes.add(
					TFactory::instance().make_tnode(TNode::CONST_GEN_SRC, pin, num_tnodes())
				);
				const_gen_src->create_out_edge(opin_node, TPara::T_CONST_GEN, TPara::T_CONST_GEN);
			}
		}
	}
	...
	void TEngine::create_primitive_inst_conns(Instance* inst)
	{
		...
		if(seq_src && seq_sink){
			filtered_foreach2 (Pin*, ipin, inst->pins(), is_sink(), is_connected()) {
				delay = find_primitive_inst_delay(ipin, &TimingInfo::timing_type, SETUP_RISING);
				ipin->property_value(TNODE)->create_out_edge(seq_sink, delay._rising, delay._falling);
			}
		}
		...
		filtered_foreach2 (Pin*, opin, inst->pins(), is_source(), is_connected()) {
			...
			if(seq_src && seq_sink){
				delay = find_primitive_inst_delay(opin, &TimingInfo::timing_type, RISING_EDGE);
				seq_src->create_out_edge(opin->property_value(TNODE), delay._rising, delay._falling);
			}
			else{
				filtered_foreach2 (Pin*, ipin, inst->pins(), is_sink(), is_connected()) {
					delay = find_primitive_inst_delay(opin, &TimingInfo::related_pin, ipin->name());
					ipin->property_value(TNODE)->create_out_edge(opin_node, delay._rising, delay._falling);
				}
			}
	...
	TData TEngine::find_primitive_inst_delay(Pin* rel_pin, string TimingInfo::* const tp, const string& t_value)
	{
		double r_delay = TPara::ZERO, f_delay = TPara::ZERO;
		auto timings = static_cast<TPort*>(rel_pin->port())->timings();
		auto it = find_if(timings, [tp,&t_value](const TimingInfo& tinfo) { return tinfo.*tp == t_value; });
		if(it != timings.end()) {
	...
	void TEngine::topo_sort()
	{
		...
		for (TNode* node : tnodes()) node->clr_visited();
		for (TNode* node : tnodes()) topo_visit(node);
	}
	...
	int TEngine::t_domain_index(const TNode* node) const
	{ return _t_domains - 1; }

	TEngine::IntTDataMap TEngine::t_domain_index_map(const TNode* node) const
	{ IntTDataMap m; m.insert(std::make_pair(_t_domains - 1, chief_max_delay())); return m; } 
	...
	void TEngine::update_slack_to_pin(TEdge* tedge) const
	{
		...
		for (const TData& src_tarr : tedge->from_node()->t_arrs()){
			slack_it->_rising  = sink_treq_it->_rising  - src_tarr._rising  - tedge->r_delay();
			slack_it->_falling = sink_treq_it->_falling - src_tarr._falling - tedge->f_delay();
			++slack_it; ++sink_treq_it;
		}
...
// create_xml
//	DOM::Element* TNode::create_xml(DOM::Element* root) const { return nullptr; }
//	DOM::Element* TEdge::create_xml(DOM::Element* root) const { return nullptr; }

// echo_dotfile
//	void TNode::echo_dotfile(const TNode& from_node, std::ofstream& ofs) const {}
//	void TEdge::echo_dotfile(const TNode& from_node, std::ofstream& ofs) const {}
