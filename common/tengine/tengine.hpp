...
namespace COS {
	...
	// without ownership
	template<typename T, typename L = Less<T> > using SharedTESet = PtrSet<T, L, false>;

	class Pin; class Port; class Instance; class Module; class TDesign;
	class TNode; class TEdge;
	...
	struct TData {
		...
		void   set_value(double rising, double falling) { _rising = rising; _falling = falling; }
	};
...
//		virtual DOM::Element* create_xml(DOM::Element* root)						 const = 0;
//		virtual void		echo_dotfile(const TNode& from_node, std::ofstream& ofs) const = 0;
	};
	...
	public:
		...
		bool	less_than(const TEdge& e) const { 
			return this->average_delay() != e.average_delay() ?
				this->average_delay() <  e.average_delay() : TElement::less_than(e); 
		}
...
//		virtual void		echo_dotfile(const TNode& from_node, std::ofstream& ofs) const;
	...
	public:
		enum TNodeType {
			...
			IN_PORT,   /*IN_PAD,*/
			OUT_PORT,  /*OUT_PAD,*/
			CELL_IPIN, CELL_OPIN, CELL_CLKPIN,
			SEQ_SRC,   SEQ_SINK,
			CONST_GEN_SRC,
			NUM_TNODETYPE
		};
		...
		tdata_type		  t_arrs()		 { return tdata_type(_t_arr_vec.begin(), _t_arr_vec.end()); }
		const_tdata_type  t_arrs() const { return const_tdata_type(_t_arr_vec.begin(), _t_arr_vec.end()); }
		tdata_type		  t_reqs()		 { return tdata_type(_t_req_vec.begin(), _t_req_vec.end()); }
		const_tdata_type  t_reqs() const { return const_tdata_type(_t_req_vec.begin(), _t_req_vec.end()); }
		int			   num_tdata() const { return _t_arr_vec.size(); }
		TData	  t_arr(int t_idx) const { return _t_arr_vec[t_idx]; }
		void   resize_tdata(int i)       { _t_arr_vec.resize(i, TData(TPara::MINIMUM_FLOAT, TPara::MINIMUM_FLOAT)); 
		...
		TEdge*	  create_out_edge(TNode* t_tnode, double r_delay = TPara::ZERO, double f_delay = TPara::ZERO);
...
//		virtual void		echo_dotfile(const TNode& from_node, std::ofstream& ofs) const;
	...
	public:
		...
		virtual TEdge*	 make_tedge(TNode* f_tnode, TNode* t_tnode, double r_delay, double f_delay);
	...
	public:
		...
		explicit TEngine(TDesign* t = nullptr) : _t_domains(TPara::DEFAULT_TDOMAINS), _target(t) 
		{}
		virtual ~TEngine() {}
		...
		tdata_type		  max_delays()       { return tdata_type(_max_delay_vec.begin(), _max_delay_vec.end()); }
		const_tdata_type  max_delays() const { return const_tdata_type(_max_delay_vec.begin(), _max_delay_vec.end()); }
	...
	protected:
		...
		const_sorted_tnodes_type sorted_tnodes() const { return _sorted_tnodes.range(); }
		...
		TNode* create_node(TNode::TNodeType type, Pin* owner, bool set_pin_prop = true);
		...
		TData  find_primitive_inst_delay(Pin* rel_pin, string TimingInfo::* const tp, const string& t_value);
		...
		virtual IntTDataMap t_domain_index_map(const TNode* node) const; // only for primary output
...
//  TimingInfo
	struct TimingInfo
	{
		...
		TimingInfo(const string& type, const string& pin, const string& sense, double rise, double fall) :
			timing_type(type), related_pin(pin), timing_sense(sense), intrinsic_rise(rise), intrinsic_fall(fall) {}
	};
