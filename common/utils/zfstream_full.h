...
public:
	zofstream(const char* f) : outfile(f, std::ios_base::out | std::ios_base::binary | std::ios_base::trunc), std::ostream(&outbuf) {
...
class zifstream : public std::istream {
	//std::ios_base::open_mode bin_in = std::ios_base::in | std::ios_base::binary;
	io::file_descriptor_source infile;
	io::filtering_istreambuf inbuf;
	rand_cipher decoder;
	io::zlib_decompressor decompressor;
public:
	zifstream(const char* f) : infile(f, std::ios_base::in | std::ios_base::binary), std::istream(&inbuf) {
