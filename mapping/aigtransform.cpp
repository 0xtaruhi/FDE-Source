...
struct IPin { Pin* pin; bool inv = false; };
...
void AigManager::makeAigInstance(Instance* inst) {
	...
	Instance* aigInst = inst->module()->create_instance(name, makeAigModule(inst->down_module()));
...
Module* AigManager::makeAigModule(Module* mod) {
	...
	for (Port* port : mod->ports()) {
		Port* aigPort = _workMod->create_port(port->name() + "_Aig", port->dir(), port->type());
		if (aigPort->dir() == INPUT) vInputPorts.push_back(aigPort); //这里没考虑INOUT型的port，以及总线
		if (aigPort->dir() == OUTPUT) outputPort = aigPort;
	}

	vector<IPin> vMidPins;
	for (string cube : vCubes) {
		ASSERT(cube.size() == vInputPorts.size(), mod->name() + ": truthtable error");
		vector<IPin> vInputPins;
		for (size_t i = 0; i < cube.size(); i++) {
			switch (cube[i]) {
				case '1': vInputPins.push_back({vInputPorts[i]->mpin(), false}); break;
				case '0': vInputPins.push_back({vInputPorts[i]->mpin(), true }); break;
			}
		}
...
//	mod->flatten();
	...
	for (Instance* inst : combInstance) inst->release();
}
