...
uint32_t pinSignature(Pin* pin) {
	...
	if (pin->property_exist(pSignature)) return pin->property_value(pSignature);
	signature <<= 1;
...
struct MapCut {
	...
	void addLeaf(Pin *pin) {
		for (Pin* p : leaves) if (pin == p) return;
...
Property<uint32_t>& pTruthtableItems = create_temp_property<uint32_t>(INSTANCE, "truthtable_items", 0, COPY);

uint32_t calcTruthtable(int n, vector<int> index0, vector<int> index1, uint32_t tt0, uint32_t tt1, uint32_t ttRoot) {
	vector<int> state(n);
	uint32_t items = 0;
	for (int st = 0; st < (1 << n); st++) {
		for (int index = 0; index < n; index++) state[index] = !!(st & (1 << index));
		...
		if (ttRoot & (1 << (result0 + result1 * 2))) items |= (1 << st);
	}
	return items;
}
...
{
	if (oneCount(signature) > lutSize) return;  // invalid

	for (Pin* leaf : cut0->leaves) addLeaf(leaf);
	for (Pin* leaf : cut1->leaves) addLeaf(leaf);
	if (leaves.size() > lutSize) return;        // invalid

	vector<int> index0(cut0->leaves.size(), -1);
	vector<int> index1(cut1->leaves.size(), -1);
	for (int i = 0; i < leaves.size(); i++) {
		for (int j = 0; j < cut0->leaves.size(); j++) if (cut0->leaves[j] == leaves[i]) index0[j] = i;
		for (int j = 0; j < cut1->leaves.size(); j++) if (cut1->leaves[j] == leaves[i]) index1[j] = i;
	}
	truthtable = calcTruthtable(leaves.size(), move(index0), move(index1), cut0->truthtable, cut1->truthtable, root->property_value(pTruthtableItems));
...
bool isDfsRoot(Instance* node) {
	if (node->module_type() != "AIG") return false;
...
void getNodeDfsRec(vector<Instance*>& nodes, Instance* node) {
	if (!node || node->module_type() != "AIG" || node->property_value(pVisited)) return;
...
vector<Instance*> getNodeDfs(Module* top) {
	...
	for (Instance* inst : top->instances())
		if (isDfsRoot(inst)) getNodeDfsRec(nodes, inst);
	pVisited.clear();
	return nodes;
}
...
void addCut(CutVec& allCuts, CutPtr cut) {
	...
	if (allCuts.size() <= maxCuts) return;
	auto worst = max_element(allCuts.rbegin(), allCuts.rend(), betterCut).base() - 1;
	allCuts.erase(worst);
}
...
bool isLutRoot(Instance* node) {
	if (!node || node->module_type() != "AIG") return false;
	Pin* opin = node->find_pin("Y");
	for (Pin* pin : opin->net()->sink_pins())
		if (pin->is_mpin() || pin->instance()->module_type() != "AIG") return true;
	return false;
}
...
vector<Instance*> getLutRoots(vector<Instance*>& dfsNodes) {
	...
	int max_level = 0;
	for (Instance* node : dfsNodes) if (isLutRoot(node)) {
	...
	for (int i = 0; i < lutRootNodes.size(); i++) {
		...
		for (Pin* leaf : node->property_value(pBestCut)->leaves) {
			...
			if (nodeA && (nodeA->module_type() == "AIG") && !nodeA->property_value(pVisited)) {
...
void setTruthtableProperty(Instance* inst, uint32_t truthtable) {
	static auto& pTruthtable  = COS::create_property<string>(COS::INSTANCE, "INIT");
	static auto& pTruthtable1 = COS::create_property<string>(COS::INSTANCE, "INIT_1");
	static auto& pTruthtable2 = COS::create_property<string>(COS::INSTANCE, "INIT_2");
...
void MappingManager::doMapCut() {
	...
	for (Instance* node : dfsNodes) enumNode(node, _args.lutSize);

	createLutModule(_pDesign, _args.lutSize);
	int index = 0;
	for (Instance* node : getLutRoots(dfsNodes)) makeLut(node, index++);
	for (Instance* node : dfsNodes) node->release();

	vector<Net*> netsToRemove;
	for (Net* net : _pDesign->top_module()->nets())
		if (net->num_pins() == 0) netsToRemove.push_back(net);
	for (Net* net : netsToRemove) net->release();
}
