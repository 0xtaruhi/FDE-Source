...
void addBuf(Design* pDesign) {
	...
	for (Port* port : top->ports()) {
		switch (port->dir()) {
			case INPUT : Pi .push_back(port); break;
			case OUTPUT: Po .push_back(port); break;
			case INOUT : Pio.push_back(port); break;
		}
	}
	...
	for (Port* pi : Pi) {
		...
		for (Pin* pin : pi->mpins()) {
			if (!pin->is_connected()) {//如果输入Port悬空
				...
				for (Pin* sink_pin : pin->net()->sink_pins()) {
					if (sink_pin->type() == CLOCK || sink_pin->name() == "CK" || sink_pin->name() == "CKN" || sink_pin->name() == "CLK") {
						isClk = true;
						break;
					}
				}
				if (isClk) {	// 添加clk_buf和clk_ibuf
					Instance* clk_buf  = top->create_instance("Buf-pad-" + pin->name(), CKBUF);
					Instance* clk_ibuf = top->create_instance("IBuf-clkpad-" + pin->name(), CKBUF);
...
void connect_or_merge(Pin* pin, Net* net) {
	if (!net) return;
	if (!pin->net()) pin->connect(net);
	else pin->net()->merge(net);
}
...
void createConst(Design* pDesign) {
	...
	for (Instance* inst : top->instances()) {
		if (inst->down_module() == VCC && inst != iVcc || inst->down_module() == YVCC) {
			connect_or_merge(iVcc->pin(0), inst->pin(0)->net());
			torelease.push_back(inst);
		}
		else if (inst->down_module() == GND && inst != iGnd || inst->down_module() == YGND) {
