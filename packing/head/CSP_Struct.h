...
namespace PACK {
	...
	// forward declaration
	class Query; class QueryType; class BConstraint;
	...
	public:
		 ...
		 *  every value in domain must satisfy the type constraint of the corresponding
		 *  rule %GObject.
		 */
		 ...
		 *
		 *  We determine the value of the %Variable one by one until all Variables are instantiated.
		 *  Every %Variable have their query order to demonstrate when they are instantiated. This
		 *  order is determined by the search strategy of the CSP solver.
		 *  \return The query order of this %Variable
		 */
		 ...
		 *  When instantiating a %Variable we will check if all the constraints are satisfied. 
		 *  \param c The corresponding BConstraint to add
		 */
		void add_constraint(BConstraint* c)      { constraints_.push_back(c); }

		/*! \brief add outgoing query
		 *
		 *  One is outgoing Query of the %Variable if the %Variable is the source %Variable
		 *  of the Query.
		 *  \param q The outgoing query to add
		 */
		void add_outgoing_query(Query* q)        { out_queries_.push_back(q); }

		/*! \brief add incoming query
		 *
		 *  One is incoming Query of the %Variable if the %Variable is the target %Variable
		 *  of the Query. This routine will set the type query of the %Variable too.
		 *  \param q The incoming query to add
		 */
		 ...
		 *
		 */
		typedef boost::iterator_range<std::vector<Query*>::const_iterator> queries_type;
		 ...
		 *
		 */
		typedef boost::iterator_range<std::vector<BConstraint*>::const_iterator> csts_type;
	...
	public:
		 ...
		 *
		 *  \attention We haven't used weight to optimize searching now, but it's worth
		 *  being implemented.
		 */
		 ...
		 */
		Variable* get_cause(Variable* var) const { return var == var1_ ? var2_ : var1_; }

		/*! \brief check if constraint appliable
		 *
		 *  The %BConstraint is only appliable when the two Variables are both instantiated.
		 *  \return If constraint appliable
		 */
		bool      appliable()              const;
 
		/*! \brief check if constraint satisfied
		 *
		 *  This can only be called when the %BConstraint is appliable. It's overrided by
		 *  various types of constraint.
		 *  \return If constraint satisfied
		 */
	 ...
	 *
	 *  This is a unary constraint. It represents the type constraint according the rule
	 *  GObject. For GNode, it will check type name, connected pin etc. For GArc, it will
	 *  check type name, source pin name, sink pins name etc.
	 */
	 ...
	 *
	 *  This is a binary constraint. The first Variable is always involved with a rule GNode, and
	 *  the second is involved with a rule GArc. It will check if the source of the  rule GArc 
	 *  instance is the instance of the rule GNode.
	 */
	 ...
	 *
	 *  This is a binary constraint. The first Variable is always involved with a rule GNode, and
	 *  the second is involved with a rule GArc. It will check if the target of the  rule GArc 
	 *  instance is the instance of the rule GNode.
	 */
	 ...
	 *
	 *  This is a binary constraint. It represent the common net constraint between the two
	 *  GNode instance. Both two Variables are involved with a rule GNode and it will check if 
	 *  the two image PKInstances are satisfied to the common net constraint described by
	 *  the two rule GNode.
	 *  \warning This constraint is still not tested. By now no rule will produce such constraint.
	 */
	...
	/*! \brief class %Query
	 *
	 *  %Query is used to query a domain of a Variable that satisfied specific constraint.
	 *  That means every %Query involved with a BConstraint. When doing query, we should first
	 *  instantiate the source Variable, then we find out which instances satisfy the 
	 *  specific constraint with the source instance. For example I have a instance of a 
	 *  Variable(source Variable) involved with a rule GNode and now I want to know what's 
	 *  outgoing GArc can I get, so I use QueryOutgoing involved with ConstraintSource to get 
	 *  the domain of the target Variable involved with a rule GArc. 
	 */
	class Query	{
	public:
		/*! \brief type of %Query
		 *
		 *  According to different types of constraint we general have 5 types of query.
		 *  Maybe you should ask why there's no QUERY_COM_NET, indeed it can be here but
		 *  I consider the query is waste of time and I just ignore it. Such ignorer is OK
		 *  because all these 5 types of query are enough to query all domains of Variables.
		 *  Still I preserve the common net constraint for checking, the match we found will
		 *  satisfy the rule.
		 */
	...
	public:
		 ...
		 *
		 *  This getter is only used by QueryType when calling QueryType::query_gobjects()
		 *  \return The type of target Variable
		 */
		 ...
		 *
		 *  The query is only appliable when the source Variable is instantiated. QueryType
		 *  is always appliable because it have no source Variable.
		 *  \return If the query appliable
		 */
		bool          appliable() const;
		 ...
		 *
		 */
		void activate()   { if (correspond_) correspond_->activate();   }

		/*! \brief deactivate involved constraint
		 *
		 *  When we finish a query we always can deactivate the involved constraint for saving
		 *  cpu time, because the constraint is always satisfied.
		 */
		void deactivate() { if (correspond_) correspond_->deactivate(); }
		 ...
		 *
		 *  This routine is used to predict the size of the queried domain. It's useful for 
		 *  constructing search strategy. Always perform small size query is preferred.
		 *  \return The predict size of the query result
		 */
		virtual int                 size()    = 0;
	...
	public:
		 ...
		 *
		 *  This routine is called by size predictor of QueryIncoming. It calculates the
		 *  average number of incoming arcs. It only valid for the target Variable involved
		 *  with a rule GNode.
		 */
		 ...
		 *
		 *  This routine is called by size predictor of QueryOutgoing. It calculates the
		 *  average number of outgoing arcs. It only valid for the target Variable involved
		 *  with a rule GNode.
		 */
		 ...
		 *
		 *  %QueryType is slightly different from other queries. Because %QueryType not only
		 *  do the type name check but also some properties check I move all these actions
		 *  into this routine, it can do it once and get the result everywhere by calling 
		 *  QueryType::execute(). In fact QueryType::execute() is only a result getter.
		 */
	 ...
	 *
	 *  This %QueryIncoming is used to query all incoming arcs of a Variable involved with a rule GNode.
	 *  The source Variable is always involved with a rule GNode and the target one with a rule GArc.
	 */
	 ...
	 *
	 *  This %QueryOutgoing is used to query all outgoing arcs of a Variable involved with a rule GNode.
	 *  The source Variable is always involved with a rule GNode and the target one with a rule GArc.
	 */
	 ...
	 *
	 *  This %QuerySource is used to query the source GNode of a Variable involved with a rule GArc.
	 *  The source Variable is always involved with a rule GArc and the target one with a rule GNode.
	 */
	 ...
	 *
	 *  This %QueryTarget is used to query the target GNode of a Variable involved with a rule GArc.
	 *  The source Variable is always involved with a rule GArc and the target one with a rule GNode.
	 */
