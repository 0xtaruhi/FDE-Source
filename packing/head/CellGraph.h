...
namespace PACK {
	...
	protected:
		 ...
		 *
		 *  Each rule within layer is exclusive. That means when a GObject matches one rule
		 *  it will not match another rule within the same layer. Layer_lock_ will be updated
		 *  after each layer finished matching.
		 */
		bool     layer_lock_;
	};
	...
	// forward declaration
	class GNode; class GArc; class CellGraph;
	...
	private:
		 ...
		 *
		 *  I use scoped_ptr for auto memory management, release myself from memory hell.
		 *  You can see my boost memory management tricks in various place.
		 */
	...
	public:
		 ...
		 *
		 *  Boost iterator_range typedef of GArc, used to iterate incoming and outgoing
		 *  arcs.
		 */
		typedef boost::iterator_range<std::list<GArc*>::const_iterator> const_arcs_type;
		...
		/*! \brief suicide
		 *
		 *  Release corresponding resources like incoming & outgoing arcs, type collections
		 *  within CellGraph etc.
		 */
		void release();
		 ...
		 */
		typedef boost::iterator_range<std::vector<cstr_ptr>::const_iterator> const_pstr_type;
		...
		{ return const_pstr_type(node_type_->connect_pins_.begin(), node_type_->connect_pins_.end()); }
		 ...
		 *
		 *  I use this structure to note down all the common net pins between two Instances.
		 *  It's only used by rule %GNode.
		 */
		 ...
		 *
		 *  I use this map to find out any common net information corresponding to this %GNode.
		 *  It's only used by rule %GNode.
		 */
		struct CommonNet {
			GNode* target;
		 ...
		 */
		void              add_connect_pin(cstr_ptr cpstr) { node_type_->connect_pins_.push_back(cpstr); }
	...
	private:
		/*! \brief type information of %GNode
		 *
		 *  It's a counterpart of GObject::obj_type_, I use it to prevent boring static_cast
		 */
		 ...
		 *
		 *  It's only significant for rule %GNode.
		 *  \warning The correctness of this structure's creation is not tested! By now
		 *  no rule requires common net constraint.
		 */
	...
	public:
		...
		/*! \brief suicide
		 *
		 *  Release corresponding resource like incoming/outgoing arc of its target/source
		 *  GNode, corresponding PKNet, type collector of CellGraph etc.
		 */
		void release();
		 ...
		 *
		 */
		typedef boost::iterator_range<std::list<cstr_ptr>::const_iterator> const_pstr_type;
		 ...
		 */
		GArcType::SinkNumType sink_num_type()         const { return arc_type_->sink_num_type_;             }
		 ...
		 */
		int                   num_pure_sink_of_net()  const { return arc_type_->ori_net_->num_pure_sinks(); }
		...
		{ return const_pstr_type(arc_type_->sink_names_.begin(), arc_type_->sink_names_.end()); }

		/*! \brief constraint type of sink number setter
		 *
		 *  \param[in] t Constraint type of sink number
		 */
		void set_sink_num_type(GArcType::SinkNumType t) { arc_type_->sink_num_type_ = t;           }
		
		/*! \brief remove a sink on this %GArc
		 *
		 *  It's used to update CellGraph dynamically. This should only be called by CellGraph
		 *  in theory, maybe you should use friend to guarantee this protocol.
		 *  \param[in] cpstr Sink pin name that removed
		 */
		void remove_sink_pname(cstr_ptr cpstr)          { arc_type_->sink_names_.remove(cpstr);    }
		
		/*! \brief add a sink on this %GArc
		 *
		 *  It's used to update CellGraph dynamically. This should only be called by CellGraph
		 *  in theory, maybe you should use friend to guarantee this protocol.
		 *  \param[in] cpstr Sink pin name that added
		 */
		void add_sink_pname(cstr_ptr cpstr)             { arc_type_->sink_names_.push_back(cpstr); }
	...
	public:
		 ...
		 */
		CellGraph(PKCell* cell) 
			: context_(cell), output_graph_idx_(0) {}
		 ...
		 *  converted to a GNode, each PKNet in PKCell will be converted to a set of GArc.
		 *  
		 */
		void      build_graph();
		 ...
		 *
		 *  This routing will fill the CellGraph::type_map_ which is the type lookup map for
		 *  GObject.
		 *  \deprecated This routine is substituted by type insertion in CellGraph::create_gnode()
		 *  and CellGraph::create_garc() routines.
		 */
		void      fill_type_map();
		 ...
		 *
		 *  I use ptr_set because of the demand for fast GObject insertion and deletion.
		 */
	...
	private:
		 ...
		 *
		 *  This routine will fill the information specific for rule GNode and rule GArc. 
		 */
		 ...
		 *  like add it to CellGraph::gobjects_, insert it to type map etc. The parameters
		 *  is the same as the constructor of GNode.
		 */
		 ...
		 *  like add it to CellGraph::gobjects_, insert it to type map etc. The parameters
		 *  is the same as the constructor of GArc.
		 */
		inline GArc*  create_garc(CellGraph*, GNode*, GNode*, PKNet*);

		/*! \brief slot correspond to cell change signal
		 *
		 *  This will be called after an Instance created. It will create a corresponding GNode.
		 *  \param[in] inst Instance that created
		 */
		void after_inst_created(PKInstance* inst);

		/*! \brief slot correspond to cell change signal
		 *
		 *  This will be called before pin unhooked. It will update the corresponding GArc.
		 *  \param[in] net Net that unhooked
		 *  \param[in] pin Pin that unhooked
		 */
		void before_pin_unhooked(PKNet* net, PKPin* pin);

		/*! \brief slot correspond to cell change signal
		 *
		 *  This will be called before pin hookuped. It will update the corresponding GArc.
		 *  \param[in] net Net that hookuped
		 *  \param[in] pin Pin that hookuped
		 */
		void before_pin_hookuped(PKNet* net, PKPin* pin);

		/*! \brief slot correspond to cell change signal
		 *
		 *  This will be called before pin rehook. It will update the corresponding GArc.
		 *  \param[in] old_net Net that unhooked
		 *  \param[in] new_net Net that hookuped
		 *  \param[in] pin     Pin that rehook
		 */
		void before_pin_rehook(PKNet* old_net, PKNet* new_net, PKPin* pin);

		/*! \brief slot correspond to cell change signal
		 *
		 *  This will be called before net removed. It will delete the corresponding GArc.
		 *  \param[in] net Net that removed
		 */
		void before_net_remove(PKNet* net);

		/*! \brief slot correspond to cell change signal
		 *
		 *  This will be called before instance removed. It will delete the corresponding GNode.
		 *  \param[in] inst Instance that removed
		 */
	...
	inline GNode* CellGraph::create_gnode(CellGraph* context, const string& type, PKInstance* o) {
	...
	inline GArc* CellGraph::create_garc(CellGraph* context, GNode* source_, GNode* target_, PKNet* o) {
