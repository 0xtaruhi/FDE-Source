...
namespace PACK {
	...
	public:
		...
		static void parse_name_info(NameInfo& name_i, const string& name, const string& key_word) {
			name_i.name = name;
			if (name == key_word) name_i.is_kw = true;
			else                  name_i.is_fake = is_fake_name(name);
		}
	...
	class ICreateInst : public Instruct {
		friend class InstructParser;
	public:
		ICreateInst(TranTestCase* c)
			: Instruct(c), model_(NULL) {}

		c_str_ptr name()    const { return INSTR_NAME; }
		Object*   execute() const;
	...
	private:
		...
		NameInfo cname_i_;
		Module*    model_;//from which the correspond inst is created
	...
	private:
		static void parse(const string&, TranTestCase*);
		Object* copy_init(Instance* src_inst, Instance* tar_inst, const string& sinit) const;
