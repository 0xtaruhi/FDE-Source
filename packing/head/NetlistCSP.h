...
namespace PACK {
	...
	private:
		...
		struct VarSizeLess {
			bool operator() (const Variable* lhs, const Variable* rhs) const {
				if (lhs->type_query()->size() < rhs->type_query()->size()) return true;
				else if (lhs->type_query()->size() > rhs->type_query()->size()) return false;
		...
		struct VarQOrderLess {
			bool operator() (const Variable* lhs, const Variable* rhs) const {
				if (lhs->query_order() < rhs->query_order())      return true;
				else if (lhs->query_order() > rhs->query_order()) return false;
				return false;	// lhs < rhs;
			}
		};
	...
	private:
		...
		void   remove_injection(Variable* var) {
			std::map<GObject*, Variable*>::iterator fit = injection_map_.find(var->instance());
	...
	private:
		...
		template<typename Q> inline Q* create_query(Variable*, Variable*, BConstraint*);
	...
	template<typename C>
	inline C* NetlistCSP::create_constraint(Variable* v) 
	{ C* c = new C(v); constraints_.push_back(c); return c; }

	template<typename C>
	inline C* NetlistCSP::create_constraint(Variable* v1, Variable* v2)
	{ C* c = new C(v1, v2); constraints_.push_back(c); return c; }

	template<typename Q>
	inline Q* NetlistCSP::create_query(Variable* v, BConstraint* c)
	{ Q* q = new Q(v, c); queries_.push_back(q); return q; }

	template<typename Q>
	inline Q* NetlistCSP::create_query(Variable* v1, Variable* v2, BConstraint* c)
	{ Q* q = new Q(v1, v2, c); queries_.push_back(q); return q; }
}
