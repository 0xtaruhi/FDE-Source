...
//		enum CFG_TYPE { SLICE, IOB, GCLKIOB, GCLK, BLOCKRAM, TBUF, BUFGMUX, DLL, NUM_CFGS };
	}
	...
	protected:
		...
		bool has_inner_;
	public:
		PKConfig(const string& inst_type, const string& name,
				 const string& xdl_name, bool has_inner = false,
				 const string& def_val = CFG_NAME::DEF_VALUE);
	...
	public:
		...
		struct value_type {		// for supporting boost assign
			cfg_type type_;
			string name_;
			value_type(const char* name,   cfg_type type = NORMAL) : type_(type), name_(name) {}
			value_type(const string& name, cfg_type type = NORMAL) : type_(type), name_(name) {}
		};
		typedef COS::PtrVector<PKConfig> configs_type;
		const configs_type& get_configs(const string& inst_type) { return config_map_[inst_type]; }

		//void init();
		void insert(const value_type& val);
		void load(const string& fg);	//added by hz
		string using_type(const string& inst_type) { return cur_type_ = inst_type; } //modified by hz
