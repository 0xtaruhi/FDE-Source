...
namespace PACK {
	...
	class PKInstance; class PKNet; class PKPin;
	class GNode; class CellGraph;
	class Rule;
	class VCell; class VPort; class VPin;

	class GNode; class GArc;
	...
	public:
		PKPort(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from);
	...
	struct SimpleNet {
		...
		bool operator == (const SimpleNet& rhs) const { return src == rhs.src && inst == rhs.inst; }
	};
	...
	public:	
		...
		const_pins_type pins()        const { return static_cast<const_pins_type>(Net::pins()); }
		pins_type       source_pins()       { return static_cast<pins_type>(Net::source_pins());}
		const_pins_type source_pins() const { return static_cast<const_pins_type>(Net::source_pins()); }
		pins_type       sink_pins()         { return static_cast<pins_type>(Net::sink_pins());       }
		const_pins_type sink_pins()   const { return static_cast<const_pins_type>(Net::sink_pins()); }
		PKPin* find_pin(const string& name) { return static_cast<PKPin*>(Net::find_pin(name)); }
		...
		// VPin
		typedef boost::iterator_range<std::list<VPin*>::const_iterator> const_vpins_type;
		typedef boost::iterator_range<std::list<VPin*>::iterator>       vpins_type;

		int              num_vpins() const  { return vpins_.size();                                  }
		const_vpins_type vpins()     const  { return const_vpins_type(vpins_.begin(), vpins_.end()); }
		vpins_type       vpins()            { return vpins_type(vpins_.begin(), vpins_.end());       }

		// Unique VCell
		typedef boost::iterator_range<unique_list<VCell*>::const_iterator> const_vcells_type;
	...
	public:
		...
		const_pins_type pins() const { return static_cast<const_pins_type>(Instance::pins()); }
		pins_type       pins()       { return static_cast<pins_type>(Instance::pins());       }
		PKPin* find_pin(const string& name) { return static_cast<PKPin*>(Instance::find_pin(name)); }
	...
	public:
		...
		typedef PtrList<Instance>::typed<PKInstance>::const_range    const_instances_type;
		...
		instances_type       instances()       { return static_cast<instances_type>(Module::instances()); }
		const_instances_type instances() const { return static_cast<const_instances_type>(Module::instances()); }
		PKInstance* find_instance(const string& name) { return static_cast<PKInstance*>(Module::find_instance(name)); }
		...
		const_nets_type nets() const { return static_cast<const_nets_type>(Module::nets()); }
		nets_type       nets()       { return static_cast<nets_type>(Module::nets());       }
		PKNet* find_net(const string& name) { return static_cast<PKNet*>(Module::find_net(name)); }
		void            parse_nets();
	...
	inline PKInstance* PKPin::instance() const { return static_cast<PKInstance*>(Pin::instance()); }
	...
	public:
		...
		Instance* make_instance(const string& name, Module* down_module, Module* owner);
		Pin*      make_pin(const string& name, Port* port, Module* owner, int index, int mindex);
		Pin*      make_pin(Pin* mpin, Instance* owner);
		Port*     make_port(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from);
	};

	class RulePort : public Port {
	public:
		RulePort(const string& name, int msb, int lsb, DirType dir, PortType type, bool is_vec, Module* owner, int pin_from);
	...
	public:
		...
		typedef PtrList<Instance>::typed<RuleInstance>::const_range    const_instances_type;
		typedef PtrList<Instance>::typed<RuleInstance>::iterator       inst_iter;
		typedef PtrList<Instance>::typed<RuleInstance>::const_iterator const_inst_iter;

		instances_type       instances()       { return static_cast<instances_type>(Module::instances()); }
		const_instances_type instances() const { return static_cast<const_instances_type>(Module::instances()); }
		RuleInstance* find_instance(const string& name) { return static_cast<RuleInstance*>(Module::find_instance(name)); }
		...
		const_ports_type ports() const { return static_cast<const_ports_type>(Module::ports()); }
		RulePort* find_port(const string& name) { return static_cast<RulePort*>(Module::find_port(name)); }
