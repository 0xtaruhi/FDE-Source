...
namespace PACK {
	...
	public:
		...
		template<class T>
		MsgFormat& operator% (const T& parm) { formator_ % parm; return *this; }
        //conversion operator:convert MsgFormat to a string
		const std::string& to_string() const { formatted_ = msg_head_ + formator_.str(); return formatted_; }
		operator ccstr() const { return to_string().c_str(); }

	protected:
		boost::format formator_;//cannot be accessed by the user of MsgFormat, thus we overload %
	private:
		string msg_head_;
		mutable string formatted_;
	};
	////////////////////////sophie/////////////////////////////////////
	inline std::ostream& operator <<(std::ostream& os, const MsgFormat& msg_fmt){ os << msg_fmt.to_string(); return os; }
	//inline std::ostream& operator<< (std::ostream& out, MsgFormat& err_msg) { out << std::string(err_msg).c_str(); return out; }
	...
	public:
		...
		POErrMsg(POErr emsg_type, const string& msg_head = "[OP ERROR]: ") 
	...
	public:
		...
		TFErrMsg(TFErr emsg_type, const string& msg_head = "[TF ERROR]: ")
	...
	const char* const info_msg[] = {
		...
		"Copyright (c) 2006-2011 FPGA-SoftwareGP.MEKeylab.FDU   All rights reserved.",	//INFO_CPY_RGHT
		...
		"Progress   15%%: begin to map the netlist to hardware primittives...\n",          //INFO_MAP2HW
	...
	public:
		...
		InfoMsg(InfoMsgType msg_type, const string& msg_head = "")
