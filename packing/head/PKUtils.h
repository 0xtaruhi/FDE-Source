...
namespace PACK {
	...
	public:
		...
		typename std::list<T>::iterator       find(const T& v)       { return std::find(std::list<T>::begin(), std::list<T>::end(), v); }
		typename std::list<T>::const_iterator find(const T& v) const { return std::find(std::list<T>::begin(), std::list<T>::end(), v); }

		using base::insert;
		using base::erase;
		typename std::list<T>::iterator insert(const T& v) {
			typename std::list<T>::iterator p = locate(v);
			if (p != std::list<T>::end() && v == *p) return p;
			return insert(p, v);
		}
		...
		void erase(const T& v) {
			typename std::list<T>::iterator p = locate(v);
			if (p != std::list<T>::end() && v == *p) erase(p);
		}
	};
	...
	public:
		...
		Enumeration(IT begin, IT end) 
			: begin_(begin), end_(end), present_(begin) {}
		...
		{ begin_ = e.begin_; end_ = e.end_; present_ = e.present_; return *this; }
	...
	template<typename T, typename C> bool is_in(const T& elem, const C& container)
