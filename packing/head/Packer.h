...
namespace PACK {
	...
	public:
		...
		void load_rule_lib(const string& fn, Design* d)
		{ prule_lib_.load(fn, d); }
		...
		struct VCellDegGreater {
			bool operator() (const VCell* lhs, const VCell* rhs) const {
				...
				else if (lhs->degree() < rhs->degree()) return false;

				if      (lhs->conn_factor() < rhs->conn_factor()) return true;
				else if (lhs->conn_factor() > rhs->conn_factor()) return false;
		...
		struct VCellGainGreater {
			bool operator() (const VCell* lhs, const VCell* rhs) const {
				if      (lhs->total_gain() > rhs->total_gain()) return true;
				else if (lhs->total_gain() < rhs->total_gain()) return false;
