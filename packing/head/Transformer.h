...
namespace PACK {
	...
	class TranOperation; class Transformer;
	...
	template<typename I>
	inline I* TranTestCase::create_instruct()
	{ I* pi = new I(this); instructs_.add(pi); return pi; }

	template<typename T>
	inline T* TranTestCase::create_test_expr(bool is_invert)
	{ T* pt = new T(this, is_invert); test_expr_.reset(pt); return pt; }
	...
	public:
		//constructor
		Transformer()
			: cur_match_(NULL), cur_vcell_(NULL), cur_op_(NULL), 
			  top_cell_(NULL), slice_cell_(NULL), slice_inst_(NULL),
			  slice_full_flag_(false),
		...
		// for safety, maybe you should concatenate the up-cell name to the instance name
		...
		void insert_poss_dangling_net(Net* net)
