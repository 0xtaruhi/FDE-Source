...
namespace PACK {
	...
	{ 
		...
		if (q->qtype() == Query::QUERY_TYPE) type_query_ = static_cast<QueryType*>(q); 
	}
	...
	BConstraint::BConstraint(Variable* var1, Variable* var2, int weight, ConstraintEnum type)
	...
	Query::Query(Variable* src, Variable* tar, int weight, BConstraint* c, QueryEnum e)
		: source_(src), target_(tar), weight_(weight), correspond_(c), qtype_(e) 
	{ src->add_outgoing_query(this); tar->add_incoming_query(this); }

	QueryType::QueryType(Variable* var, BConstraint* c)
		: Query(var, 0, c, Query::QUERY_TYPE),
		  average_in_(-1), average_out_(-1) {}
	...
	bool ConstraintType::execute()
	{
		...
		if (rule_obj->type() != inst_obj->type()) return false;
		...
		if (rule_obj->kind() == GObjType::GNODE) {
			...
			for (cstr_ptr cpstr: rule_node->connect_pin()) {
				...
				if (pin == NULL || pin->net() == NULL) return false;
			}
		}
		else {
			...
			for (cstr_ptr rule_pin_name: rule_arc->sink_pin_names()) {
				...
				for (cstr_ptr inst_pin_name: inst_arc->sink_pin_names())
					if (*rule_pin_name == *inst_pin_name) { found = true; break; }
	...
	{
		if (target_->instance() != NULL)                    return false;
		if (source_ != NULL && source_->instance() == NULL) return false;
		return true;
	}
	...
	int QueryType::average_in_degree() {
		if (query_result_.empty())                          return 0;
		if (target_->rule_gobj()->kind() == GObjType::GARC) return 0;
	...
	int QueryOutgoing::size() { return source_->type_query()->average_out_degree(); }
