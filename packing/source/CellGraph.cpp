...
namespace PACK {
	...
	GArcType::GArcType(const string& src_type, const string& tar_type, PKNet* o)
		: GObjType(str(format("%1%_ARC_%2%") % src_type % tar_type), GObjType::GARC),
	...
	{
		if (t == SRC_NAME) arc_type_->src_name_ = &name;
		else arc_type_->sink_names_.push_back(&name);
	}
	...
	void CellGraph::build_graph() {
		...
		for (PKInstance* inst: context_->instances()) {
			...
			if (context_->type() == KEY_WORD::RULE_CELL) {
				...
				gnode->set_rule_to_lock(rinst->rule_lock());//whether the rule_lock_ attribute of ruleInstance is true or false
			}
		}
		...
		for (PKNet* net: context_->nets()) {
			...
			for (const SimpleNet& snet: net->simple_nets()) {
				...
				if (src_pin->is_mpin()) continue;//nets connecting to cell pins are all neglected

				garc = create_garc(this, src_pin->instance()->gobject(), snet.inst->gobject(), net);
		...
		if (context_->type() == KEY_WORD::RULE_CELL) build_rule_cell();
		// draw();
	}
	...
	void CellGraph::build_rule_cell() {
		...
		for (RulePort* port: rule_cell->ports()) {		// todo: pins
			...
			if (port->dir() == COS::OUTPUT) {
				GArcType::SinkNumType sink_num_type = port->is_connect() ? GArcType::MORE : GArcType::NOT_LESS;
				...
				for (const SimpleNet& snet: net->simple_nets())	{
					PKPin* src_pin   = snet.src;
//					PKPin* asink_pin = snet.second[0];
					GNode* node      = snet.inst->gobject();

					// prevent multi sources
					if (src_pin != static_cast<PKPin*>(port->mpin())) continue;
					...
					// add connect pin for present node and add common net info for previous node
					for (PKPin* pin: snet.sinks) {
						if (port->is_connect())
			...
			for (PKPin* src_pin: net->source_pins()) {
				if (!src_pin->is_mpin()) {
					...
					for (PKPin* sink_pin: net->sink_pins())
						if (sink_pin->is_mpin() && static_cast<RulePort*>(sink_pin->port())->is_bus_ignore())
							outignore = true;
						else
							outignore = false; // if out-port connect to another internal pin, default ignore is invalid
					if(!outignore)
	...
	CellGraph::GObjList* CellGraph::find_gobjs(const string& n)
	{
		...
		if (it == type_map_.end()) return NULL;
	...
	{
		if (pin->is_mpin() || net == NULL) return;
	...
	void CellGraph::before_pin_hookuped(PKNet* net, PKPin* pin) {
		...
		if (pin->dir() == COS::OUTPUT && net->num_pure_sinks() != 0) {
			...
			for (PKPin* spin: net->sink_pins()) {
				if (spin->is_mpin()) continue;
				PKInstance* sinst = spin->instance();
				GArc* garc = cps_inst_map[sinst];
				if (!garc) {
					// build arc for new output pin
					garc = cps_inst_map[sinst] = create_garc(this, pin->instance()->gobject(), sinst->gobject(), net);
			...
			if (!inst_found) {
				for (PKPin* src_pin: net->source_pins()) {
					if (src_pin->is_mpin()) continue;

					GArc* garc = create_garc(this, src_pin->instance()->gobject(), pin->instance()->gobject(), net);
	...
	void CellGraph::draw()
	{
		...
		string cmd = str(format("dot -Tpng temp.dot -o cell_graph\\%1%%2%.png") % context_->name() % output_graph_idx_++);
