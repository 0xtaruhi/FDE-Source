...
namespace PACK {
	...
	static void inst_cfg(Instance* inst, ostream& xdl_file, ConfigInfo& config_info) {
		string inst_type = inst->module_type();

		xdl_file << "inst \"" << inst->name() << "\" \"" 
			<< inst_type << "\",unplaced bonded ,\n" 
			<< "\tcfg \" \n";
	...
	static void net_cfg(Net* all_net, ostream& xdl_file) {
		...
		for (Pin* pin: all_net->pins()) {
			if (pin->is_mpin() || pin->name() == "PAD") continue;
			if (pin->is_source())			
				xdl_file << "\n  outpin \"" << pin->instance()->name().c_str() << "\" " << pin->name() << " ,";
			else if (pin->is_sink())
				xdl_file << "\n  inpin \"" << pin->instance()->name().c_str() << "\" " << pin->name() << " ,";
		}
		xdl_file << "\n  ;" ;
	}
	...
	void Packer::write_xdl(Design* design, bool fencry)
	{
		...
		if (chip_type_==CHIP_TYPE_NAME::FDP3){		
			xdl_file << "design " << "\"" << design->name() << "\"" << " xc2s300epq208-6 v3.1;";
		}
		else if (chip_type_==CHIP_TYPE_NAME::FDP4){		
			xdl_file << "design " << "\"" << design->name() << "\"" << " xc2v80fg256-6 v3.1;";
		}
		else
		...
		for (Instance* inst: design->top_module()->instances()) {
			...
			if (inst->module_type() == CFG_NAME::IOB) {
				for (PKConfig* cfg: config_info_.get_configs(CFG_NAME::IOB)) {
					if (inst->property_exist(cfg->cos_cfg()) && inst->property_value(cfg->cos_cfg()) != CFG_NAME::DEF_VALUE) {
		...
		rpt_file <<"IOB number=" << cell_count[CFG_NAME::IOB] - cell_count[CFG_NAME::DANGLEIOB] << "\n";
		...
		if (cell_count[CFG_NAME::DANGLEIOB] > 0)
			FDU_LOG(DEBUG) << "TIPS: Design has " << cell_count[CFG_NAME::DANGLEIOB] << " DANGLE IOB!";
