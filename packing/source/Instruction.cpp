...
namespace PACK {
	...
	Property<string>& INIT = create_property<string>(INSTANCE, "INIT", std::string(), COS::COPY);
	Property<string>& INIT_HEX = create_property<string>(INSTANCE, "INIT_HEX", std::string(), COS::COPY);
	...
	static const char* const instr_types[] = { 
		ICreateInst::INSTR_NAME
	...
	// Note: this is function's param must be the old instance(Lut[1-4]) created by Synplify
	// NOT LUT created by our substitution!!
	// Default each lut's input pin is connected! 
	...
	void InstructParser::parse_instruct(const string& statement, TranTestCase* test_case)
	{
		StrVec svec;
	...
	void ICreateInst::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		ASSERT(pvec.size()==NUM_PARAM, 
			POErrMsg(POErrMsg::UEXP_ARG_NUM) % INSTR_NAME % NUM_PARAM % pvec.size());
        //regex the fake name
		...
		InstructParser::parse_name_info(new_instr->cname_i_, 
			pvec[1], KEY_WORD::SLICE_CELL);

        //fake name with"{}";except slice, iob, gclkiob and gclk,the model cell is in the hw_lib
		if (!new_instr->cname_i_.is_kw && !new_instr->cname_i_.is_fake){
			Library* lib = PKApp::instance().design().libs().find(pvec[2]);
			ASSERT(lib, POErrMsg(POErrMsg::UFND_ELEM) % "library" % pvec[2] % INSTR_NAME);

			new_instr->model_ = lib->find_module(pvec[1]);
			ASSERT(new_instr->model_, POErrMsg(POErrMsg::UFND_ELEM) % "cell" % pvec[1] % INSTR_NAME);
		}
	}
	...
	void ICreateIob::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		split_params(sparam, pvec);
		ASSERT(pvec.size()==NUM_PARAM, POErrMsg(POErrMsg::UEXP_ARG_NUM) % INSTR_NAME % NUM_PARAM %pvec.size());
	...
	void ICloneCell::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		InstructParser::parse_name_info(new_instr->model_cname_i_, 
			pvec[2], KEY_WORD::SLICE_CELL);

		if (!new_instr->model_cname_i_.is_kw && !new_instr->model_cname_i_.is_fake) {
			Library* lib = PKApp::instance().design().libs().find(pvec[3]);
			ASSERT(lib, POErrMsg(POErrMsg::UFND_ELEM) % "library" % pvec[3] % INSTR_NAME);

			new_instr->model_ = lib->find_module(pvec[2]);
			ASSERT(new_instr->model_, POErrMsg(POErrMsg::UFND_ELEM) % "cell" % pvec[2] % INSTR_NAME);
		}
	}
	...
	void IReconnect::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		ASSERT(avec.size()==2, POErrMsg(POErrMsg::UEXP_ARG_FMT) % pvec[0] % INSTR_NAME);

		InstructParser::parse_name_info(new_instr->old_iname_i_, 
			avec[0], KEY_WORD::SLICE_INST);
		new_instr->old_pname_ = avec[1];

		split_specific(pvec[1], avec, ".");
		ASSERT(avec.size()==2, POErrMsg(POErrMsg::UEXP_ARG_FMT) % pvec[1] % INSTR_NAME);

		InstructParser::parse_name_info(new_instr->new_iname_i_, 
			avec[0], KEY_WORD::SLICE_INST);
	...
	void IReconnectOpt::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		split_params(sparam, pvec);
		ASSERT(pvec.size()>=NUM_PARAM, 
			POErrMsg(POErrMsg::UEXP_ARG_NUM_MORE) % INSTR_NAME % NUM_PARAM % pvec.size());
		...
		for(int i=1; i<pvec.size(); ++i) {
			...
			ASSERT(avec.size()==2, POErrMsg(POErrMsg::UEXP_ARG_FMT) % pvec[i] % INSTR_NAME);

			new_instr->opt_inames_i_.push_back(NameInfo());
			InstructParser::parse_name_info(new_instr->opt_inames_i_.back(), 
				avec[0], KEY_WORD::SLICE_INST);
	...
	void IUnhook::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		InstructParser::parse_name_info(new_instr->iname_i_, avec[0], KEY_WORD::SLICE_INST);
	...
	void IDupConnect::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		InstructParser::parse_name_info(new_instr->src_iname_i_, avec[0], KEY_WORD::SLICE_INST);
		new_instr->src_pname_ = avec[1];

		split_specific(pvec[1], avec, ".");
		ASSERT(avec.size()==2, POErrMsg(POErrMsg::UEXP_ARG_FMT) % pvec[1] % INSTR_NAME);

		InstructParser::parse_name_info(new_instr->tar_iname_i_, avec[0], KEY_WORD::SLICE_INST);
	...
	void ISetProperty::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		InstructParser::parse_name_info(new_instr->iname_i_, pvec[0], KEY_WORD::SLICE_INST);
	...
	void ICloneProperty::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		InstructParser::parse_name_info(new_instr->src_iname_i_, pvec[0], KEY_WORD::SLICE_INST);
		InstructParser::parse_name_info(new_instr->tar_iname_i_, pvec[1], KEY_WORD::SLICE_INST);
	...
	void ICopyProperty::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		InstructParser::parse_name_info(new_instr->src_iname_i_, pvec[0], KEY_WORD::SLICE_INST);
		InstructParser::parse_name_info(new_instr->tar_iname_i_, pvec[2], KEY_WORD::SLICE_INST);
	...
	void IShareNet::parse(const string& sparam, TranTestCase* test_case)
	{
		...
		for(size_t i=0; i<pvec.size()/2; i++){
			...
			InstructParser::parse_name_info(sc.sn_iname1_, avec1[0], KEY_WORD::SLICE_INST);
			sc.sn_pname1_ = avec1[1];
			split_specific(sn[1], avec2, ".");
			InstructParser::parse_name_info(sc.sn_iname2_, avec2[0], KEY_WORD::SLICE_INST);
			...
			InstructParser::parse_name_info(sc.do_iname1_, todo1[0], KEY_WORD::SLICE_INST);
			sc.do_pname1_ = todo1[1];
			split_specific(todo[1], todo2, ".");
			InstructParser::parse_name_info(sc.do_iname2_, todo2[0], KEY_WORD::SLICE_INST);
	...
	Object* ICreateInst::execute() const
	{
		...
		if (model_ != NULL)//model in the hw_lib
			new_inst = tar_cell->create_instance(tf->make_name(name_prefix_, temp_suffix_), model_);
		else {
			Module* model = tf->find_cell(cname_i_);
			ASSERT(model, TFErrMsg(TFErrMsg::UFND_ELEM) % "cell" % cname_i_.name % INSTR_NAME);
			new_inst = tar_cell->create_instance(tf->make_name(name_prefix_, name_suffix_), model);
	...
	Object* ICreateIob::execute() const
	{
		...
		Instance::pin_iter iob_pin = find_if(buf_net->pins(), [](const Pin* pin) { return pin->is_mpin(); });
	...
	Object* ICreateNet::execute() const
	{
		...
		Net* new_net = tar_cell->create_net(tf->make_name(name_prefix_, name_suffix_));
	...
	Object* ICloneCell::execute() const
	{
		...
		Module* model;
		if (model_ != NULL) model = model_;
		else                model = tf->find_cell(model_cname_i_);

		ASSERT(model, TFErrMsg(TFErrMsg::UFND_ELEM) % "cell" % model_cname_i_.name % INSTR_NAME);

		Module* new_cell = model->clone(tf->make_name(name_prefix_, name_suffix_), cell_owner_);
	...
	Object* IReconnect::execute() const
	{
		...
		ASSERT(old_inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % old_iname_i_.name % INSTR_NAME);
		ASSERT(new_inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % new_iname_i_.name % INSTR_NAME);

		PKPin* old_pin = static_cast<PKPin*>(old_inst->find_pin(old_pname_));
		PKPin* new_pin = static_cast<PKPin*>(new_inst->find_pin(new_pname_));
		ASSERT(old_pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % old_pname_ % INSTR_NAME);
		ASSERT(new_pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % new_pname_ % INSTR_NAME);
		...
		if (new_pin->is_used()) return NULL;
		new_pin->set_used();

		Net* net = old_pin->net();
		if (net == NULL) return NULL;

		//ASSERT(net, TFErrMsg(TFErrMsg::USAT_PIN) % "unconnected" % old_pname_ % INSTR_NAME);
		ASSERT(!new_pin->net(), 
			TFErrMsg(TFErrMsg::USAT_PIN_COND) % new_pname_ % "unconnected" % INSTR_NAME);

		tf->insert_poss_dangling_inst(old_inst);
		// emit signal here!
		tar_cell->before_pin_unhook(static_cast<PKNet*>(net), static_cast<PKPin*>(old_pin));
		old_pin->disconnect();

		tar_cell->before_pin_hookup(static_cast<PKNet*>(net), static_cast<PKPin*>(new_pin));
		new_pin->connect(net);
	...
	Object* ICreateSlice::execute() const
	{
		...
		Instance* new_inst = NULL;
		if (model_ != NULL)//model in the hw_lib
			new_inst = tar_cell->create_instance(tf->make_name(name_prefix_, name_suffix_), model_);
		else {
			Module* model = tf->find_cell(cname_i_);
			ASSERT(model, TFErrMsg(TFErrMsg::UFND_ELEM) % "cell" % cname_i_.name % INSTR_NAME);
			new_inst = tar_cell->create_instance(tf->make_name(name_prefix_, name_suffix_), model);
	...
	Object* IConnect::execute() const
	{
		...
		string net_name = old_inst->name()+"_"+old_pin->name()+"_net_"+new_inst->name()+"_"+new_pin->name();
	...
	Object* IExconnect::execute() const
	{
		...
		tar_cell->before_pin_unhook(static_cast<PKNet*>(net1), static_cast<PKPin*>(old_pin));
		old_pin->disconnect();
		tar_cell->before_pin_unhook(static_cast<PKNet*>(net2), static_cast<PKPin*>(new_pin));
		new_pin->disconnect();

		tar_cell->before_pin_hookup(static_cast<PKNet*>(net1), static_cast<PKPin*>(new_pin));
		new_pin->connect(net1);
		tar_cell->before_pin_hookup(static_cast<PKNet*>(net2), static_cast<PKPin*>(old_pin));
		old_pin->connect(net2);

		if(old_inst == new_inst && new_inst->down_module()->name() == CELL_NAME::LUT){
	...
	Object* IReconnectOpt::execute() const
	{
		...
		for(int i=0; i<opt_inames_i_.size(); ++i) {
			...
			ASSERT(new_inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % opt_inames_i_[i].name % INSTR_NAME);
			PKPin* new_pin = static_cast<PKPin*>(new_inst->find_pin(opt_pnames_[i]));
			ASSERT(new_pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % opt_pnames_[i] % INSTR_NAME);

			if (!new_pin->is_used()) {
				ASSERT(!new_pin->net(), TFErrMsg(TFErrMsg::USAT_PIN_COND) % opt_pnames_[i] % "unconnected" % INSTR_NAME);
	...
	Object* IUnhook::execute() const
	{
		...
		ASSERT(inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % iname_i_.name % INSTR_NAME);
		...
		// emit signal here!
		tar_cell->before_pin_unhook(static_cast<PKNet*>(pin->net()), static_cast<PKPin*>(pin));
		pin->disconnect();
	...
	Object* IDupConnect::execute() const
	{
		...
		ASSERT(src_inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % src_iname_i_.name % INSTR_NAME);
		ASSERT(tar_inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % tar_iname_i_.name % INSTR_NAME);

		Pin* src_pin = src_inst->find_pin(src_pname_);
		Pin* tar_pin = tar_inst->find_pin(tar_pname_);
		ASSERT(src_pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % src_pname_ % INSTR_NAME);
		ASSERT(tar_pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % tar_pname_ % INSTR_NAME);
		...
		//ASSERT(net, TFErrMsg(TFErrMsg::USAT_PIN) % "unconnected" % src_pname_ % INSTR_NAME));
		ASSERT(!tar_pin->net(), TFErrMsg(TFErrMsg::USAT_PIN_COND) % tar_pname_ % "unconnected" % INSTR_NAME);

		// emit signal here!
		tar_cell->before_pin_hookup(
			static_cast<PKNet*>(net), 
	...
	Object* IUsePin::execute() const
	{
		...
		for (int i=0; i<opt_inames_i_.size(); ++i) {
			...
			ASSERT(inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % opt_inames_i_[i].name % INSTR_NAME);
			PKPin* pin = static_cast<PKPin*>(inst->find_pin(opt_pnames_[i]));
			ASSERT(pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % opt_pnames_[i] % INSTR_NAME);
	...
	static Property<string>& find_or_create_property(Instance* inst, const string& name)
	{
	...
	Object* ICloneProperty::execute() const
	{
		...
		// 450
		Property<string>* prop = dynamic_cast<Property<string>*>(src_inst->find_property(prop_name_));
		//修改此处的逻辑 prop，因为pro有值不代表是src_inst的pro，有可能是别的pro
	...
	Object* ICopyProperty::execute() const {
		...
		Property<string>& tar_prop = find_or_create_property(tar_inst, tar_prop_name_);
		string pval;
		if (src_prop_name_ == PROP_NV::NAME)
			pval = src_inst->name();
		else {
			Property<string>& sp = find_or_create_property(src_inst, src_prop_name_);
			if(!src_inst->property_exist(sp)) return NULL;
	...
	Object* ICopyProperty::copy_init(Instance* src_inst, Instance* tar_inst, const string& sinit) const {
	...
	Object* IShareNet::execute() const
	{
		...
		for (ShareCond sc: share_conds_) {
			// judge the given two pins are connected to the same net
			{
				...
				if(net1 == net2) sc.isconnect = true;
				else sc.isconnect = false;
			}
			// only do the process if the two pins' nets are different
			{
				if(!sc.isconnect){
					...
					if (pin1->is_used()) continue;
					pin1->set_used();
					ASSERT(!pin1->net(), TFErrMsg(TFErrMsg::USAT_PIN_COND) % sc.do_pname1_ % "unconnected" % INSTR_NAME);

					tf->insert_poss_dangling_inst(inst2);
					tar_cell->before_pin_unhook(static_cast<PKNet*>(net), static_cast<PKPin*>(pin2));
					pin2->disconnect();

					tar_cell->before_pin_hookup(static_cast<PKNet*>(net), static_cast<PKPin*>(pin1));
					pin1->connect(net);

					if(inst2 == inst1 && inst2->down_module()->name() == CELL_NAME::LUT){
						string table = inst2->property_value(INIT);
						inst2->set_property(INIT, sub_expr(table, sc.do_pname2_, sc.do_pname1_));
					}
				}
			}
	...
	Object* IReconnect2::execute() const
	{
		...
		ASSERT(tar_inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % old_iname_i_.name % INSTR_NAME);
		PKPin* tar_pin = static_cast<PKPin*>(tar_inst->find_pin(old_pname_));
		ASSERT(tar_pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % old_pname_ % INSTR_NAME);
		if (tar_pin->is_used()) return NULL;
		tar_pin->set_used();
		...
		for(size_t i=0; i<opt_inames_i_.size(); i++){
			...
			Net* net = src_pin->net();
			if(!net) continue;
			tar_cell->before_pin_unhook(static_cast<PKNet*>(net), static_cast<PKPin*>(src_pin));
			src_pin->disconnect();
			tar_cell->before_pin_hookup(static_cast<PKNet*>(net), static_cast<PKPin*>(tar_pin));
			tar_pin->connect(net);
			break;
		}
