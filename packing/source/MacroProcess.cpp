...
namespace PACK {
	...
	struct ChainCell {
		...
		ChainCell(VCell* vc)
			: lut(NULL),cymux(NULL),xor_(NULL),addf(NULL),level(0),addf_flag(false)
		{
	...
	static void convert_vcell_to_chaincell(Packer::VCellList& vcells, vector<ChainCell*>& chain_cells) {
		for (VCell* vc: vcells) {
			if (vc->type() == VCELL_TYPE::CARRY_CHAIN || vc->type() == VCELL_TYPE::MULTIPLIER) {
	...
	static void find_singal_chain_from_cin(PKInstance* seed, AllChainCells& chain_cells, SingleChain& chain) {
		if (!is_in(seed, chain_cores)) return;
		...
		if(cinet != NULL){
			...
			if(muxcout_pin != NULL && muxcout_pin->instance()->down_module()->name() == CYMUX){
			...
			else if(muxcout_pin == NULL || (muxcout_pin != NULL && muxcout_pin->instance()->down_module()->name() != CYMUX)){
	...
	static void find_single_chain_from_addf_cin(PKInstance* seed, AllChainCells& chain_cells, SingleChain& chain) {
		if (!is_in(seed, chain_cores)) return;
		...
		if(cinet != NULL){
			...
			// the cell is in the middle of the chain
			if(addfcout_pin != NULL && addfcout_pin->instance()->down_module()->name() == ADDF){
			...
			// 1, the cell has no pin named with CYMUX_O
			// 2, the cell luckly has a pin named with CYMUX_O, but it is not CYMUX
			else if(addfcout_pin == NULL || (addfcout_pin != NULL && addfcout_pin->instance()->down_module()->name() != ADDF)){
				first_chain_cells.push_back(cell);
				return;
			}
		}
	...
	static void find_singal_chain_from_cout(PKInstance* seed, AllChainCells& chain_cells, SingleChain& chain) {
		if(!is_in(seed, chain_cores)) return;
		...
		if(conet != NULL) {
			...
			// the cell is in the middle of the chain
			if(muxcin_pin != NULL && muxcin_pin->instance()->down_module()->name() == CYMUX){
			...
			else if(muxcin_pin == NULL && xorcin_pin != NULL && xorcin_pin->instance()->down_module()->name() == XOR){
			...
			else if(muxcin_pin == NULL || (muxcin_pin != NULL && muxcin_pin->instance()->down_module()->name() != CYMUX)){
				;
			}
			return;
		}
	}
	static void find_single_chain_from_addf_cout(PKInstance* seed, AllChainCells& chain_cells, SingleChain& chain) {
		if (!is_in(seed, chain_cores)) return;
		...
		if(conet != NULL){
			...
			// the cell is in the middle of the chain
			if(addfcin_pin != NULL && addfcin_pin->instance()->down_module()->name() == ADDF){
			...
			// this is the second last carry chain cell
			// and the last chain cell has no cymux(only lut+xor)
			//else if(muxcin_pin == NULL && xorcin_pin != NULL && xorcin_pin->owner().instof().name() == XOR){
			//	co_xor = &xorcin_pin->owner();
			//	co_seed = co_xor->property_value<ChainCell*>(CHAIN_CELL)->lut;
			//	co_seed->set_used();
			//	ChainCell* next_cell = co_xor->property_value<ChainCell*>(CHAIN_CELL);
			//	chain.push_back(next_cell);
			//	next_cell->level = chain_level++;
			//}
			// this is the last carry chain cell with cymux
			else if(addfcin_pin == NULL || (addfcin_pin != NULL && addfcin_pin->instance()->down_module()->name() != ADDF)){
				;
			}
			return;
		}
	...
	static void find_singal_chain(PKInstance* seed, AllChainCells& chain_cells, SingleChain& chain)
	{
		chain_level = 0;
	...
	static void find_single_addf_chain(PKInstance* seed, AllChainCells& chain_cells, SingleChain& chain)
	{
		chain_level = 0;
			...
			for (ChainCell* cell: chain){
				if(dq_chain.empty())
					...
					if(cell->level < dq_chain.front()->level)
						...
						ASSERT(0, "There are two chain cells in the same chain with the same level.");
				}
			}
			chain.clear();
			chain_level = 0;
	...
	static void group_carry_chain(AllChainCells& chain_cells, vector<SingleChain>& chains)
	{
	...
	void Packer::pack_carry_chain()
	{
		...
		for (SingleChain chain: chains) {
			...
			for (CellPair& cp: cc_slices) {
				tf_.set_sfull_flag();
				tf_.transform(cp.first);				//先打位于LUT F的进位链
...
#ifdef _DEBUG
					for (Net::pin_iter p_it=f_sink_pin_range_.begin();p_it!=f_sink_pin_range_.end();++p_it)
						cout<<"\tsink pins:"<<p_it->owner()<<"->"<<p_it->name()<<endl;	
#endif
				}
				//打开XB的情况：
				//情况1：最后一级进位链的cout从XB出去；
				//情况2：虽然不是最后一级，但是别的器件需要用到进位链数据，也从XB出去
...
#ifdef _DEBUG
					cout<<"CARRY CHAIN INFO: [F] This is the last level of the carry chain. Carry out by XBUSED/XBMUX."<<endl;
					cout<<"\tnet name:"<<fcout_net->name()<<endl;
#endif
				}
...
#ifdef _DEBUG
					cout<<"CARRY CHAIN INFO: [F] This level has multi-sink pin.  Carry out by XBUSED/XBMUX."<<endl;
					cout<<"\tnet name:"<<fcout_net->name()<<endl;
#endif
				}
				
				//因为在匹配库里打开了cout，现在要关闭，并且将cout端移除本来连接的线网
				//
				if(fcout_net != NULL){
					fcout->disconnect();
					fcout->clear_used();
				...
				if(cp.second != NULL){
					#ifdef _DEBUG
						cout<<"CARRY CHAIN INFO: [G] This level is packed into Slice G"<<endl;
					#endif
					tf_.transform(cp.second);
					fcout_net = fcout->net();										//获得cout端连接的线网
					
					if (fcout_net != NULL){
						Net::pins_type g_sink_pin_range_=fcout_net->sink_pins();
						
#ifdef _DEBUG
						for (Net::pin_iter p_it=g_sink_pin_range_.begin();p_it!=g_sink_pin_range_.end();++p_it){
							cout<<"\tsink pins:"<<p_it->owner()<<"->"<<p_it->name()<<endl;
						}
#endif
					}
					//进位链最后一级的进位输出仍应该从cout出去，ise和我们自己的布线都会自动将cout线网重新连接到YB线网去，这一步不需要packing来重连线网
					//但我们的布局布线需要把最后一级的YBMUX打开
					
					//下面这个if如果条件成立事情就搞大了
					//这一段代码处理cout端连接的线网问题
					//
					//满足条件的线网连接有以下3种情况
					//1-srcpin有低级进位链的cout，sinkpin有高级进位链的cin，一个其他slice或iob的输入pin
					//2-srcpin有低级进位链的cout，sinkpin有高级进位链的cin，多个其他slice或iob的输入pin
					//3-srcpin有低级进位链的cout，sinkpin有多个其他slice或iob的输入pin
					 if (fcout_net != NULL && fcout_net->sink_pins().size()>1){
						...
						for (Net::pin_iter p_it=sink_pin_range_.begin();p_it!=sink_pin_range_.end();++p_it){
						...
						if (has_cin)
						{
							//先创一条新的net：boy_net_name_
							#ifdef _DEBUG
								cout<<"CARRY CHAIN INFO: [G] Divide carry chain net to two nets"<<endl;
							#endif

							Module* net_owner_= fcout_net->module();
							const string boy_net_name_=fcout_net->name()+"_boy_net";
							Net* boy_net_=net_owner_->create_net(boy_net_name_, NORMAL);			//Net& create_net(const string& net_name, NetType type = NORMAL)

							vector<Pin*> pin_ptr_vec;
							for (Net::pin_iter p_it=sink_pin_range_.begin();p_it!=sink_pin_range_.end();++p_it){
								cout<<"name="<<p_it->name()<<endl;
								if (p_it->name()!=CHAIN_NAME::ADDF_CI){		//把除了高级进位链的cin端以外的全部pin找出来,放进pin_ptr_vec
									Pin* new_pin_ptr_=(*p_it);
									pin_ptr_vec.push_back(new_pin_ptr_);
								}
							}
							
							//将放在pin_ptr_vec里的pin全部重新连接到新的线网上去
							for (vector<Pin*>::iterator it=pin_ptr_vec.begin();it!=pin_ptr_vec.end();++it){
							...
							yb->connect(boy_net_);					//将yb也hookup到新线网上
							yb->set_used();
...
#ifdef _DEBUG
							cout<<"CARRY CHAIN INFO: [G] Use YBMUX for the other outlet"<<endl;
				...
				// if the cell's cin is not connected, that means it is connected to constant originally
