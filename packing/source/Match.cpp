...
namespace PACK {
	...
	//////////////////////////////////////////////////////////////////////////
	// class VPin
	void VPin::unhook()           { if (net_) {net_->remove_vpin(this); net_ = NULL;} }
	...
	void VCell::compute_total_gain()
	{
		...
		pk_fac_.total_gain = (1.0-alpha)*pk_fac_.share_gain + alpha*pk_fac_.conn_gain;
	}
	...
	void VCell::compute_seed_factor()
	{
		...
		pk_fac_.conn_factor = (float)num_terminals / (pk_fac_.degree * pk_fac_.degree);
	}
...
//				for (_RPortPinMap::value_type& port_pin_pair: rport_pin_map) {
				for (RulePort* rport: rule_->rule_cell()->ports()) {
					VPort* vport = vcell->create_port(rport);
					Pin* rpin = rport_pin_map[rport];	// port_pin_pair.second
					if (rpin != NULL) {
						PKInstance* image_inst = vcell->find_image_inst(rpin->instance()->name());
