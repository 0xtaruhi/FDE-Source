...
namespace PACK {
	...
	NetlistCSP::NetlistCSP() 
		: cur_match_(NULL), rule_graph_(NULL), image_graph_(NULL),
		  solver_(this) {}
	...
	{
		cur_match_ = match;
		set_rule_graph(cur_match_->rule()->rule_cell()->relative_graph());//将当前rule下的cell设为当前图
	...
	{
		if (rg == rule_graph_) return;//若已经获得该rule的cell图，则返回
		
		clear();

		rule_graph_ = rg;
		build_constraint_graph();//建立含约束的图
	}

	void NetlistCSP::clear() {
		if (rule_graph_)
			for (GObject* gobj: rule_graph_->gobjects())
				delete obj_var_map_[gobj];//删除图的相关结点元素，即删除该图
		obj_var_map_.clear();
		type_set_.clear();
	}
	...
	void NetlistCSP::build_constraint_graph() {
		Variable* var;
		for (GObject* gobj: rule_graph_->gobjects()) {	//the gobjects are already in orders
...
//		for (GObjVarMap::value_type& mt: obj_var_map_) {
			...
			query->query_gobjects(ig->find_gobjs(query->gobj_type()));//query results according to the type name of the object
...
//		for (GObjVarMap::value_type& mt: obj_var_map_) {
			...
			if (rule_obj->kind() == GObjType::GNODE) {
				RuleInstance* rule_inst  = static_cast<RuleInstance*>(static_cast<GNode*>(rule_obj)->correspond());
				PKInstance*   image_inst = static_cast<GNode*>(rela_var->instance())->correspond();
				amatch->push_back(Match::InstPair(rule_inst, image_inst));

				if (rule_obj->rule_to_lock()) { 
					rela_var->instance()->set_rule_lock();//current result instance can be matched in the current rule only once
	...
	SolutionBackjump::SolutionBackjump(NetlistCSP* ncsp) 
		: ncsp_(ncsp) {}
	...
	void SolutionBackjump::solve()
	{
		...
		do 
		{
			...
			case NEXT:
				if (cur_query_idx >= (int)sorted_queries_.size() - 1) {
					...
					if (cur_query->appliable())	{
						cur_var->set_dom_enumer(cur_query->execute());//instantiate the cur_var
						backjump_vars.clear();
			...
			case INSTANTIATE:
				if (direction == 2) status = BACK_JUMP;
				else                status = BACK;
				...
				while(dom_enumer->has_more_elem()) {
					...
					if (gobj->rule_lock()) continue;//the gobj is image and unlocked
					...
					if (conflict_vars.size() == 0) {
						status = NEXT;
						add_injection(cur_var);//the pair of cur_var and its instantiated gobject
						break;
					}
					if (status == BACK_JUMP) {
						backjump_vars.insert(get_top_conflict_var(conflict_vars));//conflict_vars do exist
					}
	...
	Variable* SolutionBackjump::get_top_conflict_var(const std::vector<Variable*>& conflict_vars)
	{
	...
	void SolutionBackjump::add_backjump_target(Query* q, QOrderVarSet& backjump_var)
	{
		if (q->src_var() != NULL) 
...
//		for (NetlistCSP::GObjVarMap::value_type& ov_pair: ncsp_->obj_var_map_)
			vars_left.insert(ncsp_->obj_var_map_[rule_obj]);	//set of vars to be instantiated

		GNode dummy(NULL, "DUMMY", NULL);
		search_breadth_first(breadth_vars, vars_left, &dummy);//深度优先搜索
	...
	void SolutionBackjump::search_breadth_first(SizeOrderVarSet& breadth_vars, SizeOrderVarSet& vars_left, GObject* dummy)
	{
		if (!breadth_vars.empty())
		{
			SizeOrderVarSet reached_var;// breadth searched vars related to the current var
			for (Variable* var: breadth_vars)
			{
				Query* best_query = get_best_query(var);
				if (best_query != NULL) sorted_queries_.push_back(best_query);
				vars_left.erase(vars_left.find(var));
				var->set_instance(dummy);//some kind of flag indicating current var is instantiated and can be used for other query except query type
		...
		if (vars_left.empty()) return;
		else {
			breadth_vars.insert(*vars_left.begin());// var with the smallest query result size
	...
	void SolutionBackjump::get_reached_var(Variable* var, const SizeOrderVarSet& vars_left, SizeOrderVarSet& reached_vars)
	{
		for (BConstraint* c: var->constraints()) {
			if (c->type() == BConstraint::CONSTRAINT_COMNET) continue;
			if (vars_left.find(c->get_cause(var)) != vars_left.end())//find the other var that caused the binary constraint
				reached_vars.insert(c->get_cause(var));//next reachable vars to be queried
		}
	}
