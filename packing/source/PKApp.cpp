...
namespace PACK {
	...
	void PKApp::try_process()
	{
		...
		time(&t_end);
		FDU_LOG(INFO)<< InfoMsg(InfoMsg::INFO_PACK_FINISH) % difftime(t_end, t_start);
	}

	void PKApp::initialize()
	{
		COS::IO::using_simvl();
		//packer_.block_config();//call the function in PKConfig.h in the end//modified by hz
		if(!log_fname_.empty())
		...
		try	{
			CosFactory::set_factory(new PKFactory()); /*sophie:setFactory before load celllib, because you may operate on lib contents*/
			design_.load("xml", lib_fname_);
		}
		catch (std::ios_base::failure& e) {
			FDU_LOG(ERR) << "cannot open library file: " << lib_fname_ << ", please check the file name and path.\n";
			exit(EXIT_FAILURE);
		}
		...
		try {
			...
			} else if (netlist_fname_.find(".blif")!=string::npos)
			{
				...
				FDU_LOG(ERR) << "Can not recogonize this type of file: " << netlist_fname_;
				exit(EXIT_FAILURE);
			}
			//design_.load("edif", netlist_fname_);
		}
		catch (std::ios_base::failure& e) {
			FDU_LOG(ERR) << "cannot open netlist file: "<<netlist_fname_ << ", please check the file name and path.\n";
			exit(EXIT_FAILURE);
		}

		FDU_LOG(INFO)<< InfoMsg(InfoMsg::INFO_LOAD_PLIB) % rule_fname_;
		try { packer_.load_rule_lib(rule_fname_, &design_); }
		catch (std::ios_base::failure& e) {
			FDU_LOG(ERR) << "cannot open rule library file: "<< rule_fname_ << ", please check the file name and path.\n";
			exit(EXIT_FAILURE);
		}
	}
	...
	void  PKApp::parse_command(int argc, char *argv[])
	{
		...
		try	{
			desc_.add_options()("output,o", po::value<string>(), "Output file")
				("chiptype,c", po::value<string>(), "Select chip type")
				("fde,f", po::value<string>(), "Select fde file")
			...
			hidden.add_options()("netlist,n", po::value<string>(), "Netlist File")
				("lib,l", po::value<string>(), "Library File")
				("rule,r", po::value<string>(), "RuleLibrary File")
			...
			p.add("netlist", 1); p.add("lib", 1); p.add("rule", 1);
			po::variables_map vm;             
