...
namespace PACK {
	...
	PKConfig::PKConfig(const string& inst_type, const string& name, const string& xdl_name, bool has_inner, const string& def_val) :
		config_(create_config(inst_type, name, def_val)), xdl_name_(xdl_name), has_inner_(has_inner) {}

	void PKConfig::xdl_out(ostream& os, Instance* inst) {
		string inner;
		string pval = inst->property_value(config_);
		ASSERTSD(!pval.empty());
		if (has_inner_ && xdl_name_[0] >='U' && pval == CFG_NAME::DEF_VALUE)		// fdp4pro
			return;
		if (has_inner_ && pval != CFG_NAME::DEF_VALUE)
			inner = inst->property_value(create_temp_property<string>(INSTANCE, xdl_name_ + "NAME"));
	...
	void ConfigInfo::insert(const value_type& val) {
		...
		switch (val.type_) {
		case NORMAL:
			cfg = new PKConfig(cur_type_, val.name_, val.name_); break;
		case RAM_INIT:
			cfg = new PKConfig(cur_type_, boost::to_upper_copy(val.name_), val.name_); break;
		case LUT:
			ASSERT(cur_type_ == CFG_NAME::SLICE, "block type error, SLICE required");
			cfg = new PKConfig(cur_type_, val.name_, val.name_, true); break;
		}
	...
	void ConfigLoader::load_block(xml_node *node) {
		foreach_child(blk_node, node, "block") {
			...
			foreach_child(prm_node, blk_node, "primitive")
				load_primitive(prm_node);

#ifdef _POWER
			if(btemp == CFG_NAME::SLICE){
				owner->insert(ConfigInfo::value_type("F_TRUTHTABLE",ConfigInfo::LUT));//for power analysis
