...
namespace PACK {
	...
	{ string temp = trim_copy(s); return split(svec, temp, is_any_of(" "), token_compress_on); }

	StrVec& split_params(const string& sparam, StrVec& svec)
	{ string temp = trim_copy_if(sparam, is_any_of("()")); return split(svec, temp, is_any_of(",")); }
