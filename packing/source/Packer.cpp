...
namespace PACK {
	...
	void Packer::try_pack(Design* design, bool fEncry_)
	{
		...
		top_cell_ = static_cast<PKCell*>(design->top_module());//PKCell以module为基类
		tf_.set_top_cell(top_cell_);//tf为transformer类对象
	...
	{
		top_cell_->enable_update_graph();//top_cell_为PKCell类，指向设计的顶层的模块Module类

		RuleLibaray* hw_rule_lib = prule_lib_.hw_rule_lib();
		ASSERT(hw_rule_lib, "hardware rule library not found");

		for (RuleLibaray::Layer* layer: hw_rule_lib->layers()) {//遍历每个layer
			if (layer->empty()) continue;
			PtrVector<Match> layer_matches;
			for (Rule* rule: *layer) {//以每个rule和顶层模块构成一个match
				Match* amatch = layer_matches.add(new Match(rule, top_cell_));//Match(Rule*,PKCell)
				ncsp.solve(amatch);//按生成的match进行CSP匹配
			#ifdef _DEBUG
				if (amatch->num_matches() != 0)
					amatch->write();//debug模式下输出匹配结果（以xml格式）
			#endif
			}
	...
	void Packer::map_to_io() {
		...
		for (VCell* vc: uncluster_loop) {
			if(vc->type() == VCELL_TYPE::GCLKIOB || vc->type() == VCELL_TYPE::BUFGMUX) {
				tf_.set_sfull_flag();
				tf_.transform(vc);
		...
		for  (PKInstance* inst: top_cell_->instances()) {
			...
			if ( tcfg == CFG_NAME::GCLK || tcfg == CFG_NAME::BUFGMUX ) {
				for (PKPin* pin: inst->pins())
					if(pin->is_source()) pin->net()->set_type(NetType::CLOCK);
			}
		}
	}
	...
	void Packer::map_to_cluster()
	{
		...
		for (VCell* seed_vcell: uncluster_vcells_loop) {
			if (seed_vcell->is_clustered()) continue;
	...
	void Packer::add_to_cluster(VCell* vcell) {
		...
		if (vcell->rule()->layer_idx() != SLICE_LAYER) return;
	...
	void Packer::update_partial_gain(VPort* vport)
	{
		...
		if (!net->is_touch()) {
			...
			for (VCell* vcell: net->unique_vcells()) {
				if (vcell->is_clustered()) continue;
		...
		for (VPin* vpin: net->vpins()) {
			if (vpin->vport()->owner()->is_clustered()) continue;
	...
	VCell* Packer::get_next_vcell() {
		...
		for (VCell* cddt_vcell: candidate_vcells) {
			if (cddt_vcell->rule()->layer_idx() != SLICE_LAYER) continue;
			if (tf_.is_feasible(cddt_vcell)) return cddt_vcell;
		}
		return NULL;
	}
