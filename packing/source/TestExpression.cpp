...
namespace PACK {
	...
	static const char* const texpr_types[] = { 
		TTrue::TEXPR_NAME
		, TFaninType::TEXPR_NAME
		, TSliceFull::TEXPR_NAME
		, TPinUsed::TEXPR_NAME
		, TInstUsed::TEXPR_NAME
		, TCurrentProcess::TEXPR_NAME
	};
	...
	// TestExprParser
	void TestExprParser::parse_test_expr(const string& statement, TranTestCase* test_case)
	{
		StrVec svec;
		bool   is_invert = false;

		split_statement(statement, svec);
		ASSERT(svec.size()==1 || svec.size()==2, POErrMsg(POErrMsg::IVLD_STATE) % statement);
		...
		TExprType etype = lexical_cast<TExprType>(svec[0]);//convert a string to a enum type
		ASSERT(etype!=UKWN_EXPR, POErrMsg(POErrMsg::UKWN_TEXPR) % svec[0]);

		string sparam;
		if (svec.size()>1) sparam = svec[1];

		parsing_array[etype](sparam, test_case, is_invert);
	}

	// TestExpr Parsing
	void TTrue::parse(const string& sparam, TranTestCase* test_case, bool is_invert)
	{
		StrVec pvec;

		split_params(sparam, pvec);
		ASSERT(sparam.empty(), POErrMsg(POErrMsg::UEXP_ARG_NUM) % TEXPR_NAME % NUM_PARAM % pvec.size());

		test_case->create_test_expr<TTrue>(is_invert);
	}

	void TFaninType::parse(const string& sparam, TranTestCase* test_case, bool is_invert)
	{
		StrVec pvec;

		split_params(sparam, pvec);
		ASSERT(pvec.size() >= NUM_PARAM, 
			POErrMsg(POErrMsg::UEXP_ARG_NUM_MORE) % TEXPR_NAME % NUM_PARAM % pvec.size());

		TFaninType* texpr = test_case->create_test_expr<TFaninType>(is_invert);

		StrVec avec;
		split_specific(pvec[0], avec, ".");
		ASSERT(avec.size()==2, POErrMsg(POErrMsg::UEXP_ARG_FMT) % pvec[0] % TEXPR_NAME);

		InstructParser::parse_name_info(texpr->iname_i_, avec[0], KEY_WORD::SLICE_INST);
		texpr->pname_        = avec[1];//pin's name
		for (int i=1; i<pvec.size(); ++i)
			texpr->tnames_.push_back(pvec[i]);
	}

	void TSliceFull::parse(const string& sparam, TranTestCase* test_case, bool is_invert)
	{
		StrVec pvec;
	...
	void TPinUsed::parse(const string& sparam, TranTestCase* test_case, bool is_invert)
	{
		StrVec pvec;

		split_params(sparam, pvec);
		ASSERT(pvec.size() == NUM_PARAM, POErrMsg(POErrMsg::UEXP_ARG_NUM) % TEXPR_NAME % NUM_PARAM % pvec.size());
	...
	/////////////////////////sophie///////////////
	void TInstUsed::parse(const string& sparam, TranTestCase* test_case, bool is_invert)
	{
		StrVec pvec;
		...
		InstructParser::parse_name_info(texpr->cname_i_, avec[0], KEY_WORD::SLICE_CELL);
		InstructParser::parse_name_info(texpr->iname_i_, avec[1], KEY_WORD::SLICE_INST);
		
	}

	void TCurrentProcess::parse(const string& sparam, TranTestCase* test_case, bool is_invert)
	{
		StrVec pvec;

		split_params(sparam, pvec);
		ASSERT(pvec.size() == NUM_PARAM, 
			POErrMsg(POErrMsg::UEXP_ARG_NUM) % TEXPR_NAME % NUM_PARAM % pvec.size());

		TCurrentProcess* texpr = test_case->create_test_expr<TCurrentProcess>(is_invert);

		StrVec avec;
		split_specific(pvec[1], avec, "==");
		ASSERT(avec.size()==2, POErrMsg(POErrMsg::UEXP_ARG_FMT) % pvec[1] % TEXPR_NAME);

		InstructParser::parse_name_info(texpr->iname_i_, pvec[0], KEY_WORD::SLICE_INST);
	...
	/////////////////////////sophie///////////////
	// TestExpr Execution
	bool TTrue::execute() const
	{
		return is_inverted_^true;
	}
	...
	bool TFaninType::execute() const
	{
		...
		ASSERT(inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % iname_i_.name % TEXPR_NAME);

		Pin* pin = inst->find_pin(pname_);
		ASSERT(pin, TFErrMsg(TFErrMsg::UFND_ELEM) % "pin" % pname_ % TEXPR_NAME);

		if (pin->net() == NULL) return is_inverted_^false;

		ASSERT(pin->net()->source_pins().size()==1, 
			TFErrMsg(TFErrMsg::USAT_PIN_COND) % pname_ % "single-driven" % TEXPR_NAME);

		Pin* fanout_pin = *pin->net()->source_pins().begin();

		if (fanout_pin->is_mpin()) return is_inverted_^false;
	...
	bool TInstUsed::execute() const
	{
		...
		ASSERT(cell, TFErrMsg(TFErrMsg::UFND_ELEM) % "cell" % cname_i_.name % TEXPR_NAME);

		PKInstance* inst = static_cast<PKInstance*>(cell->find_instance(iname_i_.name));
		ASSERT(inst, TFErrMsg(TFErrMsg::UFND_ELEM) % "instance" % iname_i_.name % TEXPR_NAME);
