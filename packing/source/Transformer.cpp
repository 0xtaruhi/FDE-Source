...
namespace PACK {
		...
		if (is_step_one) {
			...
			while(poss_dangling_insts_.size() != 0 || poss_dangling_nets_.size() != 0) {
				for (Net* net: poss_dangling_nets_) {
					...
					if (is_dangling) {
						poss_dangling_insts_.insert(new_poss_dinsts.begin(), new_poss_dinsts.end());
						net->release();
					}
			...
			if (definit_dangling_insts_.size() != 0) {
				for (Instance* inst: definit_dangling_insts_) {
					// emit signal here! this signal can be ignored, because here the cell graph 
	...
	void Transformer::remove_dangling() {
		...
		new_poss_dnets.clear(); new_poss_dinsts.clear();
		while(poss_dangling_insts_.size() != 0 || poss_dangling_nets_.size() != 0) {
			for (Net* net: poss_dangling_nets_) {
				if (is_net_dangling(net, new_poss_dinsts)) {
...
/*
	bool Transformer::is_inst_dangling(Instance* inst, vector<Net*>& poss_dangling_nets)
	{
		...
		for (Pin& pin: inst->pins()) {
			if (pin.dir() == COS::INPUT)
				...
				has_output_pin = true; // assume it only has input/output pin
...
*/
	...
	bool Transformer::is_net_dangling(Net* net, vector<Instance*>& poss_dangling_insts)
	{
		bool is_dangling = (net->source_pins().size()==0 || net->sink_pins().size()==0);
		if (is_dangling)
			for (Pin* pin: net->pins())
				if (!pin->is_mpin()) poss_dangling_insts.push_back(pin->instance());
		return is_dangling;
	}
	...
	void Transformer::execute(TranOpVec* op_vec)
	{
		for (TranOperation* op: *op_vec) {
			...
			for (const TranTestCase* test_case: op->test_cases()) {
				if (!test_case->test_expr()->execute()) continue;
	...
	bool Transformer::is_feasible(VCell* vcell) {
		...
		static cstr_ptr pname_test[NUM_TEST] = {
			&PIN_NAME::CLK, &PIN_NAME::CE, &PIN_NAME::SR
		};
		...
		if (num_lut_in_cur_slice_ + vcell->rule()->rule_cell()->num_lut() > LUT_SIZE_PER_SLICE) 
			return false;
		if (num_ff_in_cur_slice_  + vcell->rule()->rule_cell()->num_ff() > FF_SIZE_PER_SLICE)  
			return false;
	...
	Transformer::TranOpVec* Transformer::parse_rule(Rule* rule)
	{
		...
		if (fit != tran_operations_.end()) return fit->second;
	...
	string Transformer::make_name(const string& prefix, const string& suffix)
	{
		...
		return n.append(gbl_prefix).append(lexical_cast<string>(idx)).append(gbl_prefix).append(suffix);
	}

	Module* Transformer::find_cell(const NameInfo& cname_i)
	{
		if (cname_i.is_kw) return slice_cell_;
	...
	Instance* Transformer::find_instance(const NameInfo& iname_i) 
	{
		...
		if (cur_op_->target() != top_cell_){
			...
			{
				NameCellMap::iterator fit = created_cell_lktb_.find(cur_op_->target()->name());
			    Module* cell = fit == created_cell_lktb_.end() ? NULL : fit->second;
				return cell->find_instance(iname_i.name);
			}
			else*/
