...
namespace FDU { namespace Place {
	 ...
	 */
	/************************************************************************/
	bool SAPlacer::try_swap(double t, double rlim, 
							double& cost, double& bb_cost, double& tcost, 
							double inv_pre_bb_cost, double inv_pre_tcost)
	{
		...
		if(_mode == PLACER::TIMING_DRIVEN){
			...
			delta_cost  = (1 - PLACER::TIMING_TRADE_OFF) * delta_bb_cost * inv_pre_bb_cost 
	...
	*/
	...
	double SAPlacer::get_std_dev(int n, double sum_of_square, double av)
	{
		...
		if(n <= 1)		 std_dev = 0.;
		else			 std_dev = (sum_of_square - n * av * av) / (double) (n - 1);

		/* Very small variances sometimes round negative */
		if(std_dev > 0.) std_dev = sqrt (std_dev);
		else			 std_dev = 0.;
	...
	*/
	/************************************************************************/
	double SAPlacer::starting_t(double rlim,
								double& cost, double& bb_cost, double& tcost,
								double inv_pre_bbcost, double inv_pre_tcost, bool isAllFixed)
	{
		...
		for (int i = 0; i < num_swap_try; ++i){
			if (try_swap(PLACER::HIGHEST_T, rlim, 
						 cost, bb_cost, tcost, 
						 inv_pre_bbcost, inv_pre_tcost))
			{
				++num_swap_accepted;
				av			  += cost;
	...
	*/
	...
	void SAPlacer::try_place(PlaceArgs args)
	{
		...
		//得到初始化温度及其cost
		double	t = starting_t(rlim, cost, bb_cost, tcost, inv_pre_bb_cost, inv_pre_tcost, isAllFixed);
        ...
        if (!isAllFixed)
        {
            while(!exit_sa_alg(t, cost)){
                ...
                for (int i = 0; i < move_lim; ++i){
                    //成功交换一次
                    if (try_swap(t, rlim, cost, bb_cost, tcost, inv_pre_bb_cost, inv_pre_tcost)){
                        ++num_success;
