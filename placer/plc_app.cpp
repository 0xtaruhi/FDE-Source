...
namespace FDU { namespace Place {
	...
	*/
	...
	{
		//读入arch文件
		INFO(CONSOLE::PROGRESS % "10" % ("loading arch library \"" + _args._arch_lib + "\""));
		try { FPGADesign::instance()->loadArchLib(_args._arch_lib,new ArchFactory); }
		catch(exception& e){
			ERR(CONSOLE::FILE_ERROR % "can NOT load arch library.");
			exit(EXIT_FAILURE);
		}
		//读入网表文件
		INFO(CONSOLE::PROGRESS % "20" % ("loading netlist \"" + _args._input_nl + "\""));
		try {
		...
		if(!_args._plc_csts.empty()){
			INFO(CONSOLE::PROGRESS % "30" % ("loading constraint \"" + _args._plc_csts + "\""));
			try { _design.load("constraint", _args._plc_csts); }
			catch(exception& e){
		...
		if(!_args._library.empty()){
			INFO(CONSOLE::PROGRESS % "35" % ("loading template cell library \"" + _args._library + "\""));
			try {_design.load(_args._encrypt ? "xml*" : "xml", _args._library);  }
			catch(exception& e){
		...
		if(_args._mode == PLACER::TIMING_DRIVEN){
			...
			INFO(CONSOLE::PROGRESS % "40" % ("loading delay library \"" + _args._delay_lib + "\""));
			try {_design.load("delay", _args._delay_lib); }
			catch(exception& e){
...
//		static boost::format CST  ("|%-30s|%-10s|%-5d|\n");
		...
		ASSERT(ofs.is_open(), (CONSOLE::FILE_ERROR % "can NOT write jtag constraints."));
...
//					throw place_error(CONSOLE::PLC_ERROR % (inst.name() + ": INVALID IOB for jtag constraints."));
					echo = false;
					INFO(CONSOLE::PLC_WARNING % (inst->name() + ": IGNORE dangling JTAG IOB."));
				}
			...
			if(echo) {
				...
				string pad_name = arch_lib->find_pad_by_position(inst->property_value(positions));
				ofs << CST %dir													\
						   // jtag need to have lower case name
						   %boost::algorithm::to_lower_copy(inst->name())		\
						   %pad_name;
						   //%pad_name.substr(1);
			}
		}
