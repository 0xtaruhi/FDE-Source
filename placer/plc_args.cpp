...
namespace FDU { namespace Place {
	...
	{
		cout << "\nUsage: " << endl;
		cout << "  " << "placer [--arch|-a] <arch_lib.xml> " 
	...
	void PlaceArgs::try_parse(int argc, char* argv[])
	{
		try
		{
			...
			visible.add_options()
				("arch,a",			value<string>(), "arch   library(default at 1st)")
				...
				("delay,d",			value<string>(), "delay	 library")
				("cst,c",			value<string>(), "place constraints")
				("jtag,j",			value<string>(), "jtag  constraints (export)")
				("effort_level,l",  value<string>(), "place effort level")
				("bounding_box,b",					 "bounding_box placer")
				("timing_driven,t",					 "timing_driven placer")
				("update_rand_seed,u",				 "update seed of randomizer")
				("fde,f",			value<string>(), "set fde file")
			...
			if(vm.count("bounding_box") && vm.count("timing_driven")){
				throw std::runtime_error("ONLY allow to use either bounding_box or timing_driven placer");
			}

			if(vm.count("effort_level")) {
				string level = vm["effort_level"].as<string>();
				_effort_level = level == "medium" ? PLACER::MEDIUM :
								level == "high"	  ? PLACER::HIGH   : PLACER::STANDARD;
			} else {
