...
namespace FDU { namespace Place {
	...
	void BoundingBox::compute_bounding_box(const Point& device_scale)
	{
		...
		for (PLCInstance* inst: _owner->connected_insts()) {
			...
			if (logic_pos.x == _x_max) {  // Recall that xMin could equal xMax -- don't use else
				_x_max_edge++;
			}
			if (logic_pos.x < _x_min) {
				_x_min		= logic_pos.x;
	...
	void BoundingBox::update_bounding_box(const Point& device_scale, const Point& pos_old, const Point& pos_new)
	{
