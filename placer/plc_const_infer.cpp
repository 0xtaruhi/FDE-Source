...
namespace FDU { namespace Place {
	...
	pair<Net*, Net*> ConstantsGener::find_constants_nets(TDesign *design)
	{
		...
		Property<string>& types = 
			create_temp_property<string>(COS::NET, NET::TYPE);
 	...
 	void ConstantsGener::generate_dummy_cell(TDesign* design, Net* gnd_net, Net* vcc_net)
 	{
		Library* work_lib = design->find_or_create_library("work_lib");
		_const_slice_cell = work_lib->create_module("DUMMY_CONSTANTS_GENER", CELL::SLICE);
		if(gnd_net) generate_gnd_part(design);
		if(vcc_net)	generate_vcc_part(design);
 	}

	void ConstantsGener::generate_dummy_inst(TDesign* design, Net* gnd_net, Net* vcc_net)
	{
		_const_slice_inst = design->top_module()->create_instance("inst_dummy_constants_gener", _const_slice_cell);
		if(gnd_net){
...
#ifdef fdp3000k
		Instance* lut4  = _const_slice_cell->create_instance("dummy_gnd_G", tcell_lib->modules().find("LUT4"));
#else
		Instance* lut4  = _const_slice_cell->create_instance("dummy_gnd_G", tcell_lib->modules().find("G"));
#endif

 		Instance* gymux = _const_slice_cell->create_instance("gymux", tcell_lib->modules().find("GYMUX"));
 		Instance* yused = _const_slice_cell->create_instance("yused", tcell_lib->modules().find("YUSED"));
 	...
 	void ConstantsGener::generate_vcc_part(TDesign* design)
 	{
		...
		_const_slice_cell->create_port("X",  OUTPUT);
 		...
 		// instances
 		Library*  tcell_lib = design->find_or_create_library("template_cell_lib");
 		Instance* fxmux = _const_slice_cell->create_instance("fxmux", tcell_lib->modules().find("FXMUX"));
 		Instance* xused = _const_slice_cell->create_instance("xused", tcell_lib->modules().find("XUSED"));
#ifdef fdp3000k
		Instance* lut4  = _const_slice_cell->create_instance("dummy_vcc_F", tcell_lib->modules().find("LUT4"));
#else
		Instance* lut4  = _const_slice_cell->create_instance("dummy_vcc_F", tcell_lib->modules().find("F"));
#endif
