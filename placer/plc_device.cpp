...
// streams
	...
	static const char* site_type[] = { "SLICE", "TBUF", "IOB", "BLOCKRAM", "GCLK", "GCLKIOB", "DLL", "VCC", "BUFGMUX", "BRAM16","MULT18X18","DCM" };
	static EnumStringMap<Site::SiteType> site_type_map(site_type);
	std::istream& operator >> (std::istream& s, Site::SiteType& type) { 
		try					{ type = site_type_map.readEnum(s); }
		catch(bad_cast& e)	{ type = Site::IGNORE; return s; }
		return s; 
	}
...
// DeviceInfo
	...
	double DeviceInfo::get_p2p_delay(PLCInstance* src, PLCInstance* sink)
	{
		...
		if(src_type == Site::SLICE || src_type == Site::BLOCKRAM) {
			if(sink_type == Site::SLICE)
				delay = lookup_delay_from_table(_delay_tables[DEVICE::CLB_TO_CLB], delta_row, delta_col);
			else if(sink_type == Site::IOB) 
				delay = lookup_delay_from_table(_delay_tables[DEVICE::CLB_TO_OUTPUT], delta_row, delta_col);
			else
				delay = lookup_delay_from_table(_delay_tables[DEVICE::CLB_TO_CLB], delta_row, delta_col);
		} 
		else if(src_type == Site::IOB) {
			if(sink_type == Site::SLICE)
				delay = lookup_delay_from_table(_delay_tables[DEVICE::INPUT_TO_CLB], delta_row, delta_col);
			else if(sink_type == Site::IOB)
				delay = lookup_delay_from_table(_delay_tables[DEVICE::INPUT_TO_OUTPUT], delta_row, delta_col);
			else
				delay = lookup_delay_from_table(_delay_tables[DEVICE::INPUT_TO_CLB], delta_row, delta_col);
		}
		else {
	...
	{
		Library* tile_lib = FPGADesign::instance()->find_or_create_library(LIBRARY::TILE);
		Property<RscStat>& site_infos = create_temp_property<RscStat>(COS::MODULE, CELL::SITE_INFO);
		for (Module* cell: tile_lib->modules())
		{
			...
			for (Instance* inst: cell->instances()) {
				...
				if(is_logic_site(site_type)){
					if(rsc_stat.count(site_type)) rsc_stat[site_type] += 1;
					else						  rsc_stat[site_type]  = 1;
				}
			}
		}
	...
	double DeviceInfo::lookup_delay_from_table(const DelayTable& table, int delta_row, int delta_col)
	{
