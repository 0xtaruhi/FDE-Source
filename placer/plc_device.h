...
namespace FDU { namespace Place {
	...
	struct Site
	{
		enum SiteType{ 
			...
			BLOCKRAM,				// BRAM	
			GCLK, GCLKIOB, DLL,		// CLKT, CLKB
			VCC, BUFGMUX,BRAM16,MULT18X18,DCM,	// New site in V2
			NUM_OF_SITE_TYPE 
		};
		...
		// "get" here means some algorithm used in these functions, not simply get the value
		// so these function should be take serious
	...
	public:
		~Tile() {
			for (Sites::value_type& s: _sites) delete s.second;
		}
		//设置/得到Tile的类型
	...
	public:
		...
		void	set_carry_chain(const vector<vector<int>> carry){DEVICE::carry_chain = carry;}
		...
		static void	 
		load_delay_tables(const string& file, bool encrypt = false);
		static void resize_delay_table(int size) {_delay_tables.resize(size);}
		static DelayTable&
		delay_table(DEVICE::DelayTableType type) { return _delay_tables[type]; }
		
		static Point	 
		lookup_phy_pos(const Point& p) { return _phy_pos_lut.at(p); }

	protected:
		double	 lookup_delay_from_table(const DelayTable& table, int delta_row, int delta_col);
