...
//  	using COS::Library;
	...
	public:
		PLCInstance(const std::string& name, Module& instof, Module& owner)
			: Instance(name, &instof, &owner), 
			  _curr_loc_site(NULL),	_curr_logic_pos(), 
			  _is_fixed(false), _swapable_type(FLOORPLAN::SITE)
		{}
		//设定该instance的从属site，这里保存了两个指针，一个是原来的，一个是当前的
		void	set_curr_loc_site(Site* site)	{ _past_loc_site = _curr_loc_site;
												  _curr_loc_site = site; }
		//得到当前的site
		...
		//设置当前逻辑地址
		void	set_curr_logic_pos(const Point& pos) { _past_logic_pos = _curr_logic_pos; 
													   _curr_logic_pos = pos; }
		//得到当前/以前逻辑地址
	...
	public:
		...
		PLCNet(const std::string& name, COS::NetType type, Module& owner)
			: Net(name, type, &owner,NULL),
			  _is_ignored(false),
			  _affected_type(NONE),
			  _bounding_box(new BoundingBox(this)),
			  _total_tcost(0.)
		{}
		...
		void	update_bounding_box(const Point& pos_from, const Point& pos_to, const Point& device_scale) 
	...
	public:
		...
		typedef PtrList<Instance>::typed<PLCInstance>::const_range    const_instances_type;
		typedef PtrList<Instance>::typed<PLCInstance>::iterator       instance_iter;
		typedef PtrList<Instance>::typed<PLCInstance>::const_iterator const_instance_iter;
