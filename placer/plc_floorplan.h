...
namespace FDU { namespace Place {
	...
	protected:
		...
		//检查并保存布局信息
		void   check_and_save_place(){ _nl_info.check_place(); _nl_info.save_place(); }
		...
		Result satisfy_cst_rules_for_site	   (SwapObject* from_obj, Point& pos_to, double rlim);
		// from_site must be LUT6
		// from_obj has 2 insts
		// pos_to.z is meaningless because both slices will be swapped
		Result satisfy_cst_rules_for_lut6	   (SwapObject* from_obj, Point& pos_to, double rlim);
		// from_site must be carry chain
		// from_site has several insts(a whole carry chain)
		// pos_to.z is known after this function
		Result satisfy_cst_rules_for_carrychain(SwapObject* from_obj, Point& pos_to, double rlim);

		typedef std::vector<PLCInstance*>							SwapInsts;
		void   update_qualified_pos_for_carrychain(int tile_col, int index);
		void   update_qualified_pos_for_carrychain(SwapInsts& from_insts, SwapInsts& to_insts);
//		void   update_qualified_pos_for_lut6();

		void   swap_insts				(SwapInsts&	 from_insts, SwapInsts& to_insts, 
		...
		{ return (double)std::max(_dev_info.device_scale().x, _dev_info.device_scale().y); }
		/************************************************************************/
		/* 计算模拟退火过程的移动次数 */
		/************************************************************************/
		int	   num_swap_try()	const
		{ return (int)(_effort_level * pow(_nl_info.design()->top_module()->num_instances(), 1.3333)); }

		virtual FLOORPLAN::SwapableType 
		find_to(SwapObject* from_obj, Point& pos_to, double rlim);
