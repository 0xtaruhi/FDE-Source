...
namespace FDU { namespace Place{
	...
	void DelayTableHandler::load_delay(xml_node* node)
	{
		...
		this->_delay_table_type  = lexical_cast<DelayTableType>(get_attribute(node, "name"));
		this->_delay_table_scale = lexical_cast<Point>(get_attribute(node, "scale"));
		string content = node->value();
		trim(content);
		load_table(DeviceInfo::delay_table(_delay_table_type), 
			_delay_table_scale, 
			Tokens(content, seq));
	}

	void DelayTableHandler::load_table(DeviceInfo::DelayTable& table, const Point& scale, const Tokens& tokens)
	{
