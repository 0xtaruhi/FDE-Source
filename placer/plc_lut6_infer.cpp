...
namespace FDU { namespace Place {
	...
	{
		Property<string>& hsets = create_temp_property<string>(COS::INSTANCE, INSTANCE::HSET);
		...
		for (PLCNet* net: static_cast<PLCModule*>(design->top_module())->nets()) {
			Net::pin_iter F5_it   = find_if(net->pins(), [](const Pin* pin) { return pin->name() == DEVICE::F5;   });
			Net::pin_iter F5IN_it = find_if(net->pins(), [](const Pin* pin) { return pin->name() == DEVICE::F5IN; });

			if(F5_it != net->pins().end() && F5IN_it != net->pins().end()){
				// allow more pins besides F5 & F5IN
//				ASSERT(net.num_pins() == 2, 
//					   place_error(CONSOLE::PLC_ERROR % (net.name() + ": illegal LUT6 net.")));
