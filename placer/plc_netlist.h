...
namespace FDU { namespace Place {
	...
	class PLCNet; class PLCInstance;
	struct Site;
...
//		typedef std::map<int, std::vector<std::set<Point> > >	QualifiedPos;
	...
	public:
		...
		virtual ~NLInfo() { for (SwapObject* o: _swap_objs) delete o; }
		//得到设计网表
		...
		//得到可交换对象的数目
//		int			  num_swap_objects()       const { return _swap_objs.size(); }
		int			  num_swap_objects()       const { return _num_swap_objs; }
		//得到/剔除index的可交换对象
		SwapObject*   swap_object(int index)		 { return _swap_objs[index]; }
		void		  pop_swap_object(int index)	 { _swap_objs[index] = _swap_objs.back();
													   _swap_objs.pop_back(); 
													   --_num_swap_objs; }
		//增加一个可交换对象
...
//				make_pair(tile_col, std::vector<std::set<Point> >(DEVICE::NUM_SLICE_PER_TILE))
				make_pair(tile_col, std::vector<std::vector<int> >(DEVICE::NUM_CARRY_PER_TILE))
			);
	}
...
// inline functions
	...
	inline void SwapObject::clr_qualified_pos(int tile_col, int index)
	{
		...
		//if(_qualified_pos.count(tile_col)&&_qualified_pos[tile_col][index].size() !=0)
	...
	*/
	/************************************************************************/	
	inline void SwapObject::add_qualified_pos(const int index, const Point& logic_pos)
	{
...
// 			_qualified_pos[logic_pos.y];
	...
	*/
	/************************************************************************/	
	inline bool SwapObject::exist_qualified_pos(int rows, const Point& logic_pos, const int index)
	{
		//if(_qualified_pos.count(logic_pos.y))
