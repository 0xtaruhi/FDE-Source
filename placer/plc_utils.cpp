...
namespace FDU { namespace Place {
	...
	{
		/************************************************************************/
		/* 初始化的值都是针对3000k的，如果是v2的话程序会处理更新这些值                                            */
		...
		/************************************************************************/
		static const char* delay_table_types[] = { "clb2clb", "input2clb", "clb2output", "input2output", "num_delay_table_type" };
		static EnumStringMap<DelayTableType> typemap(delay_table_types);
		istream& operator >> (istream& s, DelayTableType& type) { type = typemap.readEnum(s); return s; }
		ostream& operator << (ostream& s, DelayTableType  type) { return typemap.writeEnum(s, type); }
	}
