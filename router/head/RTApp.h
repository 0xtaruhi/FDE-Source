...
namespace FDU { namespace RT {
	...
	struct RTParams
	{
		...
		explicit RTParams (
			RTAlgorithm rt_algo_        = DIRECTED_SEARCH ,
			BaseCostType bcost_type_    = DEMAND_ONLY,
			double first_iter_pres_fac_ = 0.5 ,
			double initial_pres_fac_    = 0.5 ,
			double pres_fac_mult_       = 2. ,
			double hist_fac_            = 1. ,
			int max_iters_              = 30 ,
			int bb_fac_                 = 3 ,
			double max_crit_            = 0.99 ,
			double crit_exp_            = 1.,
			double astar_fac_             = 0.5
			) :
			first_iter_pres_fac(first_iter_pres_fac_),
			initial_pres_fac(initial_pres_fac_),
			pres_fac_mult(pres_fac_mult_),
			hist_fac(hist_fac_),
			max_iters(max_iters_),
			bb_fac(bb_fac_),
			max_crit(max_crit_),
			crit_exp(crit_exp_),
			astar_fac(astar_fac_),
			rt_algo(rt_algo_),
			bcost_type(bcost_type_) {}
	};
