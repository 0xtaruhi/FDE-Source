...
namespace FDU {namespace RT {
	...
	public:
		...
		CstNet* creat_cstNet(const string& name) {return _cstNets.add(new CstNet(name));}
	...
	public:
		RTCstLoadHandler(const string& cstFileName, CstNets *pCstNets) : _cstFileName(cstFileName), _pCstNets(pCstNets) {}
		void load();
	private:
