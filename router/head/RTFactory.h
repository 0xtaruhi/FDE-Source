...
namespace FDU { namespace RT {
	...
	public:
		RTNet(const string& name, NetType type, Module* owner, Bus* bus) :
			COSRTNet(name, type, owner,bus),
			is_ignore_(false),
			is_partial_routed_(false),
			src_node_(NULL),
			seg_begin_(path_.end()),
			seg_end_(path_.end()) {}
		...
		void free_path_back() { path_.clear(); seg_begin_ = seg_end_ = path_.end(); }
