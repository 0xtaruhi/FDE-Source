...
namespace FDU { namespace RT {
	...
	public:
		struct RTInfo 
		{
			...
			RTInfo() :
				...
				pres_cost(1.),
				hist_cost(1.),
				path_cost(HUGE_FLOAT),
				total_cost(HUGE_FLOAT),
				tarflg(0){}
		};
	...
	public:
		...
		{
			if (occ_ < capacity_) rt_info_.pres_cost = 1.;
			else rt_info_.pres_cost = 1. + ( occ_ + 1 - capacity_ ) * pres_fac;
		}
		void mod_pres_cost()
		{
			rt_info_.pres_cost = 1. + rt_info_.pres_cost;
		}
		void update_prev_info(RTNode* prev_node, double path_cost, RRGSwitch* prev_sw = NULL, double total_cost = 0.)
		{
		...
		double cong_cost() const { return Router::Instance().get_base_cost_infos()[base_cost_idx_].base_cost /*base_cost */*
	...
	public:
		...
		const_nodes_type nodes() const { return static_cast<const_nodes_type>(RRGraph::nodes()); }
	};

	class RTNodeFactory : public RRGFactory{
	public:
		RRGNode*   make_rrgnode(RRGArchNet* owner)
				{ return new RTNode(owner);}
	};
}}
