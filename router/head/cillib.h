...
namespace FDU {
	namespace rt_cil_lib {
		...
		template<typename R, typename T, typename C = boost::ptr_vector<T> > struct pContainer : public C {
			...
			struct iter_pair : Rng {
				...
				typename std::iterator_traits<IT>::pointer find(const string& name) const {
					...
					if (it == (*this).end()) {
						//				std::cout << Warning("container: no such member ... " + name) << std::endl;
						return NULL;
					}
					else
						return &*it;
					//			ASSERT(it != end(), FDU::Exception("container: no such member ..." + name));
				}
			};
			...
			template<typename PT> T& add(PT t) { T& rt = *t; this->push_back(t); return rt; }		// PT is T* or std::unique_ptr<T>
		...
		// 	{ T* rt = &*t; it = C::insert(it, t); return std::make_pair(rt, it); }
			...
			const_range_type range() const { return const_range_type((*this).begin(), (*this).end()); }
		};

		template<typename T> struct cilContainer : pContainer<FDU::rt_cil_lib::CilBase, T> {};
		...
		public:
			...
			sramElem& getSram(const std::string &sramName) const { *srams().find(sramName); }
		...
		inline bool Element::hasRequiredPath(const std::string& in, const std::string& out) {
		...
		public:
			...
			Element* getElement(const std::string &elemName) { return elements().find(elemName); }
		};
