...
//		HeapNode(RTNode* owner, RTNode* prev_rt_node, RRGSwitch* prev_sw, double tcost)
//			: _owner(owner), _prev_rt_node(prev_rt_node), _prev_sw(prev_sw), _total_cost(tcost) 
//		{}
		void assign(RTNode* owner, RTNode* prev_rt_node, RRGSwitch* prev_sw, double tcost)
		{
			_owner = owner; _prev_rt_node = prev_rt_node; _prev_sw = prev_sw; _total_cost = tcost; 
		}
...
// breadth-first heap
	...
	{
//		BFHeapNode(RTNode* owner, RTNode* prev_n, RRGSwitch* prev_sw, double tcost)
//			: HeapNode(owner, prev_n, prev_sw, tcost) {}
		static BFHeapNode* create(RTNode* owner, RTNode* prev_n, RRGSwitch* prev_sw, double tcost);
		void release();
	};
	...
	public:
		...
		BFHeapNode* heap_head() { return static_cast<BFHeapNode*>(Heap::heap_head()); }
		void add_node_to_heap(RTNode* cur_n, RTNode* prev_n, RRGSwitch* prev_sw, \
							  double cur_total_cost);

		void expand_neighbours(RTNode*, RTNode*, RTNode*, const RTNet::BBFac&, double, double);
...
//			: HeapNode(owner, prev_n, prev_sw, tcost), _bpath_cost(bp_cost), _Rupstream(Rup) 
//		{}

		void assign(RTNode* owner, RTNode* prev_n, RRGSwitch* prev_sw, double tcost, double bp_cost, double Rup)
		{
			HeapNode::assign(owner, prev_n, prev_sw, tcost); _bpath_cost = bp_cost; _Rupstream = Rup; 
		}
	...
	{
	public:
		~TDHeap() { for(HeapNode* n: _heap) static_cast<TDHeapNode*>(n)->release(); }
		TDHeapNode* heap_head() 
		{ return static_cast<TDHeapNode*>(Heap::heap_head()); }
		void add_node_to_heap(RTNode* cur_n, RTNode* prev_n, RRGSwitch* prev_sw, \
							  double cur_total_cost, double cur_bp_cost, double Rup);

		void   expand_neighbours(TDHeapNode*, const RTNet::BBFac&, double, double, const Pin&);
...
// inline functions
	...
	{ _heap.push_back(node); std::push_heap(_heap.begin(), _heap.end(), total_cost_smaller); }
