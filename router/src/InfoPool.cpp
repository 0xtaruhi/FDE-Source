...
namespace FDU { namespace RT {
	...
	bool InfoPool::check_netlist() const 
	{
		...
		for (RTNet* net: static_cast<RTCell*>(p_design_->top_module())->nets())
		{
			...
			for (const Pin* pin: net->pins())
				if (pin->is_mpin())
				{
					cerr << WarMsg(WarMsg::NLTWNG_PORT_CONN) % pin->name() % net->name() << endl;
					is_fatle = true;
				}
		}
		return is_fatle;
	...
	void InfoPool::echo_netlist() const 
	{
		...
		int num_sinks = 0;
		int num_ignore = 0;
		for (const RTNet* net: static_cast<RTCell*>(p_design_->top_module())->nets())
		{
	...
	{
		FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_ECHO_CHIP) % FPGADesign::instance()->name();
        FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_ECHO_RESOURCE);
		FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_ECHO_RSRC) % "segment" % p_rrg_->num_nodes();
		FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_ECHO_RSRC) % "pip"     % p_rrg_->num_pips();
	}
	...
	void InfoPool::echo_success_rt(int num_try) const
	{
		...
		FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_SUCES_RT) % num_try % prop_pip % num_non_pips;
	}
}}