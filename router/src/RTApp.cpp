...
namespace FDU { namespace RT {
	...
	RTApp::RTApp() :
		desc_("Options"),
		encrypted_(true),
		router_(&Router::Instance()) {} //返回一个router实例的地址
	...
	void RTApp::store_cmd(int argc, char* argv[], po::variables_map& vm) //存储命令行参数
	{
		...
		optional_desc.add_options()
			...
			("breadth-first,b", "Breadth-first routing")
			("timing-driven,t", "Timing-driven routing")
			("directed-search,d", "Directed-search routing(Default)")
			("noencrypt,e", "run as NOT encrypt version")
			("help,h", "Produce help message")
			("astar-factor,s", po::value<double>(), "Astar factor")
			("cst,c", po::value<string>(), "router constraint file")
	...
	void RTApp::set_algorithm(po::variables_map& vm)	//设置route的算法（默认directed-search）
	{
	...
	void RTApp::set_default_output_file_name()  //若没有设定输出文件，设定默认输出文件(_rt.xml)
	{
		if(!file_extension_is_xml(netlist_fname_)) 
		{
			FDU_LOG(ERR) << ErrMsg(ErrMsg::CMDERR_ILL_FILE_FMT) % "netlist";  //若netlist不是xml，输出无效
			display_usage_and_exit();
		}
		else
		{
	...
	bool RTApp::file_extension_is_xml(const std::string& file_name) const //判断文件后缀是否为 .XML
	{
	...
	void RTApp::error_check()	//输入错误判断
	{
		...
		else
		{
			if(!file_extension_is_xml(output_fname_))
				output_fname_ =  output_fname_ + ".xml";	//输入文件没有.xml后缀，添加xml后缀
	...
	{
		FDU_LOG(INFO) << "\nUsage: ";
		FDU_LOG(INFO) << "\trouter <netFile.xml|XML> <archFile.xml|XML> [-o RouteResultFile.xml]";
		FDU_LOG(INFO)<< "\n";	
	...
	{
		FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_LOAD_NL) % netlist_fname_;	//netlist 文件
		...
		if (!cil_fname_.empty()) design_.set_property(cil_filename, cil_fname_);
	}
