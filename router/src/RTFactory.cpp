...
#define using_ise_plc
	...
	void RTNet::init_rt_info(RTGraph* rrg, int bb_fac, const CstNets* _pCstNets)
	{
		if (_pCstNets)
		{
			...
			if(cstNet && cstNet->num_pips() > 0) {
				ASSERT(num_pips() == 0, this->name() + ": you have pips infomation in input netlist file,\
		...
		for(const Pin* pin: this->pins())
		{
			if ( pin->name().find("CLK") != string::npos || pin->name().find("CK") != string::npos ){
				set_type(NetType::CLOCK);
				break;
			}
			else if (pin->name().find("SR") != string::npos || pin->name().find("RST") != string::npos)
			{
	...
	void RTNet::load_bb_fac(int bb_fac)
	{
		...
		int nx = p_fpga_design->scale().x, 
			ny = p_fpga_design->scale().y;
	...
	void RTNet::load_src_sink_nodes(RTGraph* rrg)
	{
		...
		ASSERT(pos.x != -1 && pos.y != -1, ErrMsg(ErrMsg::RTERR_POS) % "source pin" % src_pin->name());

		// load source node
		src_node_ = static_cast<RTNode*>(rrg->find_logic_pin_node(src_pin->instance(), *src_pin, pos));
		...
		for (Pin* sink_pin: sink_pins())
		{
			...
			ASSERT(pos.x != -1 && pos.y != -1, ErrMsg(ErrMsg::RTERR_POS) % "sink pin" % sink_pin->name());
			RTNode* sink_node = static_cast<RTNode*>(rrg->find_logic_pin_node(sink_pin->instance(), *sink_pin, pos));
			ASSERT(sink_node != NULL,
				ErrMsg(ErrMsg::RTERR_UFND_NODE) % "sink" % sink_pin->name() % name());
			sink_node->set_sink();
	...
	void RTNet::load_info_for_routed_net(RTGraph* rrg)
	{
		...
		for(const PIP* pip: routing_pips())
		{
			if(pip == routing_pips().front() || next_pip_from_net_record)
			{
				...
				RTNode* from_node = static_cast<RTNode*>(rrg->find_grm_net_node(pip->from(), pip->position()));
				ASSERT(from_node, "can not find pip, may be you are using a different arch file...");
				from_node->add_occ();
				fixed_path_.push_back(from_node);
			}
			RTNode* to_node = static_cast<RTNode*>(rrg->find_grm_net_node(pip->to(), pip->position()));
			ASSERT(to_node, "can not find pip, may be you are using a different arch file...");
			to_node->add_occ();
			...
			{
				ASSERT(to_node->is_sink(), ErrMsg(ErrMsg::NLTERR_LAST_PIP_ILLEGAL) % pip->to() % this->name());
			}
		}
	...
	{
		int unf_num = 0;
		path_node_iter node_it  = path_.begin(), 
					   end_node = path_.end();
		for (; node_it != end_node; ++node_it)
		{
	...
	void RTNet::path_update_one_cost(path_node_iter path_str, int add_or_sub, double pres_fac)
	{
		path_node_iter node_it  = path_str, 
			      end_node = path_.end();
	...
	RTNet::path_node_iter RTNet::update_trace_back(RTNode* sink_node)
	{
		...
		if ( seg_end_ != path_.end() )
		{
			...
			//return ret_iter from the second node of current branch path, for the branch path 
			//must have one common node with trunk path
			temptail = seg_end_;
			++temptail;
			++temptail;
			ret_iter = temptail;
