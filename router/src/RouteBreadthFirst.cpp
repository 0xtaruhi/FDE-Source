...
namespace FDU { namespace RT {
		...
		for (RTNode* snk_node: net.sink_nodes())
		{
			...
			while (while_loop_should_not_end(curr_node, snk_node))
			{
				...
				if( path_cost > new_cost )
				{
					...
					bf_heap.expand_neighbours(src_node, curr_node, snk_node, bb, new_cost, rt_params_.astar_fac);
				}
			...
			curr_node->update_prev_info(bf_heap_node->_prev_rt_node, bf_heap_node->_total_cost);
	...
	void Router::expand_trace_back(RTNet::path_node_iter node_it, RTNet& net, BFHeap& bf_heap)
	{
