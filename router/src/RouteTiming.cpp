	...
	int Router::try_timing_driven_route()
	{
		...
		for (int itry = 1; itry <= rt_params_.max_iters; ++itry)
		{
			...
			if (num_infeasible == 0)
			{
				...
				FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_ECHO_NUM_URT_NET) % itry % num_infeasible;
...
// 			reset_base_cost_infos();
		...
		for (int i=0; i<net.sink_nodes().size(); ++i)
		{
			...
			td_heap.add_rtree_to_heap(RTree.root(), *sink_pin, TarCrit, rt_params_.astar_fac);
			...
			while (cur_rt_node != sink_node)
			{
				...
				if (old_path_cost > new_path_cost && old_total_cost > new_total_cost)
				{
					...
					cur_rt_node->update_prev_info(cur_heap_node->_prev_rt_node, new_path_cost, cur_heap_node->_prev_sw, new_total_cost);
				
					td_heap.expand_neighbours(cur_heap_node, bb, TarCrit, rt_params_.astar_fac, *sink_pin);
				}
	...
	void Router::mark_timing_sinks(RTNet& net, std::vector<Pin*>& sink_pins)
	{
		...
		for (; sink_pin_it != sink_pins.end(); ++sink_pin_it)
		{
			...
			RTNode* sink_node = static_cast<RTNode*>(rrg_->find_logic_pin_node((*sink_pin_it)->instance(), *(*sink_pin_it), pos));
			ASSERT(sink_node != NULL,
				ErrMsg(ErrMsg::RTERR_UFND_NODE) % "sink" % (*sink_pin_it)->name() % net.name());
