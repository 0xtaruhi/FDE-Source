...
namespace FDU { namespace RT {
	...
	Router::Router()
		: design_(nullptr), top_cell_(nullptr), rrg_(new RTGraph()), _pCstNets(nullptr) {}

	Router::~Router() { delete rrg_; if (_pCstNets) delete _pCstNets; }
	...
	void Router::initialize(TDesign* design, const RTParams& rt_params)
	{
		...
		design_    = design;
		if(_pCstNets) {
			ASSERT(design_->name() == _pCstNets->getDesignName(), "the design name in your netlist file and constraint file is different...");
		}
...
#endif
		time(&rrg_end);
		FDU_LOG(INFO) << "  * Building rrg complete. Elapsed Time " << difftime(rrg_end, rrg_begin);
//#define CHECK_ARCH
#ifdef CHECK_ARCH
...
#define SEG_TYPE_NUM 3
	void Router::set_rrgnodes_base_cost_index()
	{
		for (RRGNode* node: rrg_->nodes())
		{
			...
			case RRGNode::SOURCE: 
			case RRGNode::CLK   : 
			case RRGNode::SINK  : rt_node->set_timing(0., 0. + node->Cext());
					              rt_node->set_base_cost_idx(rt_node->type());
					              break;
			case RRGNode::SINGLE:
			case RRGNode::HEX   : rt_node->set_timing(rt_node->R()*rt_node->length()*0.5, rt_node->C()*rt_node->length()*0.5 + rt_node->Cext());
					              rt_node->set_base_cost_idx(rt_node->direction()*SEG_TYPE_NUM+rt_node->type());
							      break;
			case RRGNode::LONG  : rt_node->set_timing(rt_node->R()*LLENGTH, rt_node->C()*LLENGTH + rt_node->Cext());
								  rt_node->set_base_cost_idx(rt_node->direction()*SEG_TYPE_NUM+rt_node->type());
								  break;
			default             : ASSERT(0, (ErrMsg(ErrMsg::RTERR_NODE_TYPE) % rt_node->owner_name() % rt_node->from_pos() % rt_node->to_pos()));
					              break;			
			}
		}
	}
	void Router::add_base_cost_info()
	{
		...
		for (int i=1; i<= SEG_TYPE_NUM; ++i)
		{
			...
			switch (i)
			{
			case 1: base_cost_infos_[index].inv_length = 1./SINGLE_LEN; break;
			case 2: base_cost_infos_[index].inv_length = 1./HEX_LEN;    break; 
			case 3: base_cost_infos_[index].inv_length = 1./nx;         break;
			default: base_cost_infos_[index].inv_length = OPEN;         break;
			}		  
		}
		...
		for (int i=1; i<= SEG_TYPE_NUM; ++i)
		{
			...
			case 1: base_cost_infos_[index].inv_length = 1./SINGLE_LEN; break;
			case 2: base_cost_infos_[index].inv_length = 1./HEX_LEN;    break; 
			case 3: base_cost_infos_[index].inv_length = 1./ny;         break;
			default: base_cost_infos_[index].inv_length = OPEN;         break;
			}  
		}
	...
	void Router::load_base_cost_TValue()
	{
		...
		ArchCell::inst_iter grm_it = boost::find_if(tile->instances(), 
								[this](const ArchInstance* inst) { return rrg_->is_grm(inst); });
		if(grm_it != tile->instances().end())
		{
			for (Net* net: grm_it->down_module()->nets())
			{
				RTNode* rt_node = static_cast<RTNode*>(rrg_->find_grm_net_node(net->name(), pos));
				if (rt_node->type() == RRGNode::SOURCE || rt_node->type() == RRGNode::SINK)
					continue;
		...
		for (cost_idx = SEGX_COST_INDEX_START; cost_idx < base_cost_infos_.size(); ++cost_idx)
		{
			...
			else
			{
				...
				base_cost_infos_[cost_idx].TQuadratic = (R_sw + R_average) * 0.5 * C_average;
	...
	void Router::save_base_cost()
	{
		...
		if (rt_params_.bcost_type == DELAY_NORMALIZED || rt_params_.bcost_type == DEMAND_ONLY)
		{
			base_cost_infos_[SOURCE_COST_INDEX].base_cost = delay_norm_fac;
			base_cost_infos_[CLK_COST_INDEX].base_cost = delay_norm_fac/2;       //special for gclk
		...
		for (int i = SEGX_COST_INDEX_START; i < max_idx; ++i)
		{
			if (rt_params_.bcost_type == INTRINSTIC_DELAY)
				base_cost_infos_[i].base_cost = base_cost_infos_[i].TLinear + base_cost_infos_[i].TQuadratic;
			else
			{
...
//for clk nets timing requirement
	...
	{
		//GCLK
		base_cost_infos_[CLK_COST_INDEX].base_cost = base_cost_infos_[CLK_COST_INDEX].saved_base_cost*0.5;
		//single
		base_cost_infos_[SEGX_COST_INDEX_START].base_cost = base_cost_infos_[SEGX_COST_INDEX_START].saved_base_cost*2;
		base_cost_infos_[SEGX_COST_INDEX_START+SEG_TYPE_NUM].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+SEG_TYPE_NUM].saved_base_cost*2;
        //hex
		base_cost_infos_[SEGX_COST_INDEX_START+1].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+1].saved_base_cost;
		base_cost_infos_[SEGX_COST_INDEX_START+1+SEG_TYPE_NUM].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+1+SEG_TYPE_NUM].saved_base_cost;
        //long
		base_cost_infos_[SEGX_COST_INDEX_START+2].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+2].saved_base_cost;
		base_cost_infos_[SEGX_COST_INDEX_START+2+SEG_TYPE_NUM].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+2+SEG_TYPE_NUM].saved_base_cost;
	}
	void Router::reset_base_cost_infos()
	{
		//GCLK
		base_cost_infos_[CLK_COST_INDEX].base_cost = base_cost_infos_[CLK_COST_INDEX].saved_base_cost;
		//single
		base_cost_infos_[SEGX_COST_INDEX_START].base_cost = base_cost_infos_[SEGX_COST_INDEX_START].saved_base_cost;
		base_cost_infos_[SEGX_COST_INDEX_START+SEG_TYPE_NUM].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+SEG_TYPE_NUM].saved_base_cost;
		//hex
		base_cost_infos_[SEGX_COST_INDEX_START+1].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+1].saved_base_cost;
		base_cost_infos_[SEGX_COST_INDEX_START+1+SEG_TYPE_NUM].base_cost = base_cost_infos_[SEGX_COST_INDEX_START+1+SEG_TYPE_NUM].saved_base_cost;
		//long
