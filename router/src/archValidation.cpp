...
namespace FDU{namespace RT{
	...
	void TilePortValidation::validate() const{
		...
		for (ArchInstance* inst: static_cast<ArchCell*>(_arch->find_library("arch")->find_module(_arch->name()))->instances()){
			vldtInstance(inst, ofs);
		}
		ofs.close();
	}

	void TilePortValidation::vldtInstance(const ArchInstance *inst, ostream &os) const{
	...
	void TilePortValidation::vldtInstanceSide(const ArchInstance *inst, SideType side, ostream &os) const{
		ASSERT(side == BOTTOM || side == RIGHT, "TilePortValidation: I just accept side of BOTTOM OR RIGHT.");
		...
		const Point oppositePos = side == BOTTOM ? Point(pos.x + 1, pos.y) : Point(pos.x, pos.y + 1);

		const Port *pt = inst->down_module()->find_port(sideStr);
		const ArchInstance *oppositeInst = _arch->get_inst_by_pos(oppositePos);
		const Port *oppositePt = oppositeInst->down_module()->find_port(oppositeSideStr);

		if(pt == NULL && oppositePt != NULL){
			PRINTLN(os, format("%s(%s) has no %s side port but its opposite tile %s(%s) has %s port.")
				%inst->name() %lexical_cast<string>(pos) %sideStr %oppositeInst->name() %lexical_cast<string>(oppositePos) %oppositeSideStr);
		}
		if(oppositePt == NULL && pt != NULL){
			PRINTLN(os, format("%s(%s) has no %s side port but its opposite tile %s(%s) has %s port.")
				%oppositeInst->name() %lexical_cast<string>(oppositePos) %oppositeSideStr %inst->name() %lexical_cast<string>(pos) %sideStr);
		}
		if(pt && oppositePt && !portsFit(pt, oppositePt)){
			PRINTLN(os, format("%s(%s)'s %s side port AND %s(%s)'s %s side port do not match.")
				%inst->name() %lexical_cast<string>(pos) %sideStr %oppositeInst->name() %lexical_cast<string>(oppositePos) %oppositeSideStr);
		}
	}
