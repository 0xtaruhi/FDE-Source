...
// Heap
	...
	public:
		SimplePool(int size = 32) : next_size(size), chunk(0), chunk_size(-1), chunk_idx(-1), block_idx(-1) {}
		~SimplePool() { for (int i = 0; i < store.size(); ++i) delete[] store[i].first; }
		T* malloc() {
...
//	static boost::pool<> bf_pool(sizeof(BFHeapNode));
	...
	BFHeapNode* BFHeapNode::create(RTNode* owner, RTNode* prev_n, RRGSwitch* prev_sw, double tcost)
	{
	...
	void BFHeap::add_node_to_heap(RTNode* cur_n, RTNode* prev_n, RRGSwitch* prev_sw, double cur_total_cost)
	{
		if(cur_total_cost >/*=*/ cur_n->path_cost()) 
			return;
		
		Heap::add_node_to_heap(BFHeapNode::create(cur_n, prev_n, prev_sw, cur_total_cost));
	}

	void BFHeap::expand_neighbours(RTNode* src_node, RTNode* rt_node,  RTNode* snk_node, const RTNet::BBFac& bb, double path_cost, double astar_fac)
	{
		RTNode*  child_node;
		double   total_cost;
	...
	int BFHeap::get_expect_cost(RTNode* src_node, RTNode* curr_node, RTNode* snk_node)
	{
// 		if(distance_between_nodes(src_node, snk_node) > 1)
// 		{
// 			return distance_between_nodes(curr_node, snk_node) / distance_between_nodes(src_node, snk_node);
// 		}
// 		else
			return distance_between_nodes(curr_node, snk_node);
	}
	
	int BFHeap::distance_between_nodes(const RTNode* first_node, const RTNode* second_node)
	{
	...
	int BFHeap::distance_between_points(const Point& first_pos, const Point& second_pos)
	{
...
//	static boost::object_pool<TDHeapNode> td_pool;
	...
	TDHeapNode* TDHeapNode::create(RTNode* owner, RTNode* prev_n, RRGSwitch* prev_sw, double tcost, double bp_cost, double Rup)
	{
								  ...
								  double cur_total_cost, double cur_bp_cost, double Rup)
	{
		...
		Heap::add_node_to_heap(TDHeapNode::create(cur_n, prev_n, prev_sw, cur_total_cost, cur_bp_cost, Rup));
	}

	void TDHeap::expand_neighbours(TDHeapNode* td_heap_node, const RTNet::BBFac& bb, double TarCrit, double astar_fac, const Pin& sink_pin)
	{
		...
		for (RRGSwitch* sw: rt_node->switches())
		{
			...
			double Tdel = sw->delay() + (NewRupstream - child_node->R()/2.) * child_node->C();

			//////////////////////////////////////////////////////////////////////////
			///from source to child new_path_cost = old_path_cost + (1 - TarCrit) * p(n)b(n)h(n) + TarCrit * delay(n)
			double new_path_cost = td_heap_node->_bpath_cost + (1 - TarCrit) * child_node->cong_cost() + TarCrit * Tdel;

			//////////////////////////////////////////////////////////////////////////
			///total_cost = path_cost + crit_fac * expected_cost
			double new_total_cost = new_path_cost; // + astar_fac * get_expected_cost(child_node, sink_pin, NewRupstream, TarCrit);

			add_node_to_heap(child_node, rt_node, sw, new_total_cost, new_path_cost, NewRupstream);
		}	
	}

	double TDHeap::get_expected_cost(RTNode* cur_rt_node, const Pin& tar_pin, double Rupstream, double TarCrit)
	{		
		int num_segs_ortho_dir;
		int num_segs_same_dir = get_exp_segs_num(cur_rt_node, tar_pin, num_segs_ortho_dir);
		...
		double Tdel = num_segs_same_dir * T_same_dir + num_segs_ortho_dir * T_ortho_dir
			          + Rupstream * cur_rt_node->C() * (num_segs_same_dir + num_segs_ortho_dir);

		return TarCrit * Tdel + (1 - TarCrit) * cong_cost;	 
	}

	int TDHeap::get_exp_segs_num(RTNode* cur_rt_node, const Pin& tar_pin, int& num_segs_ortho_dir)
	{
		...
		Point cur_pos = (abs(cur_rt_node->from_pos().x - tar_pos.x) + abs(cur_rt_node->from_pos().y - tar_pos.y)) > \
			            (abs(cur_rt_node->to_pos().x - tar_pos.x) + abs(cur_rt_node->to_pos().y - tar_pos.y)) ? \
						cur_rt_node->to_pos() : cur_rt_node->from_pos();
		...
		{
			num_segs_ortho_dir = ceil(static_cast<double>(abs(tar_pos.y - cur_pos.y) / seg_length));
			return ceil(abs(static_cast<double>(tar_pos.x - cur_pos.x) / seg_length));
		} 
		else if (seg_dir == RRGNode::RRG_NODE_DIR::VERTICAL)
		{
			num_segs_ortho_dir = ceil(static_cast<double>(abs(tar_pos.x - cur_pos.x) / seg_length));
	...
	void TDHeap::add_rtree_to_heap(TreeNode* cur_root, const Pin& tar_pin, double TarCrit, double astar_fac)
	{
		if (cur_root->_re_expand)
		{
			RTNode* cur_rt_node = cur_root->_owner;
			double  path_cost = TarCrit * cur_root->_delay;
			double  total_cost = path_cost; // + astar_fac * get_expected_cost(cur_rt_node, tar_pin, cur_root->_Rupstream, TarCrit);
			add_node_to_heap(cur_rt_node, NULL, NULL, total_cost, path_cost, cur_root->_Rupstream);
