...
// RTNode
	...
	{
		_parent = NULL; _parent_sw = NULL; _re_expand = true;
...
// RouteTree
	...
	TreeNode* RouteTree::update_route_tree(TDHeapNode* sink_hnode)
	{
		...
		if(unbuffered_subtree_root->_parent){
			...
			tarr += unbuffered_subtree_root->_parent_sw->R() * unbuffered_subtree_root->_Cdownstream; 
	...
	void RouteTree::update_net_delays_from_route_tree(RTNet* cur_net, std::vector<Pin*>& sink_pins)
	{
		Property<TData>& DELAY = 
			create_temp_property<TData>(COS::PIN, "delay");

		for (int i=0; i<cur_net->sink_nodes().size(); ++i)
		{
			TreeNode* sink_tnode = (cur_net->sink_nodes()[i])->tree_node();
			sink_pins[i]->set_property(DELAY, TData(sink_tnode->_delay, sink_tnode->_delay));
		}
	}
	...
	TreeNode* RouteTree::update_unbuffered_ancestors_Cdownstream(TreeNode* start_of_new_path)
	{
		TreeNode*  tree_node        = start_of_new_path;
		TreeNode*  parent_tree_node = tree_node->_parent;
	...
	void RouteTree::load_unbuffered_subtree_delay(TreeNode* subtree_root, double tarr)
	{
		...
		for (TreeNode::Child& child: subtree_root->_children){
			...
			double     child_delay = delay + child_sw->R() * tree_node->_Cdownstream \
								   + child_sw->delay();
