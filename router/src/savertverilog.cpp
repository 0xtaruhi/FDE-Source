...
namespace {
	...
	Property<string>& cil_filename = create_property<string>(COS::DESIGN, "cil_fname");

	const char * valid_chars = "_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	inline string rename(const string& name) {
		return name.find_first_not_of(valid_chars) == string::npos ? name : '\\' + name + ' ';
	}
	...
	void write_cfg(const string& inst_name, const string& cfg_name, const string& cfg_value, ostream& os) {
		os << format("\n  defparam %s.%s = \"%s\";") % rename(inst_name) % cfg_name % cfg_value;
	}
			...
			for (vector<string>::iterator it = vStr.begin(); it != vStr.end(); it++)
			{
				...
				for (row = 0, base = 2; row < 4; base *= 2, row++) {
					for (column = 0; column < base + 1; column++) {
						if (item[row][column] == s_tmp && column != 0) value += 1 << (base - column);
					}
				}
			}
	...
	void write_property(Instance* inst, ostream& os) {
		...
		for (Config* cfg : inst->configs()) {
			if (inst->property_value(*cfg) == _def_val_conf) { continue; }
			string pname = cfg->name();
			if (pname.find("#LUT") != string::npos) continue;
			string pvalue = inst->property_value(*cfg);
			if (pvalue.substr(0, 7) == "#LUT:D=") {
				//std::cout << pvalue.substr(7) << "\n";
				inst->set_property(name_transform(pname), transform_property(pvalue.substr(7)));
			}
			if (pname == "SYNC_ATTR") {
		...
		for (init_pair* pp = init_props; pp->first; pp++) {
			...
			if (!inst->property_exist(prop)) continue;
			string pname = pp->second;
			string pvalue = inst->property_value(prop);
			if (pvalue[0] == '#')
				pvalue = pvalue[1] == '1' ? "FFFF" : "0000";
			os << format("\n  defparam %s.%s = 16'h%s;") % rename(inst_name) % pname % pvalue;
		}
	}
	...
	void set_gsb_default(Instance* inst, elemLib* elemlib){
		...
		for (Instance* inst_in_down_module : inst->down_module()->instances()) {
			...
			if (iter != Configinfo.end()) { conf = iter->second;}
			else {
	...
	void add_config(Instance* inst, string from, string to, elemLib* elementlib) {
		...
		string config;
		map<string, Config*>::iterator iter;
		for (RRGArchInstance* ainst : static_cast<RRGArchCell*>(inst->down_module())->instances()) {
				for (Path* path : ainst->paths()) {
					if (std::get<0>(*path)->name() == from && std::get<1>(*path)->name() == to){
						instanceref = ainst;
	...
	Instance* find_gsb_down_mod(FPGADesign* fpgadesign, Point pos) {
		...
		{
			if (inst->module_type() == "GSB") { return inst; }
		}
	}

	Pin* find_port_pin(FPGADesign* fpgadesign, Instance* gsb_inst, string name,Point pos) {
		Pin* return_pin;
		Instance* ins_tmp = fpgadesign->get_inst_by_pos(pos);
		Module* mod_tmp = ins_tmp->down_module();
		Net* net_tmp;
	...
	void connect_two_pins(Pin* pin_from, Pin* pin_to ,string net_name, RTCell* mod, Point pos) {
		Net *new_net, *net_old;

		string new_net_name = net_name + "_" + pin_from->name() + "_to_" + pin_to->name() + 
			boost::lexical_cast<string>(pos.x+1) + "_" + boost::lexical_cast<string>(pos.y+1)
			+ "_" + boost::lexical_cast<string>(pos.z+1);
		
		if (pin_to->net()){	return; }
		if ((net_old = pin_from->net()) != 0) { pin_to->connect(net_old); }
		else {
	...
	void add_gsb(FPGADesign* fpgadesign, RTCell* rtmod, string cil_fname, ostream& os){

		vector<RTNet*> rtnets;

		// 用于记录需要添加的 GSB Instance
		Instance* gsbInst;
		map<Point, Instance*> gsbInstInfos;
		map<Point, Instance*>::iterator gsbIter;

		// 读入 cil 库文件，用于生成 GSB 的配置
		FDU_LOG(INFO) << InfoMsg(InfoMsg::INFO_LOAD_ARCH) % cil_fname;	//arch文件
		cilLibrary* cillib = cilLibrary::loadCilLib(cil_fname);
		elemLib* elementlib = cillib->getElemLib();

		// 遍历已布的 net， 并添加不同位置的 GSB instance
		for (RTNet* rtnet : rtmod->nets())
			rtnets.push_back(rtnet);
		for (RTNet* rtnet : rtnets) {

			// 对于某些特殊线，clk 等，可能是没有 pip 的
			if (rtnet->num_pips() == 0)
				continue;
			...
			// 记录 src_pin 以及 sink_pins. 将 rtnet 与各个 pin 的连接关系断开
			Pin* src_pin = *(boost::find_if(rtnet->pins(), [](const Pin* pin) { return pin->is_source(); }));
			...
			for (from_node_it  = rtnet->path_begin(); from_node_it != end_node; ++from_node_it)
			{
				// 先找到 pip 所在的 position 是否有对应 GSB  
				...
				ASSERT(p_sw != NULL,
					ErrMsg(ErrMsg::RTERR_UFND_SWH) % (*from_node_it)->full_name() % (*to_node_it)->full_name());

			
				Instance* gsb_down_inst = find_gsb_down_mod(fpgadesign, p_sw->pos());
				Module* gsb_down_mod = gsb_down_inst->down_module();
				string gsb_inst_name = gsb_down_mod->name() + "_"
					+ boost::lexical_cast<string>(p_sw->pos().x + 1) + "_"
					+ boost::lexical_cast<string>(p_sw->pos().y + 1)
					+ "_" + boost::lexical_cast<string>(p_sw->pos().z + 1);
				gsbIter = gsbInstInfos.find(p_sw->pos());
				if (gsbIter != gsbInstInfos.end()){
					gsbInst = gsbIter->second;
				}
				else {
				// 若找不到则创建一个 GSB Instance
					gsbInst = rtmod->create_instance(gsb_inst_name, gsb_down_mod);
					set_gsb_default(gsbInst, elementlib);
					gsbInstInfos[p_sw->pos()] = gsbInst;
				}
				// 给 GSB 加入 config 文件
				add_config(gsbInst, p_sw->from_net()->name(), p_sw->to_net()->name(), elementlib);


				Net::const_pin_iter from_up_pin_iter = boost::find_if(p_sw->from_net()->pins(),
													[](const Pin* pin) { return pin->is_mpin(); });
				Pin* from_up_pin = (from_up_pin_iter != 
									p_sw->from_net()->pins().end() ?
								   (*from_up_pin_iter)->up_pin(gsbInst)
					               :nullptr );
				
				Net::const_pin_iter to_up_pin_iter = boost::find_if(p_sw->to_net()->pins(),
													[](const Pin* pin) { return pin->is_mpin(); });
				
				
				Pin* to_up_pin = (to_up_pin_iter !=
								 p_sw->to_net()->pins().end() ? 
								 (*to_up_pin_iter)->up_pin(gsbInst)
								 : nullptr);
				Net* to_mod_net = (to_up_pin_iter !=
								  p_sw->to_net()->pins().end() ?
								  (*to_up_pin_iter)->up_pin(gsb_down_inst)->net()
							      : nullptr);

				//如果 to_node 没有访问过，则加入 visitedNodesMap
				...
				if (from_up_pin && from_up_pin != prev_pin) {
					...
					if (visitedIter != visitedNodesMap.end()) {
						...
						if (prev_pin != from_up_pin)
							connect_two_pins(prev_pin, from_up_pin, rtnet->name(), rtmod, p_sw->pos());
					}
					else {
						connect_two_pins(prev_pin, from_up_pin, rtnet->name(), rtmod, p_sw->pos());
					}
				...
				if ((*to_node_it)->is_sink()){
					...
					sink_pin = std::find_if(sink_pins.begin(), sink_pins.end(), [slicePinName](const Pin* a) { return a->name() == slicePinName; });
					if (sink_pin == sink_pins.end() && src_pin->name() == "COUT")
						sink_pin = std::find_if(sink_pins.begin(), sink_pins.end(), [](const Pin* a) { return a->name() == "CIN"; });
					ASSERT(sink_pin != sink_pins.end(), 
							ErrMsg(ErrMsg::ERROR_FIND_SINK_PIN) % to_up_pin->name() % to_mod_net->name());
					connect_two_pins(to_up_pin, *sink_pin, rtnet->name(), rtmod, p_sw->pos());
					prev_pin = src_pin;
	...
	void write_instance(Instance* inst, ostream& os) {  													 
		...
		for (const Pin* pin : inst->pins()) {
			...
			os << format("%s.%s(%s)") % sep % rename_port(rename(pin->name())) % net_name;
			sep = ",\n    ";
		}
		os << "\n  );\n";
	}

	void write_module(FPGADesign* fpgadesign, Module* mod, string cil_fname, ostream& os) { 
		...
		for (const Port* port : mod->ports()) {
			...
			if (port->is_vector())
				str_ports += (format("[%d:%d] %s;\n") % port->msb() % port->lsb() % rename(port->name())).str();
			else str_ports += rename(port->name()) + ";\n";
		}
		if (mod->num_ports()) os << ')';
		os << ";\n";
			...
			if (!(nports == 1 && vname == find_if(net->pins(), is_mpin)->name())) {
				...
				if (nports)
					for (const Pin* pin : net->pins() | filtered(is_mpin))
						if (pin->is_source())
							str_assign += (format("  assign %s = %s;\n") % vname % rename(pin->name())).str();
						else
							str_assign += (format("  assign %s = %s;\n") % rename(pin->name()) % vname).str();
			}
		...
		// module instances
		for (Instance* inst : mod->instances()) write_instance(inst, os);
	...
	void write_cfg_test(Module* mod,const Design* fpgadesign,ostream& os) {
			...
			for (Module* prim_cell : prim_lib->modules())
			{
				...
				for (RRGArchInstance* ainst : static_cast<RRGArchCell*>(prim_cell)->instances()) {
					for (Path* path : ainst->paths()) {
						os << std::get<0>(*path)->name() << ":" << std::get<1>(*path)->name() << ainst->name() << ":" <<"\n";
					}
				}
			}
	}
...
namespace COS {
	namespace IO {
		...
		void RTVerilogWriter::write(std::ostream& ostrm) const {
			...
			write_module(fpgadesign,design()->top_module(), design()->property_value(cil_filename), ostrm);
