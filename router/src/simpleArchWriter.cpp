	...
	void TileWriter::buildTilePin(xml_node* parent, Pin* pin)
	{
		...
		if(net)
		{
			for (Pin* netPin: net->pins())
			{
				...
				//there are pins DIRECTLY connected to slice such as FXINA3 of CENTER in FDP500K.
				//if we don't allow this, don't command out ASSERTD.
				//ASSERTD(is_GRM_OR_GSB(netPin->instance()), "tile pin connect to wrong place(not opposite tile or GRM)");
...
// 		ASSERTD(pinNet);
		if(pinNet)
		{
			for (Pin* netPin: pinNet->pins())
			{
				...
				//there are pins DIRECTLY connect to tile such as SOPIN of CENTER in FDP500K.
				//if we don't allow this, don't command out ASSERTD.
				...
				{
					//we need COPY_VALUE because netPin->name() + "_OF_" + netPin->owner()->name() just generate a temp string.
					set_attribute(siteLink, "PinOfAnotherSite", netPin->name() + "_OF_" + netPin->owner()->name(), COPY_VALUE);
				}
			}
		}
	}
