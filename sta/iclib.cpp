...
double ICPath::lookup(double load, double tran, LUT& lut)
{
	...
	double delay_load_low_bound =
		...
		* (load - oload[load_base]) / (oload[load_base +1] - oload[load_base])
	...
	double value =
		(delay_load_high_bound - delay_load_low_bound)
		* (tran - itran[tran_base]) / (itran[tran_base + 1] - itran[tran_base])
...
void ICLibHandler::load_iclib( xml_node* node, ICLib *iclib )
{
	foreach_child(lib_node, node, "library")
		load_sublib(lib_node, iclib);
}

void ICLibHandler::load_sublib( xml_node* node, ICLib *iclib )
{
	foreach_child(model_node, node, "cell")
		load_icmodel(model_node, iclib);
}
...
void ICLibHandler::load_icmodel( xml_node* node, ICLib *iclib )
{
	...
	foreach_child(path_node, node, "path")
		load_icpath(path_node, icmodule);
...
void ICLibHandler::load_icpath( xml_node* node, ICModule *icmodule )
{
	...
	foreach_child(child_node, node, 0)
		smart_node_analysis(path, child_node);
...
void ICLibHandler::smart_node_analysis( ICPath* path, xml_node* node )
{
	...
	if ("cap"    == ename)
	{
		...
		if (scin_off.size())  path->cin_off   = lexical_cast<double>(scin_off);
		if (scin_on.size())   path->cin_on    = lexical_cast<double>(scin_on);
		if (scout_off.size()) path->cout_off  = lexical_cast<double>(scout_off);
	} else
	if ("spice"  == ename)
	{
	...
	if ("delayr" == ename) load_table(path, path->delayr, tokens); else
	if ("delayf" == ename) load_table(path, path->delayf, tokens); else
	if ("transr" == ename) load_table(path, path->transr, tokens); else
	if ("transf" == ename) load_table(path, path->transf, tokens); else
	if ("itran"  == ename)
	{
		for (const string &avalue: tokens) path->itran.push_back(lexical_cast<double>(avalue));
		path->y = path->itran.size();
	} else
	if ("oload"  == ename)
	{
		for (const string &avalue: tokens) path->oload.push_back(lexical_cast<double>(avalue));
...
void ICLib::load(std::istream& istrm)
{
	...
	if (node->name() == string("device")) handler.load_iclib(node, this);
}
