...
struct ICPath; class ICModule;

struct ICPath
{
	ICPath(ICModule* owner, size_t id) : owner(owner), id(id), x(0), y(0),
		cin_off(0.), cin_on(0.), cout_off(0.), cout_on(0.) {}
	...
	virtual double		lookup_delayr(double load, double tran) { return lookup(load, tran, delayr); }
	virtual double		lookup_delayf(double load, double tran) { return lookup(load, tran, delayf); }
	virtual double		lookup_transr(double load, double tran) { return lookup(load, tran, transr); }
	virtual double		lookup_transf(double load, double tran) { return lookup(load, tran, transf); }
...
public:
	...
	~ICModule() { for (ICPath* path: paths_) if (path) delete path, path = 0; }

	ICPath*	add_path(ICPath* path) { paths_.push_back(path); return path; }
	ICPath* get_path(int i)
	{
		ASSERT(i >= 0 && i < paths_.size(), "illegal access of path ID: " + boost::lexical_cast<string>(i));
		return paths_[i];
	}
...
public:
	...
	{ modules_.insert(std::make_pair(model->name(), model)); return model; }
