	...
	for (Module* tile_cell: lib->modules())
	{
		...
		for (Pin* pin: tile_cell->pins())
			...
			ptile->add_pin( pin->name(),
					NWGFactory::instance().make_pin(pin->index(), static_cast<ArchPort*>(pin->port())) );
		...
		for (Net* net: tile_cell->nets())
		{
			...
			if (!connect_to_out) continue; // check net has connected to tile pin
			...
			for (const Pin* grm_pin: grm_pins)
			{
				...
				{
					FDU_LOG(WARN) << "dangling: " + grm_pin->down_pin()->path_name() << endl;
				}
			}
			...
			if (self_pins.size() >= 2)
			{
				for (vec_pin::const_iterator it_u = self_pins.begin(); it_u != self_pins.end(); ++it_u)
				for (vec_pin::const_iterator it_d = it_u + 1;          it_d != self_pins.end(); ++it_d)
				{
