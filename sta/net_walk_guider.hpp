...
struct TilePin
{
	...
	{ return (boost::format(owner->pin_name_format()) % owner->name() % index).str(); }

	std::string    reverse_name() const
	{ Side op_dir = static_cast<Side>( (owner->side()+2) % 4 );
	  return (boost::format(owner->pin_name_format()) % boost::lexical_cast<std::string>(op_dir) % index).str(); }
...
public:
	...
	virtual ~Tile ()
	{ for (pin_map::iterator it = pin_repo_.begin(); it != pin_repo_.end(); ++it) delete it->second; }
	...
	void add_pin           (const std::string& name, TilePin* pin) {      pin_repo_.insert(std::make_pair(name, pin)); }
	void add_grm_net_pin   (const std::string& name, TilePin* pin) {  grm_net_pins_.insert(std::make_pair(name, pin)); }
	void add_self_pin_pin  (const std::string& name, TilePin* pin) { self_pin_pins_.insert(std::make_pair(name, pin)); }
...
public:
	...
	Pins find_grm_net_pins    (const std::string& tile_type, const std::string& grm_net) const;
	Pins find_tile_pin_pins   (const std::string& tile_type, const std::string& pin_name) const;
...
inline Point TilePin::offset_point() const
{
	...
	switch (dir())
	{
		...
		case BOTTOM:
			offset.x =  1; break;
		default:
			ASSERT(0, "unknown dir type"); break;
	}
	return offset;
}
...
inline NetWalkGuider::Pins NetWalkGuider::find_grm_net_pins    (const std::string& tile_type, const std::string& grm_net) const
{
...
inline NetWalkGuider::Pins NetWalkGuider::find_tile_pin_pins   (const std::string& tile_type, const std::string& pin_name) const
{
	tile_repo::const_iterator it = tile_repo_.find(tile_type);
	ASSERT(it != tile_repo_.end(), "cannot find tile: " + tile_type);
