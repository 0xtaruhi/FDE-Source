...
{
	if (!net || !net->num_pips()) return; // check net
	...
	for (int i = 0; i < new_net_name.size(); ++i)
		if (new_net_name[i] == '/') new_net_name[i] = '-'; // change to a valid file name prefix
...
void RDEngine::build_net_tree(COSRTNet* net)
{
	...
	for (const RDTree::Pip& pip: cur_tree_->pips())
	{
		...
		if (Pin* sink_pin = cur_tree_->find_sink_pin_by_seg(to))
		{
			...
			RDNode* onode = cur_tree_->add_node(sink_pin->path_name(), RDNode::OUTPUT, driver, 0);
...
void RDEngine::set_seg(RRGNode* seg)
{
	...
	for (switch_repo::iterator it = cur_switch_repo_.begin(); it != cur_switch_repo_.end(); ++it)
	{
...
void RDEngine::stretch_pins_to_adjacent_tiles(const TilePins& tpins, RDNode* driver, const Point& driver_pos)
{
	// check switch nodes
	...
	for (const TilePin* tpin: tpins)
	{
		...
		if (to_pos.x < 0 || to_pos.x >= scale.x || to_pos.y < 0 || to_pos.y >= scale.y)
			continue; // skip illegal pos

		// check arch inst
		ArchInstance* to_inst = FPGADesign::instance()->get_inst_by_pos(to_pos);
		ASSERT(to_inst, "cannot find arch instance: " + lexical_cast<string>(to_pos));

		RDNode* line_node = cur_tree_->add_node( line_node_name(driver_pos, to_pos), RDNode::LINE, driver );
...
void RDEngine::stretch_tile_pin_nodes(RDNode* driver, const string& tpin_name, ArchInstance* inst)
{
	TilePins tpins = walk_guider_.find_tile_pin_pins(inst->down_module()->name(), tpin_name);
...
void RDEngine::check_switch_attach(const Point& pos, RDNode* driver)
{
	...
	if (it == cur_switch_repo_.end()) return;
	...
	if (driver->type() == RDNode::LINE || driver->type() == RDNode::INPUT)
	{
		for (RRGSwitch* rrg_sw: it->second)
		{
			...
			RDNode* sw_node = cur_tree_->add_node( node_name, RDNode::SWITCH, driver, rrg_sw );
	...
	// switch
	else if (driver->type() == RDNode::SWITCH || driver->type() == RDNode::DUMMY) // normal switch or dummy switch
	{
		RRGSwitch* next_sw = 0;
		// find the next switch driver driven by now seg
		for (RRGSwitch* rrg_sw: it->second)
			if (rrg_sw->from_node() == cur_seg_) { next_sw = rrg_sw; break; }
		...
		if (next_sw)
		{
			...
			RDNode* next_driver = cur_tree_->add_node( switch_node_name(next_sw, cur_seg_), RDNode::SWITCH, driver, next_sw );

			for (RRGSwitch* rrg_sw: it->second)
				if (rrg_sw != next_sw)
				{
					// connect all other switches to next driver
					cur_tree_->add_node( switch_node_name(rrg_sw, cur_seg_), RDNode::SWITCH, next_driver, rrg_sw );
				}
		}
	}
}
...
void RDEngine::create_src_segment_nodes()
{
	...
	RDNode*  driver    = cur_tree_->add_node(cur_tree_->driver_pin()->path_name(), RDNode::INPUT, 0);
	cur_tree_->set_src_node_pin(driver, cur_tree_->driver_pin());

	// note: using from_net of RRGSwitch
	TilePins tpins  = walk_guider_.find_grm_net_pins(inst->down_module()->name(), rrg_sw->from_net()->name());
...
{
	ArchInstance* inst = FPGADesign::instance()->get_inst_by_pos(rrg_sw->pos());
	TilePins tpins = walk_guider_.find_grm_net_pins(inst->down_module()->name(), rrg_sw->to_net()->name());
	stretch_pins_to_adjacent_tiles(tpins, driver, rrg_sw->pos());
}


}} // end namespace FDU::STA
