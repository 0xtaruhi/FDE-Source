...
protected:
	...
	{ return (boost::format(cur_tree_->line_node_name_rule()) % cur_seg_->full_name() % from % to).str(); }

	string switch_node_name(RRGSwitch* sw, RRGNode* seg) const 
	{ return ( boost::format(cur_tree_->switch_node_name_rule())
			% seg->full_name() % sw->from_net() % sw->to_net() % sw->pos() ).str(); }
    ...
    void stretch_tile_pin_nodes(RDNode* driver, const string& tpin_name, ArchInstance* inst);
	void stretch_pins_to_adjacent_tiles(const TilePins& tpins, RDNode* driver, const Point& driver_pos);
