...
void    RDNode::calc_cap  ()
{
	...
	{
		set_cap_out(c_ext);
		if (type() == DUMMY) set_cap_in(c_ext);
	}
...
{ set_onode_cap_in(); root()->calc_cap(); }

void    RDTree::calc_all_nodes_delay()
{ root()->calc_delay(); }
...
void    RDTree::set_delay_data()
{
	...
	{
		Pin* pin = onode_pin.second; RDNode* node = onode_pin.first;
...
void RDTree::dfs_visit_seg(RRGNode* node, RDTree::seg_visit_map& nodes_repo, RDTree::seg_conn_map& conn_repo)
{
	...
	for (RRGNode* out_node: out_nodes)
	{
		...
		if (!nodes_repo[out_node]) dfs_visit_seg(out_node, nodes_repo, conn_repo);
	}
...
void RDTree::dump_spice_netlist( const std::string & name )
{
	...
	RDNode * input = root();
	sp << ".SUBCKT " << name << " IN ";
	for (node_pin_map::iterator it = sink_nodes().begin(); it != sink_nodes().end(); it++)
		sp << " " << it->first->index();
	sp << endl;
...
void RDTree::dump_spice_node(RDNode * driver, ofstream & sp, int & i)
{
	if (driver->type() == RDNode::OUTPUT)
	{
		...
		sp << "R" << i++ << " "<< driver->index() << " " << parent->index() << " 0 " << endl;
	}
	else if (driver->type() == RDNode::SWITCH || driver->type() == RDNode::DUMMY)
	{
		// switch
	...
	for (RDNode* child: driver->children())
	{	// handle child
...
void RDTree::dump_dot_node( RDNode * driver, ofstream & dot )
{
	for (RDNode* child: driver->children())
	{	// handle child
		dot << "\"" << driver->name() << "\"\t->\t\"" << child->name() << "\";" << endl;
 ...
 *    sp << ".SUBCKT " << name << " IN ";
 *
 *    for(map<INode*, Pin*>::iterator it = this->getOutputs().begin(); it != this->getOutputs().end(); it++)
 *        sp << " " << it->first->getIdx();
 *
 *    sp << endl;
 *
 ...
 *
 *
 *void IGraph::dumpSpSt(INode* driver, ofstream& sp, int& i[>, set<string>& sws<])
 *{
 *    if (driver->getType() == INodeType::OUTPUT) {
 *        INode* pre = static_cast<INode*>(*(driver->preNodes().begin()));
 *        sp << "*** OUTPUT *** " << driver->name() << endl;
 *        sp << "R" << i++ << " "<< driver->getIdx() << " " << pre->getIdx() << " 0 " << endl;
 *    }
 ...
 *    for (Node* adj: driver->adjNodes()) {
 *        INode* iadj = static_cast<INode*>(adj);
 *        dot << "\"" << driver->name() << "\"\t->\t\"" << iadj->name() << "\";" << endl;
 *        dumpDotSt(iadj, dot);
 *    }
 *}
 *
...
RDNode*   RDFactory::make_node(const std::string& name, RDNode::RDNODE_TYPE type, RDNode* parent, RRGSwitch* sw /* = 0 */)
{
