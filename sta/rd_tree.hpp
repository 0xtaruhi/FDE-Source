...
public:
	...
	RDNode (const std::string& name, RDNODE_TYPE type, RDNode* parent, RRGSwitch* sw = 0)
	...
	ICPath* ic_path() const
	{
		...
		if (type() == LINE)
		{
			...
			ASSERT(module, "cannot found module: " << "LINE" << " in STA Interconnect Lib");
		...
		ASSERT(module, "cannot found module: " << icmod << " in STA Interconnect Lib");
		int path_idx = arch_path->type();
		if (path_idx > 0)
		{
			FDU_LOG(DEBUG) << "the fdp3 con lib is not integrity now, use arch path with 0 type";
			path_idx = 0;
		}
		ICPath*   path      = module->get_path(path_idx); ASSERTS(path);
		return path;
	}
	...
	double            cap_out() const { return cap_out_;  }
	      ...
	      nodes_type children()       { return       nodes_type(children_.begin(), children_.end()); }
	const_nodes_type children() const { return const_nodes_type(children_.begin(), children_.end()); }
...
public:
	...
	{ calc_all_nodes_load (); calc_all_nodes_delay(); set_delay_data(); }


	RDNode* add_node(const std::string& name, RDNode::RDNODE_TYPE type, RDNode* parent, RRGSwitch* sw = 0);

	void    initial(const RRGraph& rrg)
	{ build_src_node(rrg); build_snk_nodes(rrg); build_pips(rrg); }

	RDNode* root()        { return src_node_.first; }
	Pin*    driver_pin()  { return src_seg_.second; }

	Pin*    find_sink_pin_by_seg(RRGNode* seg)
	{ seg_pin_map::iterator it = snk_segs_.find(seg); return it != snk_segs_.end() ? it->second : 0; }
	...
	const_pips_type  pips() const { return const_pips_type(pips_.begin(), pips_.end()); }

	string  switch_node_name_rule() const { return "%s_sw_%s_to_%s_at_%s"; }
	string    line_node_name_rule() const { return "%s_line_%s_to_%s";     }

	// void    set_src_node_pin(RDNode* src_node, Pin* pin) { src_node_.first = src_node; src_node_.second = pin; }
	void    set_src_node_pin(RDNode* src_node, Pin* pin) { src_node_ = std::make_pair(src_node, pin); }
	void    add_snk_node_pin(RDNode* snk_node, Pin* pin) { snk_nodes_.insert(std::make_pair(snk_node, pin));       }
...
protected:
	...
	void    dfs_visit_seg(RRGNode* node, seg_visit_map& nodes_repo, seg_conn_map& conn_repo);
...
public:
	...
	virtual	~RDFactory()	{}
	virtual RDNode*   make_node(const std::string& name, RDNode::RDNODE_TYPE type, RDNode* parent, RRGSwitch* sw = 0);
...
inline RDNode* RDTree::add_node(const std::string& name, RDNode::RDNODE_TYPE type, RDNode* parent, RRGSwitch* sw /* = 0 */)
{
	// ASSERTD(!nodes_.count(name), "node already exists: " + name);
	if (sw && sw->type() == RRGSwitch::DUMMY) type = RDNode::DUMMY;
	...
	// allow repeated nodes for there maybe not only one pip at the same pos in one net
	// FDU_LOG(DEBUG) << "node already exists: " + name << std::endl;
	return it->second;
}
