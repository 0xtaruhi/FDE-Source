...
void STAApp::try_process(int argc, char* argv[])
{
	...
	try
	{
		...
		//! 1. parse command
		if (!arg_->parse_command(argc, argv)) return;
		//! 2. load files
		load_files();
		//! 3. run STA
...
void STAApp::load_files()
{
	...
	FDU_LOG(INFO) << (progress_fmt % "10" % ("loading arch library \"" + arg_->arch + "\"")).str();
	try { ARCH::FPGADesign::loadArchLib(arg_->arch, new RRGArchFactory()); }
	catch(exception& e)
	{
	...
	FDU_LOG(INFO) << (progress_fmt % "20" % ("loading netlist \"" + arg_->input + "\"")).str();
	CosFactory::set_factory(new COSRTFactory());
	try { design_->load("xml", arg_->input);}
	catch(exception& e)
	{
	...
	FDU_LOG(INFO) << (progress_fmt % "30" % ("loading interconnect library \"" + arg_->iclib + "\"")).str();
...
{
	FDU_LOG(INFO) << (progress_fmt % "80" % ("saving netlist \"" + arg_->output + "\"")).str();
	try {
