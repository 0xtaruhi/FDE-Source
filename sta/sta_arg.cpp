...
bool STAArg::parse_command(int argc, char* argv[])
{
	try
	{
		...
		visible.add_options()
			("arch,a",       value<string>(),   "arch lib")
			("input,i",      value<string>(),   "design input")
			("output,o",     value<string>(),   "design output")
			("iclib,l",      value<string>(),   "interconnect lib")
			("report,r",     value<string>(),   "report file")
			...
			("dot,d",                           "dump dot file for each net")
			("help,h",                          "help"    )
			;

		options_description hidden("Hidden options");
		hidden.add_options()
			("noencrypt,e",                         "whether to use nonencrypt output")
			;
