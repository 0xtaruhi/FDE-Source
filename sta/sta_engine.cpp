...
// STAEngine
	...
	{
		_primary_inputs.clear(); _primary_outputs.clear();
	...
	void STAEngine::find_seq_primitives(Instance& inst, int t_idx)
	{
		...
		for (Instance* prim: inst.down_module()->instances()) {
			Instance::pin_iter p_it = find_if(prim->pins(), [](const Pin* pin) { return pin->type() == CLOCK; });
	...
	{
		Property< vector<TimingPoint> >& TP = create_temp_property< vector<TimingPoint> >(PIN, "timingpoint");
	...
	void STAEngine::trace_from_primary_output(STANode* po, list<STANode*>& path, bool rise, int i)
	{
		path.clear();
		path.push_back(po);
		...
		while(!_primary_inputs.count(cur_node)){
			for (const TEdge* iedge: cur_node->in_tedges()){
				...
				double cur_tarr   = rise ? cur_node->t_arr(i)._rising : cur_node->t_arr(i)._falling;
