...
// io streams
	...
	istream& operator >> (istream& s, STAReport::REPORT_MODE& mode) { mode = rp_modes_map.readEnum(s); return s; }
	ostream& operator << (ostream& s, STAReport::REPORT_MODE  mode) { return rp_modes_map.writeEnum(s, mode); }

//////////////////////////////////////////////////////////////////////////
// STA Report

	void STAReport::export_report(const std::string& fname, int num_paths, REPORT_MODE mode /* = R2R */)
	{
		...
		switch(mode)
		{
			case R2R: report_r2r(rpt, num_paths); break;
			case R2O: report_r2o(rpt, num_paths); break;
			case I2R: report_i2r(rpt, num_paths); break;
		}
		rpt.close();
	}
	...
	void STAReport::report_title(std::ofstream& rpt) const
	{
		...
		rpt << design_fmt % "Design file: " % _sta_engine->target_design()->name() << endl;
	...
	void STAReport::report_r2r(std::ofstream& rpt, int num_paths) const
	{
		...
		if (min_t_period > 0.) {
			rpt << "Minimum Period is " << boost::format("%.2f") % min_t_period << "ns" << endl;
			rpt << "Maximum Frequency is " << boost::format("%.2f") % (1. / min_t_period * 1e3) << "MHz" << endl;
		} else
			rpt << "[Warning] NO registers found!" << endl;
...
//				<< " " << _sta_engine->_clk_nets[i]->name() << " "	
			...
			for (auto [dff, t_idx]: _sta_engine->_dffs)
			{
				...
				if(dff_po->t_arr(i)._rising > 0 && dff_po->t_arr(i)._falling > 0){
					...
					//					double t_setup_rising  = (*it_node_r)->t_arr(i)._rising  - (*(++it_node_r))->t_arr(i)._rising;
					//					double t_setup_falling = (*it_node_f)->t_arr(i)._falling - (*(++it_node_f))->t_arr(i)._falling;

					max_delay_r  = max(ptr_po_r->t_arr(i)._rising/*  - t_setup_rising*/,  max_delay_r);
					max_delay_f  = max(ptr_po_f->t_arr(i)._falling/* - t_setup_falling*/, max_delay_f);
					max_period_r = max(ptr_po_r->t_arr(i)._rising,  max_period_r);
					max_period_f = max(ptr_po_f->t_arr(i)._falling, max_period_f);
...
//		}
		...
		for(int i = 1; i < _sta_engine->t_domains(); i++)
		{
			...
			for (STANode* dff_po: _sta_engine->_primary_outputs)
			{
				if (!dff_po->owner()->is_mpin())
					continue;
				//ASSERTD(dff_po, Error("dff does NOT have corresponding primary output."));
	...
	void STAReport::report_paths(ofstream&	rpt, 
								 PathSort&	paths, 
								 bool		rise,
								 double		cur_domain_max_delay, 
								 int		cur_domain, 
								 int		num_paths) const
	{
		...
		for (PathSort::value_type& path: paths | reversed){
			...
			rpt << src_des_fmt % "Source: "										\
							   ...
							   % (*nodes.rbegin())->owner()->instance()->down_module()->name() << endl;

			rpt << src_des_fmt % "Destination: "								\
							   % (*nodes.begin())->owner()->instance()->name()	\
							   % (*nodes.begin())->owner()->instance()->down_module()->name() << endl;
			...
			for (STANode* node: nodes | reversed){
				...
				tarr = rise ? node->t_arr(cur_domain)._rising : node->t_arr(cur_domain)._falling; 
				rpt << pin_fmt % dir % pin_name % tarr << endl;
			}
			rpt << table_fmt % "" % "-----" << endl;
			rpt << slack_fmt % "Slack: " % (cur_domain_max_delay - tarr) << "\n\n";
			if(idx_of_path >= num_paths) break;
		}
	}
