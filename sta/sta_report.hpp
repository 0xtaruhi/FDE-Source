...
namespace FDU { namespace STA {
	...
	public:
		...
		//void export_report(const std::string& fname, int num_paths, REPORT_MODE mode = R2R);
		void export_report(const std::string& fname, int num_paths, REPORT_MODE mode = I2R);
	...
	protected:
		...
		// for all modes
		void report_paths(std::ofstream& rpt, 
						  PathSort&		 paths, 
						  bool			 rise,
