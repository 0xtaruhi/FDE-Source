...
namespace VL2XML_PARSER{
	...
	VerilogDesign::~VerilogDesign() {
		...
		for (list<VerilogModule *>::iterator it = modules.begin();it != modules.end();++it)
		{
			delete (*it);
		}
	}

	//int	VerilogDesign::ParseDesign(vector<string> srcList, string outXMLname, string outVerilogname, string libraryName)
	//{
	//	_sourceList = srcList;
	...
	//	for (vector<string>::iterator it = _sourceList.begin();it != _sourceList.end();++it)
	//	{
...
#else
				Module *newcell = cell_lib->create_module((*it)->name, "GENERIC");//for library
				design->set_top_module(newcell);
				(*it)->ConvertModules(*newcell, *cell_lib, modules);
#endif
				/*#######there is some problem here.When a verilog file like the one with just port and module name in it ########
				#########i can't put the module in the cell lib.so the program is not general enough right now like modelSim######*/
			}
		}
		for (list<VerilogModule *>::iterator it = modules.begin(); it != modules.end(); it++)
		{
	...
	int VerilogDesign::clarifyDesign(Library* cell_lib){
		...
		/*  should be put out of the clarify function because it is a compusalry function for the whole parser*/

		/*	<4> find if the instance ports connection is conformed with the library*/
		/*		no use now */
	...
	int VerilogDesign::clarifyModules(Library& cell_lib){
		/* using the simple assumption that a module which is never instantiated is the top module*/
		/* however, this assumption is totally wrong when there are one or more module which are  */
		/* never used*/
		//use for each or standard lib to implement this instead of for loop
		for (list<VerilogModule*> ::const_iterator itM = modules.begin();itM != modules.end();++itM) {
			for(list<Instantiation *>::iterator itI = (*itM)->instantiations->begin(); itI != (*itM)->instantiations->end(); itI++) {
				for (list<VerilogModule*> ::iterator t_itM = modules.begin();t_itM != modules.end();++t_itM) {
		...
		for (list<VerilogModule*> ::const_iterator itM = modules.begin();itM != modules.end();++itM) {
			...
			if (cell_lib.find_module((*itM)->name))
				std::cout << "Warning : <module> : " << (*itM)->name << " is redefined! Please check your cell lib and verilog files." << std::endl;
		}
		if (cnt > 1) {
			std::cout<<"There are more than one module which are never instantiated!" << std::endl;
		}
		else if (cnt == 1) {
			std::cout << "The Top Module is " <<(*_topModule).name << std::endl;
		}
		else
			std::cout << "There is no one module which is not instantiated!" << std::endl;
	...
	void VerilogDesign::RegisterModules(){
		...
		//for (list<Module*> ::const_iterator itM = modules.begin();itM != modules.end();++itM)
		//{
