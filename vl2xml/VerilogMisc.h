...
namespace VL2XML_PARSER{
	...
	public:
		...
		Expression               *value;
		int lineno;  //line number
		PortConnection() { position = -1; value = NULL; lineno=0; }
		PortConnection(const string &n, int p, Expression *v) { name = n; position = p; value = v; lineno=0;}
	...
	public: 
		...
		int lineno;  //line number
		Instantiation() { parameters = NULL; connections = NULL; lineno=0;}
		// Destructor
		~Instantiation();
	};
	...
	public:
		...
		Statement                  *action;
		int lineno;  //line number
		Function() { start = stop = NULL; declarations = NULL; action = NULL; lineno=0;}
		// Destructor
		~Function();
	};
	...
	public:
		...
		Primary(const string &n) { 
			...
			arguments = NULL; lineno=0;
		}
		Primary(const string &str, int bitwidth){
			strValue = str; number.bitWidth = bitwidth; type = CONST;
			range.start = range.stop = NULL;arguments = NULL;number.negative = false; lineno=0;
		}
		...
		Primary(const string &n, Expression *i) { 
			...
			name = n; range.start = i = range.stop = i; type = NET; 
			arguments = NULL; lineno=0;
		}
		...
		Primary(const string &n, list<Expression *> *args) { 
			...
			range.start = range.stop = NULL;  lineno=0;
		}
	...
	public:
		...
		int  lineno;  //line number
		Expression                   *op2, *op3;
		Expression()                                { op1 = op2 = op3 = NULL; type = UNKNOWN;lineno=0;}

		/* this constructor is used to construct the expression for all the primary nets */
		/* a primary net is a set of net with different index while sharing same identifier */
		Expression(Primary *p)                      { primary = p; type = PRIMARY;lineno=0; }

		/* this constructor is used to construct the expression for a set of nets which is concatenated */
		/* the function of this set of nets is the same with the primary i.e the real wires, regs or ports */
		Expression(Bundle *b)                       { bundle = b; type = BUNDLE; lineno=0;}

		Expression(Expression *e1)                  { op1 = e1; op2 = op3 = NULL; type = UNKNOWN; lineno=0;}
		Expression(Expression *e1, Expression *e2)  { op1 = e1; op2 = e2; op3 = NULL; type = UNKNOWN;lineno=0; } 
		Expression(Expression *e1, 
			Expression *e2,
			Expression *e3)                  { op1 = e1; op2 = e2; op3 = e3; type = UNKNOWN;lineno=0; } 
	...
	public:
		...
		Expression              *value;
		int lineno;  //line number
		Declaration() { start = stop = start2D = stop2D = value = NULL; lineno=0; }
		// Destructor
		~Declaration();
	};
	...
	public:
		...
		Assignment() { lval = value = NULL; lineno=0; }
		// Destructor
		~Assignment();
		int lineno;  //line number
	};
	...
	public:
		...
		Bundle(list<Expression*>* m) { members = m; replication = NULL; lineno = 0; }
		Bundle(list<Expression*> *m, class Expression *r) { members = m; replication = r; lineno=0;}
		// Destructor
		~Bundle();
		int lineno;  //line number
	};
