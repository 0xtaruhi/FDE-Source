...
#define USAGE "Usage: import {infile1 [infile2] [infile3]...} -v -x [out_xml_file] -c -h"
#define print_usage(s) { std::cout << (s) << std::endl << desc << std::endl; return 1; }

using VL2XML_PARSER::VerilogDesign;

using namespace COS;
using std::ofstream;
VerilogDesign	*yydesign;//can't put that in the VL2XML_PARSER because it be become localized to the yyparse. Just 
							//get the VerilogDesign out of the namespace.
extern FILE *yyin;
...
int Vl2xmlApp::ProcessArgs(int argc, char* argv[]){
	try {
		...
		desc.add_options()
			...
			("fde,f",	po::value<string>(), "set fde file")
			("celllib,l", po::value<string>(), "set cell library")
			("input,i", po::value<vector<string>>(), "input file(s) to be parsed")/*need to be modified to support multiple file input*/
			("check,c", "check names and types")
			("xml,x",  po::value<string>(), "export xml file")
			("report,r", "export report file")
...
int Vl2xmlApp::parseDesign(){
	...
	for (vector<string>::iterator it = _SrcFileList.begin();it != _SrcFileList.end();++it)
	{
...
int Vl2xmlApp::readLibrary(string libraryFile){
	...
	//cell_lib = _XMLDesign->libs().find("cell_lib");//maybe later the name in the find should change to a variable
...
int Vl2xmlApp::saveXML(){
	...
	try
	{
		...
		std::cout << "An error happened in saving xml! The reasong is listed below :" << std::endl;
		std::cout << e.what() << std::endl;
		return 1;
	}
	POP_STEP;
