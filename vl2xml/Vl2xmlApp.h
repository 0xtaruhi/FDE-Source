...
namespace VL2XML_PARSER{
	...
	/*		this class only provide necessary functions and members to support the user to control the parser		*/
	class Vl2xmlApp{
	public :
		Vl2xmlApp():_needClarify(false),
					_needExportXML(false),
					_needReport(false),
					_needVerilog(false),
					_needRename(true),
					_needEncrypt(true),
	...
	//				_topCircuit(new Cell)
		...
		VerilogDesign		*_verilogDesign;			//only one design at a time containing one or more than one files
		//Cell				*_topCircuit;				//only one top module
		string				_prjName;
		...
		//MapInstance*		_MapInstance;				//pointer to array of MapInstance
