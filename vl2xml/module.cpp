...
namespace VL2XML_PARSER {
	...
	VerilogModule::VerilogModule() :design(NULL), ports(NULL), assignments(NULL), alwaysBlocks(NULL), initialBlocks(NULL), 
						functions(NULL), instantiations(NULL), IsTopModule(true), IsConverted(false){}
	...
	VerilogModule::~VerilogModule() {
		...
		// delete assignments
		if (assignments) {
			for(list<Assignment *>::iterator it=assignments->begin(); it!=assignments->end(); it++) 
				delete (*it);
			delete assignments;
		}
		// delete always blocks
		if (alwaysBlocks) {
			for(list<AlwaysBlock *>::iterator it=alwaysBlocks->begin(); it!=alwaysBlocks->end(); it++) 
				delete (*it);
			delete alwaysBlocks;
		}
		// delete initial blocks
		if (initialBlocks) {
			for(list<Statement *>::iterator it=initialBlocks->begin(); it!=initialBlocks->end(); it++) 
				delete (*it);
		...
		if (instantiations) {
			for(list<Instantiation *>::iterator it=instantiations->begin(); it!=instantiations->end(); it++) 
				delete (*it);
			delete instantiations;
		}
		// delete declarations
		for(list<Declaration *>::iterator it=declarations.begin(); it!=declarations.end(); it++) 
			delete (*it);
		// delete parameters
		for(list<Declaration *>::iterator it=parameters.begin(); it!=parameters.end(); it++) 
			delete (*it);
		// delete parameter overrides
		if (parameterOverrides) {
			for(list<Declaration *>::iterator it=parameterOverrides->begin(); it!=parameterOverrides->end(); it++) 
				delete (*it);
	...
	int VerilogModule::clarifyPorts(){
		return 0;
	...
	int	VerilogModule::clarifyInstanceNames(){
		return 0;
	}

	int	VerilogModule::clarifyNetNames(){
		return 0;
	}

	int VerilogModule::clarifyPortNames(){
		return 0;
	}

	int VerilogModule::clarifyInstanceType(list<VerilogModule*> &modulelist, Library* cell_lib){
		bool ModuleIsDefined = false;

		for(list<Instantiation *>::iterator itI = instantiations->begin(); itI!=instantiations->end(); itI++)
		{
			if (cell_lib->find_module((*itI)->type))
			{
				...
				for (list<VerilogModule*> ::iterator itM = modulelist.begin();itM != modulelist.end();++itM)
				{
				...
				{
					/*need more care*/
					std::cout << "Can't find " << (*itI)->type << " in neither CellLibrary and Module list" << std::endl;
				}
			}
		}
		return 0;
	...
	int VerilogModule::clarifyInstanceConnection(const list<VerilogModule*> &modulelist)
	{
		return 0;
	}

	int VerilogModule::ConvertModules(Module &cell, Library& cell_lib, list<VerilogModule*>& modules){
		...
		{
			std::cout <<"line  "<<lineno<< ":  An error happened in ConvertParameters!" << std::endl;
		}

		if (ConvertNets(cell, cell_lib, lineno))
		{
			std::cout << "line  " << lineno<< ":  An error happened in ConvertNets!" << std::endl;
		}

		if (ConvertInstances(cell, cell_lib, modules, lineno))
		{
			std::cout << "line  " << lineno << ":  An error happened in ConvertInstances!" << std::endl;
		}

		if (ConvertAssignments(cell, cell_lib, lineno))
		{
			std::cout << "line  " << lineno << ":  An error happened in ConvertAssignments" << std::endl;
		}

		if (ConvertAttributes(cell, cell_lib, lineno))
		{
			std::cout << "line  " << lineno << ":  An error happened in ConvertAttributes" << std::endl;
		}
		if (ConvertParameterOverrides(cell, cell_lib, lineno))
		{
			std::cout << "line  " << lineno << ":  An error happened in ConvertParameterOverrides" << std::endl;
		}
	...
	int VerilogModule::ConvertParameters(Module& cell, Library& cell_lib, int &lineno){
		PUSH_STEP;
		// convert parameters
		...
		for(list<Declaration *>::iterator it=parameters.begin(); it!=parameters.end(); it++) {
			...
			/* Used for pass property */
				...
				COS::Property<string>& temp = COS::create_property<string>(COS::MODULE, (*it)->name);
	...
	int VerilogModule::ConvertNets(Module &cell, Library &cell_lib, int &lineno){
		...
		PUSH_STEP;
		for(list<Declaration *>::iterator itD = declarations.begin(); itD != declarations.end(); ++itD)
		{
			...
			if (t == Declaration::INPUT || t == Declaration::OUTPUT || t == Declaration::INOUT)
			{
				if (!cell.ports().find(decl_name))
				{
					/* Now treat the "input [n:n] ...;" declaration like a port group. So when connect instance, find name with or without [n] will be good enough */
					if ((rstart == -1 && rstop == -1))
					{
					...
					{
#ifndef _BUILD_CELLLIBRARY2_0
						Port* pg = cell.create_port(decl_name, rstart, rstop, COS::DirType(t));
						for (Module::pin_iter pi = pg->mpins().begin(); pi != pg->mpins().end(); ++pi)
						{
							string n = pi->name();
							if(!instantiations->empty())
							{
...
#else
						for (list<string>::iterator itS = namelist.begin();itS != namelist.end();++itS)
						{
			...
			else//create net i.e wire/reg
			{
				for (list<string>::iterator itS = namelist.begin();itS != namelist.end();++itS)
				{
					//need to check whether the name is used to detect unintentional mistake and wire/reg replaced declaration for port 
					string net_name = *itS;
			...
			//delete tempprim;//delete the pointer hoping it will not delete the embedded pointer
		}
		return 0;
	}
	int VerilogModule::ConvertInstances(Module &cell, Library &cell_lib, list<VerilogModule*>& modules, int& lineno){
		Module* cell_ptr;
		for(list<Instantiation *>::iterator it=instantiations->begin(); it!=instantiations->end(); it++)
		{	
			...
			if (cell_ptr = cell_lib.find_module((*it)->type))
				;//std::cout << "Find " << (*it)->type << " in the CellLibrary!\n";
			else 
			{
				bool b = false;
				for (list<VerilogModule*>::iterator mit = modules.begin(); mit != modules.end(); ++mit) 
				{
					if ((*it)->type == (*mit)->name && !((*mit)->IsConverted)) 
					{
						std::cout << "Try to convert <module> : " << (*mit)->name << " for <instance> : " << (*it)->name << " ..." << std::endl;
						Module *newcell = cell_lib.create_module((*it)->type, "GENERIC");
						(*mit)->ConvertModules(*newcell, cell_lib, modules);
						std::cout << "Conversion of <module> :" << (*mit)->name << " for <instance> : "<< (*it)->name << " is done." << std::endl;
						b = true;break;
					}					
				}
				ASSERT(b, "line  "<<(*it)->lineno<<": Error in ConvertInstances. Module " << (*it)->name << " is not defined!\n");
			...
			for (list<PortConnection*>::iterator itPC = connections->begin();itPC != connections->end();++itPC)
			{
			...
			//Deal with named port connections: connect them. Note:support net connection without exlicitly declare it if it's single bit.For multi bits, report an error.
			for (list<PortConnection*>::iterator itPC = connections->begin();itPC != connections->end();++itPC)	
			{
	...
	void VerilogModule::ConnectInstance(Module& cell, Instance& inst, list<string>& namelist, string portname){
		...
		if (pg) {
			...
			/* if namelist contains more than one wire, then the instance port is a multi-bits port. Now not */
			/* only need to scalarize the net buy also scalarize the port */
			if (namelist.size() > 1)
			{
				int index = namelist.size()-1;
				for (list<string>::iterator it = namelist.begin();it != namelist.end();++it, --index){
					if (*it != "HIGHZ!") {
						string tp = portname + "[" + lexical_cast<string>(index)  + "]";//remember scale the port from high to low	because the namelist contain nets from high to low					
				...
				if (n != "HIGHZ!"){
					if (inst.find_pin(portname)) 
					{
						...
						{
							std::cout << "Warning : <net> : " << n << " is used without explicitly declaration!" << std::endl;
					...
					{
						if (cell.find_net(n))
							inst.find_pin(portname + "[" + lexical_cast<string>(0) + "]")->connect(cell.find_net(n));
						else
						{
							std::cout << "Warning : <net> : " << n << " is used without explicitly declaration!" << std::endl;
							inst.find_pin(portname + "[" + lexical_cast<string>(0) + "]")->connect(cell.create_net(n));
						}
					}
					else
					{
						//can't match any possible port name in cell library and other sub modules
						std::cout << "Parser can't find <port>: " << portname <<" or " << portname + "[" + lexical_cast<string>(0) + "]" << " in <instance>: " << inst.name() << std::endl;
						std::cout << " Please check if you use the right cell library or any spelling mistakes." << std::endl;
						exit(1);
					}
	...
	int VerilogModule::ConvertAssignments(Module& cell, Library& cell_lib, int& lineno){
		/* actually after the assignments, the two nets on the both sides of '=' is all the same	*/
		/* so now delete one which is type 'wire' and keep the port one								*/
		/* if both of the net is type 'wrie',then delete the left one								*/

		/* to handle the assignments ,first divide the nets involved into separate equivalent groups */
		map<string, int> eg;
		map<string, int>::iterator iter;
		int group_no = 0;
		for (list<Assignment*>::iterator itA = assignments->begin();itA != assignments->end();++itA)
		{
			...
			{
				std::cout << "line  " << (*itA)->lineno <<":  Error in assignment. The width on both sides are not the same!" << std::endl;
				exit(1);
			}
			for (;lit != lnetname.end(); ++lit, ++rit)
			{
				...
				{
					std::cout << "line  " << (*itA)->lineno << "Error!!Redundant assignments appear!" << std::endl;
				}
			}
			//for (;lit != lnetname.end(); ++lit, ++rit)
			//{
	...
	int VerilogModule::ConvertParameterOverrides(Module& cell, Library& cell_lib, int& lineno)
	{
		for(list<Declaration *>::iterator it=parameterOverrides->begin(); it!=parameterOverrides->end(); it++) {
			...
			string s3 = name0.substr(0, name0.find_last_of('.'));//only support ' ' i.e space.
			if (s3[0] == '\\')
				;
			else
				s3 = s3.substr(0, s3.find_first_of(' '));

			COS::Property<string>& temp = COS::create_property<string>(COS::INSTANCE, s2);
			if (p->type == Primary::CONST)
			{
				...
				{
					std::cout <<"line  "<<(*it)->lineno<< " Error : <instance> : " << s3 << " doesn't exit! Please check your verilog files." << std::endl;
					exit(1);
				}						
			}
	...
	int VerilogModule::ConvertAttributes(Module& cell, Library& cell_lib, int& lineno){
		COS::Property<Point>& temp1 = COS::create_property<Point>(COS::INSTANCE, "rloc");
		COS::Property<Point>& temp2 = COS::create_property<Point>(COS::INSTANCE, "h_set");
		COS::Property<Point>& temp3 = COS::create_property<Point>(COS::INSTANCE, "mode");

		for (list<Attribute *>::iterator it = attributes->begin(); it != attributes->end();++it){
			//cell.instances().find((*it)->owner)->set_property<string>("cfgMode", (*it)->attr, PERSIST);
			if ((*it)->type == "rloc")
			{
				cell.find_instance((*it)->owner)->set_property(temp1, lexical_cast<Point>((*it)->attr));
			}
			else if ((*it)->type == "h_set")
			{
				cell.find_instance((*it)->owner)->set_property(temp2, lexical_cast<Point>((*it)->attr));
			}
			else if ((*it)->type == "mode")
			{
				cell.find_instance((*it)->owner)->set_property(temp3, lexical_cast<Point>((*it)->attr));
			}	
		lineno = (*it)->lineno;	
	...
	void VerilogModule::PrimaryOrBundle(Expression* expr, list<string>& namelist, Module& cell, COS::Library& cell_lib){
			...
			if (prim->type == Primary::NET)
			{
				...
				else if (xm)
				{
					...
					if (!cell.instances().find("logic1_VCC")){
						...
						inst->find_pin("LOGIC_1_PIN")->connect(cell.nets().find("logic1_rails"));
					}
				}
					...
					if (!cell.instances().find("logic0_GND")){
						...
						inst->find_pin("LOGIC_0_PIN")->connect(cell.nets().find("logic0_rails"));
					}
				}
				if ((!zm) && (!xm))
				{
	...
	void VerilogModule::ParsePrimaryNet(string name, Expression* start, Expression* stop, list<string> &namelist, int& rstart, int& rstop)
	{
		PUSH_STEP;
		...
		//this condition is never met in net declaration but quite common in port connections
		else if (start == stop)//single select net e.g cout[1] or 'wire [1:1] in;'
		{
			Primary *p = start->primary;
			if (p->type != Primary::CONST) CheckMapItem(_ParameterMap, "Parameter", p->name);
			int i = (p->type == Primary::CONST) ? p->number.intValue : _ParameterMap[p->name];
			netname = name + "[" + lexical_cast<string>(i)  + "]";
			/* because some identifier generated by DC will be the same with the scalar net's name		*/
			/* generated by parser. so the parser will leave the scalar net's name untouched			*/
			/* and this will cause the variable's name unconformed with Verilog standard and may cause	*/
		...
		else if (start != stop)//use the Expression(Primary*) constructor
		{
			...
			int vstart, vstop;
			if (pstart->type != Primary::CONST) CheckMapItem(_ParameterMap, "Parameter", pstart->name);
			if (pstop->type != Primary::CONST) CheckMapItem(_ParameterMap, "Parameter", pstop->name);
			vstart = (pstart->type == Primary::CONST) ? pstart->number.intValue : _ParameterMap[pstart->name];//need parse the parameter. same below
			vstop = (pstop->type == Primary::CONST) ? pstop->number.intValue : _ParameterMap[pstop->name];

			/********************scalarize net here***************************/
			if (vstart == vstop)
			{
				netname = name + "[" + lexical_cast<string>(vstart) + "]";//###if a net declared like "wire [0:0] n1;" then the n1 should be n1[0] in xml not n1###
				namelist.push_back(netname);
			}
			else if (vstop != vstart)
			{				
				//It seems modelsim only support the "\multi [0]" format. So treat the escaped and non-escaped equally.Just comment it out.

				for (int i = (vstart>vstop ? vstart : vstop); i >= (vstart>vstop ? vstop : vstart); i--)
				{
					netname = name + "[" + lexical_cast<string>(i) + "]";
					namelist.push_back(netname);
				}//assume that identifiers with "\" can only be single bit.and multi-bitwidth can't follow escaped identifier
				//need further research because the lex file seems support the escaped identifier with multi-bits}
			}
	...
	void VerilogModule::ParsePrimaryNet(Primary *prim, list<string> &namelist)
	{
		PUSH_STEP;
		string netname;
		//standard identifier the start and stop will be NULL the same time but may still by multi bits like ADDR
		//if so you need to consider that condition
		if (prim->range.start == NULL || prim->range.stop == NULL)
		{
			//use the STL algorithm find_if may perform better but i don't know it yet ^_^
			bool singlebit = true;
			for(list<Declaration *>::iterator itD = declarations.begin(); itD != declarations.end(); ++itD)
			{
				if ((*itD)->name == prim->name)
				{
					...
					ParsePrimaryNet((*itD)->name, estart, estop, namelist, no_use_a, no_use_b);
					break;
				}				
			}
			if (singlebit)
		...
		//this condition is never met in net declaration but quite common in port connections
		else if (prim->range.start == prim->range.stop)//single select net e.g cout[1] or 'wire [1:1] in;'
		{
		...
		else if (prim->range.start != prim->range.stop)//use the Expression(Primary*) constructor
		{
	...
	void VerilogModule::ParseBundleNet(Bundle *bundle, list<string> &namelist, Module& cell, Library& cell_lib){
		PUSH_STEP;
		string netname;
		list<Expression*> *exprList = bundle->members;
		for (list<Expression*>::iterator it = exprList->begin();it != exprList->end();++it)
		{
