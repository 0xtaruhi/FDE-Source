...
namespace VL2XML_PARSER{
	...
	public:
		...
		int									ConvertModules(Module &cell, Library& cell_lib, list<VerilogModule*> &modules);
		int									ConvertNets(Module &cell, Library &cell_lib, int& lineno);
		int									ConvertInstances(Module &cell, Library &cell_lib, list<VerilogModule*> &modules,int &lineno);
	...
	private:
		...
		void								ParsePrimaryNet(string name, Expression* start, Expression* stop, list<string> &namelist, int& rstart, int& rstop);
		void								ParseBundleNet(Bundle *bundle, list<string> &namelist, Module& cell, Library& cell_lib);
		void								PrimaryOrBundle(Expression* expr, list<string>& namelist, Module& cell, Library& cell_lib);
		//void								ParseConst();

		void								ConnectInstance(Module& cell, Instance& inst, list<string>& namelist, string portname);
